...
void *VideoToolboxDecoder::run()
{
	...
	while (shouldStop() == false) {
		...
		if (queueSize > 5) {
			blog(LOG_WARNING,
			     "Video Toolbox: decoding queue overloaded. %d frames behind. Please use a lower quality setting.",
			     queueSize);

			while (mQueue.size() > 0) {
				PacketItem *item =
					(PacketItem *)mQueue.remove();
				if (item != NULL) {
					blog(LOG_INFO,
					     "Video Toolbox: dropping packet type=%d tag=%d size=%d",
					     item->getType(), item->getTag(),
					     item->size());
					delete item;
				}
			}
...
static const char *video_toolbox_decode_video_name =
	"obs_camera_video_toolbox_decode_video";
void VideoToolboxDecoder::processPacketItem(PacketItem *packetItem)
{
	...
	if (naluType == 7 || naluType == 8) {
		...
		if (naluType == 7) {
			...
			spsData = std::vector<char>(packet.begin() + 4,
						    packet.end() +
							    (frameSize - 4));
		...
		if (naluType == 8) {
			...
			ppsData = std::vector<char>(packet.begin() + 4,
						    packet.end() +
							    (frameSize - 4));
		...
		if (!waitingForPps && !waitingForSps) {
			...
			const uint8_t *const parameterSetPointers[] = {
				(uint8_t *)spsData.data(),
				(uint8_t *)ppsData.data()};
			const size_t parameterSetSizes[] = {spsData.size(),
							    ppsData.size()};
			...
			status =
				CMVideoFormatDescriptionCreateFromH264ParameterSets(
					...
					NAL_LENGTH_PREFIX_SIZE, &mFormat);
			...
			if (status != noErr) {
				...
				if (mSession == NULL) {
					...
					bool needNewDecompSession =
						(VTDecompressionSessionCanAcceptFormatDescription(
							 mSession, mFormat) ==
						 false);
					if (needNewDecompSession) {
						blog(LOG_INFO,
						     "Created Decompression session");
						this->createDecompressionSession();
					}
				}
			}
	...
	CMTime presentationDuration)
{
	...
	VideoToolboxDecoder *decoder =
		static_cast<VideoToolboxDecoder *>(decompressionOutputRefCon);
...
void VideoToolboxDecoder::createDecompressionSession()
{
	...
	CFDictionarySetValue(destinationPixelBufferAttributes,
			     kCVPixelBufferPixelFormatTypeKey, number);
	CFRelease(number);

	// Format Pixel Buffer Attributes
	CFMutableDictionaryRef videoDecoderSpecification;
	videoDecoderSpecification = CFDictionaryCreateMutable(
		NULL, 0, &kCFTypeDictionaryKeyCallBacks,
		&kCFTypeDictionaryValueCallBacks);
	...
	int err = VTDecompressionSessionCreate(NULL, mFormat,
					       videoDecoderSpecification,
...
bool VideoToolboxDecoder::update_frame(obs_source_t *capture,
				       ...
				       CMVideoFormatDescriptionRef formatDesc)
{
	...
	if (!CVPixelBufferIsPlanar(imageBufferRef)) {
		...
		frame->data[0] = static_cast<uint8_t *>(
			CVPixelBufferGetBaseAddress(imageBufferRef));
		return true;
	}
	...
	for (size_t i = 0; i < count; i++) {
		frame->linesize[i] =
			CVPixelBufferGetBytesPerRowOfPlane(imageBufferRef, i);
		frame->data[i] = static_cast<uint8_t *>(
			CVPixelBufferGetBaseAddressOfPlane(imageBufferRef, i));
	}
	return true;
}
