...
#include "obs-hyper-plugin.h"

#define TEXT_INPUT_NAME obs_module_text("HyperPlugin.Title")
...
HyperInput::HyperInput(obs_source_t *source_, obs_data_t *settings)
...
void HyperInput::deviceManagerDidUpdateDeviceList(
...
void HyperInput::deviceManagerDidChangeState(
	portal::DeviceManager::State newState)
{
	blog(LOG_INFO, "deviceManagerDidChangeState %i", newState);
	std::cout << "DeviceManager::didChangeState: " << newState << std::endl;
}

void HyperInput::deviceManagerDidAddDevice(portal::Device::shared_ptr device)
{
	...
	auto deviceConnection =
		std::make_shared<portal::DeviceConnection>(device, 4973);
	...
	deviceConnectionController
		->onProcessPacketCallback = [this](auto packet) {
		try {
			switch (packet.type) {
			case 1001: // Video Packet
				this->videoDecoder->Input(
					packet.data, packet.type, packet.tag);
				break;
			case 1002: // Audio Packet
				this->audioDecoder.Input(
					packet.data, packet.type, packet.tag);
			default:
				break;
			}
...
void HyperInput::deviceManagerDidRemoveDevice(portal::Device::shared_ptr device)
{
...
void HyperInput::loadSettings(obs_data_t *newSettings)
{
	disconnectOnInactive = obs_data_get_bool(
		newSettings, SETTING_PROP_DISCONNECT_ON_INACTIVE);

	auto device_uuid =
		obs_data_get_string(newSettings, SETTING_DEVICE_UUID);
...
void HyperInput::setDeviceUUID(std::string uuid)
{
...
void HyperInput::reconnectToDevice()
{
	connectToDevice(true);
}

void HyperInput::resetDecoder()
{
	// flush the decoders
...
void HyperInput::connectToDevice(bool force)
{
	...
	// Else there is a selected device
		...
		// Connect
		auto shouldConnect =
			!(disconnectOnInactive == true && active == false);
			    ...
			    void *data)
{
	...
	auto cameraInput = reinterpret_cast<HyperInput *>(data);

	obs_property_t *dev_list =
		obs_properties_get(props, SETTING_DEVICE_UUID);
	...
	std::for_each(
		...
		[dev_list, &index](HyperInput::MobileCameraDevice &device) {
...
static const char *GetHyperInputName(void *)
{
	return TEXT_INPUT_NAME;
}

static void UpdateHyperInput(void *data, obs_data_t *settings);

static void *CreateHyperInput(obs_data_t *settings, obs_source_t *source)
{
	HyperInput *cameraInput = nullptr;

	try {
		cameraInput = new HyperInput(source, settings);
		UpdateHyperInput(cameraInput, settings);
...
static void DestroyHyperInput(void *data)
{
	delete reinterpret_cast<HyperInput *>(data);
}

static void DeactivateHyperInput(void *data)
{
	auto cameraInput = reinterpret_cast<HyperInput *>(data);
	cameraInput->deactivate();
}

static void ActivateHyperInput(void *data)
{
	auto cameraInput = reinterpret_cast<HyperInput *>(data);
	cameraInput->activate();
}

static obs_properties_t *GetHyperPluginProperties(void *data)
{
	...
	obs_property_t *latency_modes = obs_properties_add_list(
		...
		obs_module_text("HyperPlugin.Settings.Latency"),
		OBS_COMBO_TYPE_LIST, OBS_COMBO_FORMAT_INT);

	obs_property_list_add_int(
		latency_modes,
		obs_module_text("HyperPlugin.Settings.Latency.Normal"),
		SETTING_PROP_LATENCY_NORMAL);
	obs_property_list_add_int(
		latency_modes,
		obs_module_text("HyperPlugin.Settings.Latency.Low"),
		SETTING_PROP_LATENCY_LOW);

#ifdef __APPLE__
	obs_properties_add_bool(
		ppts, SETTING_PROP_HARDWARE_DECODER,
		obs_module_text("HyperPlugin.Settings.UseHardwareDecoder"));
#endif

	obs_properties_add_bool(
		ppts, SETTING_PROP_DISCONNECT_ON_INACTIVE,
		obs_module_text("HyperPlugin.Settings.DisconnectOnInactive"));

	return ppts;
}

static void GetHyperPluginDefaults(obs_data_t *settings)
{
...
static void SaveHyperInput(void *data, obs_data_t *settings)
{
	UNUSED_PARAMETER(data);
...
static void UpdateHyperInput(void *data, obs_data_t *settings)
{
	HyperInput *input = reinterpret_cast<HyperInput *>(data);
	...
	RegisterHyperPlugin()
{
	...
	info.id = "hyper-plugin";
	info.type = OBS_SOURCE_TYPE_INPUT;
	info.output_flags = OBS_SOURCE_ASYNC_VIDEO | OBS_SOURCE_AUDIO;
	info.get_name = GetHyperInputName;

	info.create = CreateHyperInput;
	info.destroy = DestroyHyperInput;

	info.deactivate = DeactivateHyperInput;
	info.activate = ActivateHyperInput;

	info.get_defaults = GetHyperPluginDefaults;
	info.get_properties = GetHyperPluginProperties;
	info.save = SaveHyperInput;
	info.update = UpdateHyperInput;
