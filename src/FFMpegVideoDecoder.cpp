...
void FFMpegVideoDecoder::processPacketItem(PacketItem *packetItem)
{
	...
	if (!ffmpeg_decode_valid(video_decoder)) {
		if (ffmpeg_decode_init(video_decoder, AV_CODEC_ID_H264, true) <
		    0) {
	...
	if (packetItem->getType() == 1001) {
		profile_start(ffmpeg_decode_video_name);

		bool got_output;
		bool success = ffmpeg_decode_video(video_decoder, data,
						   packet.size(), &ts,
						   VIDEO_RANGE_DEFAULT,
...
void *FFMpegVideoDecoder::run()
{
	...
	while (shouldStop() == false) {
		...
		if (queueSize > 5) {
			blog(LOG_WARNING,
			     "FFMpeg: Decoding queue overloaded. %d frames behind. Please use a lower quality setting.",
			     queueSize);

			while (mQueue.size() > 0) {
				PacketItem *removedItem =
					(PacketItem *)mQueue.remove();
				if (removedItem != NULL) {
					blog(LOG_INFO,
					     "FFMpeg: dropping packet type=%d tag=%d size=%d",
					     removedItem->getType(),
					     removedItem->getTag(),
					     removedItem->size());
					delete removedItem;
