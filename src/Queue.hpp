...
public:
	PacketItem(std::vector<char> packet, int type, int tag)
		: mPacket(packet), mType(type), mTag(tag)
	{
	}

	std::vector<char> getPacket() { return mPacket; }

	int getType() { return mType; }

	int getTag() { return mTag; }

	int size() { return (int)mPacket.size(); }
};
...
public:
	...
	T remove()
	{
		...
		mConditionVariable.wait(lock, [&]() {
			return m_shouldStop || !m_queue.empty();
		});
