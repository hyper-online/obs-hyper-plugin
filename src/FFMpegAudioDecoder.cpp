...
void FFMpegAudioDecoder::processPacketItem(PacketItem *packetItem)
{
	...
	if (!ffmpeg_decode_valid(audio_decoder)) {
		if (ffmpeg_decode_init(audio_decoder, AV_CODEC_ID_AAC, true) <
		    0) {
	...
	if (packetItem->getType() == 1002) {

		bool got_output;

		bool success = ffmpeg_decode_audio(audio_decoder, data,
						   packet.size(), &audio_frame,
						   &got_output);
...
void *FFMpegAudioDecoder::run()
{
	...
	while (shouldStop() == false) {
		...
		if (queueSize > 25) {
			blog(LOG_WARNING,
			     "Audio Decoding queue overloaded. %d frames behind. Please use a lower quality setting.",
			     queueSize);
