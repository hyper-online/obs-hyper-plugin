...
typedef struct _options {
	...
	uint8_t debug, in_fmt,
		out_fmt; // fmts 0 = undef, 1 = bin, 2 = xml, 3 = json someday
} options_t;
	...
	if (!options->in_file || !strcmp(options->in_file, "-")) {
		...
		while (read(STDIN_FILENO, &ch, 1) > 0) {
			if (read_size >= BUF_SIZE) {
				...
				plist_entire =
					realloc(plist_entire,
						sizeof(char) * (read_size + 1));
				if (plist_entire == NULL) {
		...
		plist_entire =
			(char *)malloc(sizeof(char) * (filestats.st_size + 1));
		read_size = fread(plist_entire, sizeof(char), filestats.st_size,
				  iplist);
		fclose(iplist);
	}
	...
	if (options->out_fmt == 0) {
		...
		if (options->out_fmt == 1) {
			if (plist_is_binary(plist_entire, read_size)) {
				...
				plist_from_xml(plist_entire, read_size,
					       &root_node);
				plist_to_bin(root_node, &plist_out, &size);
			}
		} else if (options->out_fmt == 2) {
			if (plist_is_binary(plist_entire, read_size)) {
				plist_from_bin(plist_entire, read_size,
					       &root_node);
				plist_to_xml(root_node, &plist_out, &size);
			} else {
				plist_out = malloc(sizeof(char) * read_size);
