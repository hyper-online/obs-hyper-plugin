...
#define PLIST_XML_ERR(...)                                                  \
	if (plist_xml_debug) {                                              \
		fprintf(stderr, "libplist[xmlparser] ERROR: " __VA_ARGS__); \
	}
#else
#define PLIST_XML_ERR(...)
#endif
...
static size_t dtostr(char *buf, size_t bufsize, double realval)
{
	...
	if (isnan(realval)) {
		...
		len = snprintf(buf, bufsize, "%cinfinity",
			       (realval > 0.0) ? '+' : '-');
...
static void node_to_xml(node_t *node, bytearray_t **outbuf, uint32_t depth)
{
	...
	case PLIST_UINT:
		...
		if (node_data->length == 16) {
			val_len = snprintf(val, 64, "%" PRIu64,
					   node_data->intval);
		} else {
			val_len = snprintf(val, 64, "%" PRIi64,
					   node_data->intval);
		}
		break;
	...
	case PLIST_DATE:
		...
		{
			Time64_T timev =
				(Time64_T)node_data->realval + MAC_EPOCH;
			struct TM _btime;
	...
	if (node_data->type == PLIST_STRING || node_data->type == PLIST_KEY) {
		...
		if (node_data->length > 0) {
			...
			size_t count = 0;
			size_t amount = (node_data->length / 3 * 4) + 4 +
					(((node_data->length / maxread) + 1) *
					 (indent + 1));
			...
			while (j < node_data->length) {
				...
				count = (node_data->length - j < maxread)
						? node_data->length - j
						: maxread;
				assert((*outbuf)->len + count <
				       (*outbuf)->capacity);
				(*outbuf)->len += base64encode(
					(char *)(*outbuf)->data +
						(*outbuf)->len,
					node_data->buff + j, count);
				str_buf_append(*outbuf, "\n", 1);
				j += count;
			}
		}
...
#endif
	sscanf(strval, PLIST_SSCANF_FORMAT, &btime->tm_year, &btime->tm_mon,
	       &btime->tm_mday, &btime->tm_hour, &btime->tm_min,
	       &btime->tm_sec);
	btime->tm_year -= 1900;
	btime->tm_mon--;
#endif
...
static void node_estimate_size(node_t *node, uint64_t *size, uint32_t depth)
{
	...
	if (node->children) {
		...
		*size += (depth << 1);
		...
		switch (data->type) {
		case PLIST_DATA: {
			uint32_t req_lines = (data->length /
					      MAX_DATA_BYTES_PER_LINE(indent)) +
					     1;
			uint32_t b64len = data->length + (data->length / 3);
			b64len += b64len % 4;
			*size += b64len;
			*size += (XPLIST_DATA_LEN << 1) + 5 +
				 (indent + 1) * (req_lines + 1) + 1;
		} break;
		case PLIST_STRING:
			*size += data->length;
		...
		case PLIST_DATE:
			...
			*size += (XPLIST_DATE_LEN << 1) + 6;
			break;
		case PLIST_BOOLEAN:
			*size += ((data->boolval) ? XPLIST_TRUE_LEN
						  : XPLIST_FALSE_LEN) +
				 4;
			break;
		case PLIST_DICT:
...
static void find_char(parse_ctx ctx, char c, int skip_quotes)
{
	while (ctx->pos < ctx->end && (*(ctx->pos) != c)) {
		if (skip_quotes && (c != '"') && (*(ctx->pos) == '"')) {
			...
			if (ctx->pos >= ctx->end) {
				PLIST_XML_ERR(
					"EOF while looking for matching double quote\n");
				return;
			}
			if (*(ctx->pos) != '"') {
...
static void find_str(parse_ctx ctx, const char *str, size_t len,
		     int skip_quotes)
{
...
static void find_next(parse_ctx ctx, const char *nextchars, int numchars,
		      int skip_quotes)
{
	int i = 0;
	while (ctx->pos < ctx->end) {
...
static text_part_t *text_part_init(text_part_t *part, const char *begin,
				   size_t length, int is_cdata)
{
	part->begin = begin;
	part->length = length;
...
static text_part_t *text_part_append(text_part_t *parts, const char *begin,
				     size_t length, int is_cdata)
{
...
static text_part_t *get_text_parts(parse_ctx ctx, const char *tag,
				   size_t tag_len, int skip_ws,
				   text_part_t *parts)
{
	const char *p = NULL;
	const char *q = NULL;
	...
	do {
		...
		if (*ctx->pos == '!') {
			...
			if (*ctx->pos == '-' && *(ctx->pos + 1) == '-') {
				if (last) {
					last = text_part_append(last, p, q - p,
								0);
				} else if (parts) {
					last = text_part_init(parts, p, q - p,
							      0);
				}
				ctx->pos += 2;
				find_str(ctx, "-->", 3, 0);
				if (ctx->pos > ctx->end - 3 ||
				    strncmp(ctx->pos, "-->", 3) != 0) {
					PLIST_XML_ERR(
	...
	ctx->pos++;
	if (ctx->pos >= ctx->end - tag_len ||
	    strncmp(ctx->pos, tag, tag_len) != 0) {
...
static int unescape_entities(char *str, size_t *length)
{
	...
	while (len > 0 && i < len - 1) {
		if (str[i] == '&') {
			...
			if (i >= len) {
				PLIST_XML_ERR(
					"Invalid entity sequence encountered (missing terminating ';')\n");
				return -1;
			}
			if (str + i >= entp + 1) {
...
static char *text_parts_get_content(text_part_t *tp, int unesc_entities,
				    size_t *length, int *requires_free)
{
	char *str = NULL;
	...
	while (ctx->pos < ctx->end && !ctx->err) {
		...
		if (*(ctx->pos) == '?') {
			...
			if (((ctx->end - ctx->pos) > 3) &&
				...
				ctx->pos += 3;
			} else if (((ctx->end - ctx->pos) > 8) &&
				   !strncmp(ctx->pos, "!DOCTYPE", 8)) {
				int embedded_dtd = 0;
				ctx->pos += 8;
				while (ctx->pos < ctx->end) {
					find_next(ctx, " \t\r\n[>", 6, 1);
				...
				find_next(ctx, " \r\n\t>", 5, 1);
				PLIST_XML_ERR(
					"Invalid or incomplete special tag <%.*s> encountered\n",
					(int)(ctx->pos - p), p);
				ctx->err++;
				goto err_out;
			}
			...
			if (!strcmp(tag, "plist")) {
				...
				struct node_path_item *path_item =
					(struct node_path_item *)malloc(
						sizeof(struct node_path_item));
				if (!path_item) {
					PLIST_XML_ERR(
				...
				node_path = (struct node_path_item *)
						    node_path->prev;
				free(path_item);
			...
			if (!strcmp(tag, XPLIST_DICT)) {
				...
				if (!is_empty) {
					text_part_t first_part = {NULL, 0, 0,
								  NULL};
					text_part_t *tp =
						get_text_parts(ctx, tag, taglen,
							       1, &first_part);
					if (!tp) {
						PLIST_XML_ERR(
				...
				if (!is_empty) {
					get_text_parts(ctx, tag, taglen, 1,
						       NULL);
				}
				...
				data->boolval = 0;
				data->length = 1;
			} else if (!strcmp(tag, XPLIST_STRING) ||
				   !strcmp(tag, XPLIST_KEY)) {
				if (!is_empty) {
					text_part_t first_part = {NULL, 0, 0,
								  NULL};
					text_part_t *tp =
					...
					str = text_parts_get_content(
						tp, 1, &length, NULL);
					text_parts_free(
						(text_part_t *)first_part.next);
					if (!str) {
						PLIST_XML_ERR(
							"Could not get text content for '%s' node\n",
							tag);
					...
					if (!strcmp(tag, "key") && !keyname &&
					    parent &&
					    (plist_get_node_type(parent) ==
					     PLIST_DICT)) {
						keyname = str;
						free(tag);
						tag = NULL;
				...
				if (!is_empty) {
					...
					if (tp->begin) {
						...
						if ((length >= 11) &&
							...
							PLIST_XML_ERR(
								"Invalid text content in date node\n");
						}
						if (requires_free) {
							free(str_content);
						}
				...
				if (!*plist) {
					...
					*plist = subnode;
					...
					default:
						/* should not happen */
						PLIST_XML_ERR(
							"parent is not a structured node\n");
						ctx->err++;
						goto err_out;
					}
				}
				if (!is_empty && (data->type == PLIST_DICT ||
						  data->type == PLIST_ARRAY)) {
					struct node_path_item *path_item =
						(struct node_path_item *)malloc(sizeof(
							struct node_path_item));
					if (!path_item) {
						PLIST_XML_ERR(
							"out of memory when allocating node path item\n");
...
PLIST_API void plist_from_xml(const char *plist_xml, uint32_t length,
			      plist_t *plist)
{
