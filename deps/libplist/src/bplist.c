...
uint64_t UINT_TO_HOST(void *x, uint8_t n)
{
	...
	return (n >= 8 ? be64toh(get_unaligned_64(__up.u64ptr))
		       : (n == 4 ? be32toh(get_unaligned_32(__up.u32ptr))
				 : (n == 2 ? be16toh(get_unaligned_16(
						     __up.u16ptr))
					   : (n == 1 ? *__up.u8ptr
						     : beNtoh(get_unaligned_64(
								      __up.u64ptr),
							      n)))));
}
#else
#define UINT_TO_HOST(x, n)                                                               \
	({                                                                               \
		...
		((n) >= 8                                                                \
			 ...
			 : ((n) == 4                                                     \
				    ? be32toh(get_unaligned(__up.u32ptr))                \
				    : ((n) == 2                                          \
					       ? be16toh(get_unaligned(                  \
							 __up.u16ptr))                   \
					       : ((n) == 1                               \
							  ? *__up.u8ptr                  \
							  : beNtoh(get_unaligned(        \
									   __up.u64ptr), \
								   n)))));               \
	})
#endif
...
#define uint64_mul_overflow(a, b, r) \
	__builtin_umulll_overflow(a, b, (unsigned long long *)(r))
#else
...
#define PLIST_BIN_ERR(...)                                                  \
	if (plist_bin_debug) {                                              \
		fprintf(stderr, "libplist[binparser] ERROR: " __VA_ARGS__); \
	}
#else
#define PLIST_BIN_ERR(...)
#endif
...
static plist_t parse_bin_node_at_index(struct bplist_data *bplist,
				       uint32_t node_index);
...
#ifdef _MSC_VER
		*(uint32_t *)buf =
			float_bswap32(get_unaligned_32((uint32_t *)*bnode));
#else
		*(uint32_t *)buf =
			float_bswap32(get_unaligned((uint32_t *)*bnode));
#endif
...
#ifdef _MSC_VER
		*(uint64_t *)buf =
			float_bswap64(get_unaligned_64((uint64_t *)*bnode));
#else
		*(uint64_t *)buf =
			float_bswap64(get_unaligned((uint64_t *)*bnode));
#endif
...
static char *plist_utf16be_to_utf8(uint16_t *unistr, long len, long *items_read,
				   long *items_written)
{
	if (!unistr || (len <= 0))
		return NULL;
...
static plist_t parse_unicode_node(const char **bnode, uint64_t size)
{
	...
	tmpstr = plist_utf16be_to_utf8((uint16_t *)(*bnode), size, &items_read,
				       &items_written);
	if (!tmpstr) {
		plist_free_data(data);
		return NULL;
...
static plist_t parse_dict_node(struct bplist_data *bplist, const char **bnode,
			       uint64_t size)
{
	uint64_t j;
	...
	for (j = 0; j < data->length; j++) {
		...
		if ((index1_ptr < bplist->data ||
		     index1_ptr + bplist->ref_size > bplist->offset_table) ||
		    (index2_ptr < bplist->data ||
		     index2_ptr + bplist->ref_size > bplist->offset_table)) {
			plist_free(node);
			PLIST_BIN_ERR("%s: dict entry %" PRIu64
		...
		if (index1 >= bplist->num_objects) {
			...
			PLIST_BIN_ERR(
				"%s: dict entry %" PRIu64
				": key index (%" PRIu64
				") must be smaller than the number of objects (%" PRIu64
				")\n",
...
static plist_t parse_array_node(struct bplist_data *bplist, const char **bnode,
				uint64_t size)
{
	uint64_t j;
	uint64_t str_j = 0;
	...
	for (j = 0; j < data->length; j++) {
		...
		if (index1_ptr < bplist->data ||
		    index1_ptr + bplist->ref_size > bplist->offset_table) {
			plist_free(node);
			PLIST_BIN_ERR("%s: array item %" PRIu64
				      " is outside of valid range\n",
				      __func__, j);
			return NULL;
		}
...
static plist_t parse_uid_node(const char **bnode, uint8_t size)
{
	...
	if (data->intval > UINT32_MAX) {
		PLIST_BIN_ERR("%s: value %" PRIu64
			      " too large for UID node (must be <= %u)\n",
...
static plist_t parse_bin_node(struct bplist_data *bplist, const char **object)
{
		...
		case BPLIST_DICT: {
			...
			if ((**object & BPLIST_MASK) != BPLIST_UINT) {
				PLIST_BIN_ERR(
					...
					__func__, type, **object & BPLIST_MASK,
					BPLIST_UINT);
				return NULL;
			}
			(*object)++;
			...
			if (*object + next_size > bplist->offset_table) {
				PLIST_BIN_ERR(
					"%s: size node data bytes for node type 0x%02x point outside of valid range\n",
					__func__, type);
				return NULL;
			}
	...
	case BPLIST_REAL:
		if (pobject + (uint64_t)(1 << size) > poffset_table) {
			PLIST_BIN_ERR(
				"%s: BPLIST_REAL data bytes point outside of valid range\n",
				__func__);
			return NULL;
		}
	...
	case BPLIST_DATE:
		if (3 != size) {
			PLIST_BIN_ERR(
				"%s: invalid data size for BPLIST_DATE node\n",
				__func__);
			return NULL;
		}
		if (pobject + (uint64_t)(1 << size) > poffset_table) {
			PLIST_BIN_ERR(
				"%s: BPLIST_DATE data bytes point outside of valid range\n",
				__func__);
			return NULL;
		}
	...
	case BPLIST_DATA:
		if (pobject + size < pobject ||
		    pobject + size > poffset_table) {
			PLIST_BIN_ERR(
	...
	case BPLIST_STRING:
		if (pobject + size < pobject ||
		    pobject + size > poffset_table) {
			PLIST_BIN_ERR(
				"%s: BPLIST_STRING data bytes point outside of valid range\n",
				__func__);
			return NULL;
		}
	...
	case BPLIST_UNICODE:
		...
		if (pobject + size * 2 < pobject ||
		    pobject + size * 2 > poffset_table) {
			PLIST_BIN_ERR(
	...
	case BPLIST_ARRAY:
		if (pobject + size < pobject ||
		    pobject + size > poffset_table) {
			PLIST_BIN_ERR(
				"%s: BPLIST_ARRAY data bytes point outside of valid range\n",
				__func__);
			return NULL;
		}
	...
	case BPLIST_UID:
		if (pobject + size + 1 > poffset_table) {
			PLIST_BIN_ERR(
				"%s: BPLIST_UID data bytes point outside of valid range\n",
				__func__);
			return NULL;
		}
	...
	case BPLIST_DICT:
		if (pobject + size < pobject ||
		    pobject + size > poffset_table) {
			PLIST_BIN_ERR(
				"%s: BPLIST_DICT data bytes point outside of valid range\n",
				__func__);
			return NULL;
		}
				       ...
				       uint32_t node_index)
{
	...
	if (node_index >= bplist->num_objects) {
		PLIST_BIN_ERR(
			"node index (%u) must be smaller than the number of objects (%" PRIu64
			")\n",
	...
	if (idx_ptr < bplist->offset_table ||
	    idx_ptr >= bplist->offset_table +
			       bplist->num_objects * bplist->offset_size) {
	...
	if ((uint32_t)ptr_array_size(bplist->used_indexes) <
	    bplist->level + 1) {
		while ((uint32_t)ptr_array_size(bplist->used_indexes) <
		       bplist->level + 1) {
			ptr_array_add(bplist->used_indexes,
				      (void *)(uintptr_t)node_index);
		}
	} else {
		ptr_array_set(bplist->used_indexes,
			      (void *)(uintptr_t)node_index, bplist->level);
	}

	/* recursion check */
	if (bplist->level > 0) {
		for (i = bplist->level - 1; i >= 0; i--) {
			uint32_t node_i = (uint32_t)(uintptr_t)ptr_array_index(
				bplist->used_indexes, i);
			uint32_t node_level =
				(uint32_t)(uintptr_t)ptr_array_index(
					bplist->used_indexes, bplist->level);
			if (node_i == node_level) {
				PLIST_BIN_ERR(
					"recursion detected in binary plist\n");
				return NULL;
			}
		}
	}
...
PLIST_API void plist_from_bin(const char *plist_bin, uint32_t length,
			      plist_t *plist)
{
	...
	if (!(length >= BPLIST_MAGIC_SIZE + BPLIST_VERSION_SIZE +
				sizeof(bplist_trailer_t))) {
		PLIST_BIN_ERR(
			"plist data is to small to hold a binary plist\n");
		return;
	}
	...
	if (memcmp(plist_bin + BPLIST_MAGIC_SIZE, BPLIST_VERSION,
		   BPLIST_VERSION_SIZE) != 0) {
	...
	offset_table =
		(char *)(plist_bin + be64toh(trailer->offset_table_offset));
	...
	if (ref_size == 0) {
		PLIST_BIN_ERR(
			"object reference size in trailer must be larger than 0\n");
		return;
	}

	if (root_object >= num_objects) {
		PLIST_BIN_ERR(
			"root object index (%" PRIu64
			") must be smaller than number of objects (%" PRIu64
			")\n",
	...
	if (!bplist.used_indexes) {
		PLIST_BIN_ERR(
			"failed to create array to hold used node indexes. Out of memory?\n");
		return;
	}
...
static unsigned int plist_data_hash(const void *key)
{
	...
	case PLIST_UID:
		buff = (char *)&data
			       ->intval; //works also for real as we use an union
		size = 8;
		break;
	case PLIST_KEY:
	case PLIST_STRING:
...
static void write_raw_data(bytearray_t *bplist, uint8_t mark, uint8_t *val,
			   uint64_t size)
{
...
static uint16_t *plist_utf8_to_utf16be(char *unistr, long size,
				       long *items_read, long *items_written)
{
	...
	if (!outbuf) {
		PLIST_BIN_ERR("%s: Could not allocate %" PRIu64 " bytes\n",
			      __func__,
			      (uint64_t)((size * 2) + 1) * sizeof(uint16_t));
		return NULL;
	}
	...
	while (i < size) {
		...
		if ((c0 >= 0xF0) && (i < size - 3) && (c1 >= 0x80) &&
			...
			i += 4;
		} else if ((c0 >= 0xE0) && (i < size - 2) && (c1 >= 0x80) &&
			   (c2 >= 0x80)) {
			// 3 byte sequence
			outbuf[p++] = be16toh(((c2 & 0x3F) + ((c1 & 3) << 6)) +
					      (((c1 >> 2) & 15) << 8) +
					      ((c0 & 15) << 12));
			i += 3;
		} else if ((c0 >= 0xC0) && (i < size - 1) && (c1 >= 0x80)) {
			// 2 byte sequence
			outbuf[p++] = be16toh(((c1 & 0x3F) + ((c0 & 3) << 6)) +
					      (((c0 >> 2) & 7) << 8));
			i += 2;
		} else if (c0 < 0x80) {
			// 1 byte sequence
			...
			// invalid character
			PLIST_BIN_ERR(
				"%s: invalid utf8 sequence in string at index %lu\n",
				__func__, i);
			break;
		}
	}
...
static void write_unicode(bytearray_t *bplist, char *val, uint64_t size)
{
	...
	unicodestr =
		plist_utf8_to_utf16be(val, size, &items_read, &items_written);
	write_raw_data(bplist, BPLIST_UNICODE, (uint8_t *)unicodestr,
		       items_written);
	free(unicodestr);
}

static void write_array(bytearray_t *bplist, node_t *node,
			hashtable_t *ref_table, uint8_t ref_size)
{
	node_t *cur = NULL;
	uint64_t i = 0;
	...
	for (i = 0, cur = node_first_child(node); cur && i < size;
	     cur = node_next_sibling(cur), i++) {
		uint64_t idx = *(uint64_t *)(hash_table_lookup(ref_table, cur));
		idx = be64toh(idx);
		byte_array_append(
			bplist, (uint8_t *)&idx + (sizeof(uint64_t) - ref_size),
			ref_size);
	}
}

static void write_dict(bytearray_t *bplist, node_t *node,
		       hashtable_t *ref_table, uint8_t ref_size)
{
	node_t *cur = NULL;
	uint64_t i = 0;
	...
	for (i = 0, cur = node_first_child(node); cur && i < size;
	     cur = node_next_sibling(node_next_sibling(cur)), i++) {
		uint64_t idx1 =
			*(uint64_t *)(hash_table_lookup(ref_table, cur));
		idx1 = be64toh(idx1);
		byte_array_append(bplist,
				  (uint8_t *)&idx1 +
					  (sizeof(uint64_t) - ref_size),
				  ref_size);
	}
	...
	for (i = 0, cur = node_first_child(node); cur && i < size;
	     cur = node_next_sibling(node_next_sibling(cur)), i++) {
		uint64_t idx2 =
			*(uint64_t *)(hash_table_lookup(ref_table, cur->next));
		idx2 = be64toh(idx2);
		byte_array_append(bplist,
				  (uint8_t *)&idx2 +
					  (sizeof(uint64_t) - ref_size),
...
PLIST_API void plist_to_bin(plist_t plist, char **plist_bin, uint32_t *length)
{
	...
	for (i = 0; i < num_objects; i++) {
		...
		case PLIST_STRING:
			if (is_ascii_string(data->strval, data->length)) {
				...
				write_unicode(bplist_buff, data->strval,
					      data->length);
			}
			break;
		case PLIST_DATA:
			...
			break;
		case PLIST_ARRAY:
			write_array(bplist_buff,
				    (node_t *)ptr_array_index(objects, i),
				    ref_table, ref_size);
			break;
		case PLIST_DICT:
			write_dict(bplist_buff,
				   (node_t *)ptr_array_index(objects, i),
				   ref_table, ref_size);
			break;
		case PLIST_DATE:
	...
	for (i = 0; i < num_objects; i++) {
		...
		byte_array_append(bplist_buff,
				  (uint8_t *)&offset +
					  (sizeof(uint64_t) - offset_size),
				  offset_size);
	}
	free(offsets);
