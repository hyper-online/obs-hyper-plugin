	     ...
	     size_t needle_len)
{
	...
	for (; haystack_ < last; ++haystack_) {
		if (__builtin_expect(hash == hay_hash, 0) &&
		    *haystack_ ==
			    *needle_ /* prevent calling memcmp, was a optimization from existing glibc */
...
PLIST_API void plist_from_memory(const char *plist_data, uint32_t length,
				 plist_t *plist)
{
	if (length < 8) {
		*plist = NULL;
...
{
	plist_data_t data =
		(plist_data_t)calloc(sizeof(struct plist_data_s), 1);
	return data;
}
...
static plist_t plist_copy_node(node_t *node)
{
	...
	case PLIST_ARRAY:
		if (data->hashtable) {
			ptrarray_t *pa = ptr_array_new(
				((ptrarray_t *)data->hashtable)->capacity);
			assert(pa);
	...
	case PLIST_DICT:
		if (data->hashtable) {
			hashtable_t *ht = hash_table_new(
				dict_key_hash, dict_key_compare, NULL);
			assert(ht);
	...
	for (ch = node_first_child(node); ch; ch = node_next_sibling(ch)) {
		...
		case PLIST_ARRAY:
			if (newdata->hashtable) {
				ptr_array_add((ptrarray_t *)newdata->hashtable,
					      newch);
			}
			break;
		case PLIST_DICT:
			if (newdata->hashtable && (node_index % 2 != 0)) {
				hash_table_insert(
					(hashtable_t *)newdata->hashtable,
					(node_prev_sibling((node_t *)newch))
						->data,
					newch);
			}
			break;
...
PLIST_API plist_t plist_array_get_item(plist_t node, uint32_t n)
{
	...
	if (node && PLIST_ARRAY == plist_get_node_type(node) && n < INT_MAX) {
		ptrarray_t *pa =
			(ptrarray_t *)((plist_data_t)((node_t *)node)->data)
				->hashtable;
		if (pa) {
...
{
	ptrarray_t *pa =
		(ptrarray_t *)((plist_data_t)((node_t *)node)->data)->hashtable;
	if (pa) {
		...
		if (((node_t *)node)->count > 100) {
			...
			for (current =
				     (plist_t)node_first_child((node_t *)node);
			     pa && current;
			     current = (plist_t)node_next_sibling(
				     (node_t *)current)) {
...
PLIST_API void plist_array_set_item(plist_t node, plist_t item, uint32_t n)
{
	if (node && PLIST_ARRAY == plist_get_node_type(node) && n < INT_MAX) {
		...
		if (old_item) {
			...
			ptrarray_t *pa =
				(ptrarray_t *)((plist_data_t)((node_t *)node)
						       ->data)
					->hashtable;
			if (pa) {
...
PLIST_API void plist_array_item_remove(plist_t node)
{
	...
	if (PLIST_ARRAY == plist_get_node_type(father)) {
		...
		if (n < 0)
			return;
		ptrarray_t *pa =
			(ptrarray_t *)((plist_data_t)((node_t *)father)->data)
				->hashtable;
		if (pa) {
			ptr_array_remove(pa, n);
		}
...
PLIST_API void plist_array_next_item(plist_t node, plist_array_iter iter,
				     plist_t *item)
{
...
PLIST_API void plist_dict_next_item(plist_t node, plist_dict_iter iter,
				    char **key, plist_t *val)
{
	node_t **iter_node = (node_t **)iter;
...
PLIST_API void plist_dict_get_item_key(plist_t node, char **key)
{
	...
	if (PLIST_DICT == plist_get_node_type(father)) {
		plist_get_key_val((plist_t)node_prev_sibling((node_t *)node),
				  key);
	}
}
	...
	if (node && PLIST_DICT == plist_get_node_type(node)) {
		...
		if (ht) {
			...
			for (current =
			     ...
			     current;
			     current = (plist_t)node_next_sibling(
				     node_next_sibling((node_t *)current))) {
				data = plist_get_data(current);
				assert(PLIST_KEY ==
				       plist_get_node_type(current));

				if (data && !strcmp(key, data->strval)) {
					ret = (plist_t)node_next_sibling(
						(node_t *)current);
					break;
				}
			}
...
PLIST_API void plist_dict_set_item(plist_t node, const char *key, plist_t item)
{
	if (node && PLIST_DICT == plist_get_node_type(node)) {
		...
		hashtable_t *ht =
			(hashtable_t *)((plist_data_t)((node_t *)node)->data)
				->hashtable;
		if (ht) {
			/* store pointer to item in hash table */
			hash_table_insert(
				ht, (plist_data_t)((node_t *)key_node)->data,
				item);
		} else {
			if (((node_t *)node)->count > 500) {
				/* make new hash table */
				ht = hash_table_new(dict_key_hash,
						    dict_key_compare, NULL);
				/* calculate the hashes for all entries we have so far */
				plist_t current = NULL;
				for (current = (plist_t)node_first_child(
					     (node_t *)node);
				     ht && current;
				     current = (plist_t)node_next_sibling(
					     node_next_sibling(
						     (node_t *)current))) {
					hash_table_insert(
						ht, ((node_t *)current)->data,
						node_next_sibling(
							(node_t *)current));
				}
				((plist_data_t)((node_t *)node)->data)
					->hashtable = ht;
			}
		}
	}
...
PLIST_API void plist_dict_insert_item(plist_t node, const char *key,
				      plist_t item)
{
...
PLIST_API void plist_dict_remove_item(plist_t node, const char *key)
{
	if (node && PLIST_DICT == plist_get_node_type(node)) {
		...
		if (old_item) {
			plist_t key_node =
				node_prev_sibling((node_t *)old_item);
			hashtable_t *ht =
				(hashtable_t *)((plist_data_t)((node_t *)node)
							->data)
					->hashtable;
			if (ht) {
				hash_table_remove(ht,
						  ((node_t *)key_node)->data);
			}
			plist_free(key_node);
			plist_free(old_item);
		}
...
{
	if (!target || !*target ||
	    (plist_get_node_type(*target) != PLIST_DICT) || !source ||
...
static void plist_get_type_and_value(plist_t node, plist_type *type,
	...
	*length = data->length;
	...
	case PLIST_DATA:
		*((uint8_t **)value) =
		...
		memcpy(*((uint8_t **)value), data->buff,
...
int plist_data_compare(const void *a, const void *b)
{
	...
	case PLIST_UID:
		if (val_a->length != val_b->length)
			return FALSE;
		return val_a->intval ==
		       val_b->intval; //it is an union so this is sufficient
...
static void plist_set_element_val(plist_t node, plist_type type,
				  const void *value, uint64_t length)
{
...
PLIST_API void plist_set_data_val(plist_t node, const char *val,
				  uint64_t length)
{
...
PLIST_API int plist_date_val_compare(plist_t datenode, int32_t cmpsec,
				     int32_t cmpusec)
{
...
PLIST_API int plist_string_val_compare_with_size(plist_t strnode,
						 const char *cmpval, size_t n)
{
	if (!PLIST_IS_STRING(strnode)) {
		return -1;
	}
...
PLIST_API int plist_data_val_compare(plist_t datanode, const uint8_t *cmpval,
				     size_t n)
{
...
PLIST_API int plist_data_val_contains(plist_t datanode, const uint8_t *cmpval,
				      size_t n)
{
	if (!PLIST_IS_DATA(datanode)) {
		return -1;
