...
public:
	...
	class Delegate {
	public:
		virtual void deviceManagerDidUpdateDeviceList(
			std::map<std::string, Device::shared_ptr> devices) = 0;
		virtual void
		deviceManagerDidAddDevice(Device::shared_ptr device) = 0;
		virtual void
		deviceManagerDidRemoveDevice(Device::shared_ptr device) = 0;
		virtual void
		deviceManagerDidChangeState(DeviceManager::State state) = 0;
		virtual ~Delegate(){};
	};
	...
	std::function<void(Device::shared_ptr device)>
		onDeviceManagerDidAddDeviceCallback;
	std::function<void(Device::shared_ptr device)>
	...
	std::shared_ptr<DeviceManager> getptr() { return shared_from_this(); }

	DeviceManager::DeviceMap getDevices() { return devices; }
...
private:
	...
	friend void pt_usbmuxd_cb(const usbmuxd_event_t *event,
				  void *user_data);
};
