...
std::vector<char> SimpleDataPacketProtocol::createPing()
{
	_PortalFrame frame;

	frame.version = htonl(1);
	frame.type = htonl(2001); // ping
	frame.tag = htonl(0);
	frame.payloadSize = htonl(0);

	char *ptr = (char *)&frame;

	std::vector<char> data;
	data.insert(data.end(), ptr, ptr + sizeof(_PortalFrame));
