...
class DeviceConnection : public Channel::Delegate,
...
public:
	...
	class Delegate {
	public:
		virtual void connectionDidChangeState(
			std::shared_ptr<DeviceConnection> deviceConnection,
			DeviceConnection::State state) = 0;
		virtual void connectionDidRecieveData(
			std::shared_ptr<DeviceConnection> deviceConnection,
			std::vector<char> data) = 0;
		virtual void connectionDidFail(
			std::shared_ptr<DeviceConnection> deviceConnection) = 0;
		virtual ~Delegate(){};
	};
	...
	void channelDidReceivePacket(std::vector<char> packet, int type,
				     int tag);
	void channelDidStop();
