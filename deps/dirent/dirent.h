...
static WIN32_FIND_DATAW *dirent_first(_WDIR *dirp)
{
	...
	dirp->handle = FindFirstFileExW(dirp->patt, FindExInfoStandard,
					&dirp->data, FindExSearchNameMatch,
					NULL, 0);
	if (dirp->handle == INVALID_HANDLE_VALUE)
		goto error;
...
static int readdir_r(DIR *dirp, struct dirent *entry, struct dirent **result)
{
	...
	size_t n;
	int error = wcstombs_s(&n, entry->d_name, PATH_MAX + 1,
	...
	if (error && datap->cAlternateFileName[0] != '\0') {
		error = wcstombs_s(&n, entry->d_name, PATH_MAX + 1,
