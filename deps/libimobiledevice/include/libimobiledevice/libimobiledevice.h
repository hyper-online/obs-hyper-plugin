...
typedef idevice_connection_private
	*idevice_connection_t; /**< The connection handle. */

/** Options for idevice_new_with_options() */
enum idevice_options {
	IDEVICE_LOOKUP_USBMUX =
		1 << 1, /**< include USBMUX devices during lookup */
	IDEVICE_LOOKUP_NETWORK =
		1 << 2, /**< include network devices during lookup */
	IDEVICE_LOOKUP_PREFER_NETWORK =
		1
		<< 3 /**< prefer network connection if device is available via USBMUX *and* network */
};
...
typedef void (*idevice_event_cb_t)(const idevice_event_t *event,
				   void *user_data);
 ...
 */
LIBIMOBILEDEVICE_API_MSC idevice_error_t
 ...
 */
LIBIMOBILEDEVICE_API_MSC idevice_error_t
idevice_get_device_list(char ***devices, int *count);
 ...
 */
LIBIMOBILEDEVICE_API_MSC idevice_error_t idevice_new(idevice_t *device,
						     const char *udid);
 ...
 * @return IDEVICE_E_SUCCESS if ok, otherwise an error code.
 */
LIBIMOBILEDEVICE_API_MSC idevice_error_t idevice_new_with_options(
 ...
 * @return IDEVICE_E_SUCCESS if ok, otherwise an error code.
 */
LIBIMOBILEDEVICE_API_MSC idevice_error_t idevice_connect(
 ...
 * @return IDEVICE_E_SUCCESS if ok, otherwise an error code.
 */
LIBIMOBILEDEVICE_API_MSC idevice_error_t idevice_connection_receive_timeout(
 ...
 *     enabled and does no further error checking on cleanup.
 */
LIBIMOBILEDEVICE_API_MSC idevice_error_t idevice_connection_disable_bypass_ssl(
 ...
 */
LIBIMOBILEDEVICE_API_MSC idevice_error_t idevice_get_handle(idevice_t device,
							    uint32_t *handle);

/**
 * Gets the unique id for the device.
 */
LIBIMOBILEDEVICE_API_MSC idevice_error_t idevice_get_udid(idevice_t device,
							  char **udid);
...
*/
LIBIMOBILEDEVICE_API_MSC idevice_error_t
idevice_set_tcp_endpoint(const char *host, uint16_t port);
...
*/
LIBIMOBILEDEVICE_API_MSC idevice_error_t
idevice_get_tcp_endpoint(char **host, uint16_t *port);
