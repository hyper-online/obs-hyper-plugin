...
int main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "-d") || !strcmp(argv[i], "--debug")) {
			...
			if (!argv[i] || ((strcmp(argv[i], "All") != 0) &&
					 (strcmp(argv[i], "WiFi") != 0) &&
					 (strcmp(argv[i], "GasGauge") != 0) &&
					 (strcmp(argv[i], "NAND") != 0) &&
					 (strcmp(argv[i], "HDMI") != 0))) {
				if (argv[i] == NULL) {
					cmd_arg = strdup("All");
					continue;
				}
	...
	if (LOCKDOWN_E_SUCCESS !=
	    (ret = lockdownd_client_new_with_handshake(device, &lockdown_client,
						       TOOL_NAME))) {
		idevice_free(device);
	...
	ret = lockdownd_start_service(lockdown_client,
	...
	if ((ret == LOCKDOWN_E_SUCCESS) && service && (service->port > 0)) {
		if (diagnostics_relay_client_new(device, service,
			...
			case CMD_IOREGISTRY_ENTRY:
				...
				break;
			case CMD_IOREGISTRY:
				if (diagnostics_relay_query_ioregistry_plane(
					    diagnostics_client,
					    cmd_arg == NULL ? "" : cmd_arg,
					    &node) ==
				    DIAGNOSTICS_RELAY_E_SUCCESS) {
			...
			default:
				if (diagnostics_relay_request_diagnostics(
					    diagnostics_client, cmd_arg,
					    &node) ==
				    DIAGNOSTICS_RELAY_E_SUCCESS) {
					if (node) {
						print_xml(node);
