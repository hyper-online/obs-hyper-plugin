...
enum cmd_mode { CMD_BACKUP, CMD_RESTORE, CMD_LEAVE };
...
static void sha1_of_data(const char *input, uint32_t size,
...
static int compare_hash(const unsigned char *hash1, const unsigned char *hash2,
			int hash_len)
{
	int i;
...
static void compute_datahash(const char *path, const char *destpath,
			     uint8_t greylist, const char *domain,
			     const char *appid, const char *version,
			     unsigned char *hash_out)
{
#ifdef HAVE_OPENSSL
	SHA_CTX sha1;
...
static char *mobilebackup_build_path(const char *backup_directory,
...
static void mobilebackup_write_status(const char *path, int status)
{
	...
	plist_dict_set_item(status_plist, "Backup Success",
...
static int mobilebackup_delete_backup_file_by_hash(const char *backup_directory,
						   const char *hash)
{
	int ret = 0;
...
static int mobilebackup_check_file_integrity(const char *backup_directory,
					     const char *hash, plist_t filedata)
{
	char *datapath;
	char *infopath;
	plist_t mdinfo = NULL;
	...
	if (data_hash && (data_hash_len == 20)) {
		compute_datahash(datapath, destpath, greylist, domain, NULL,
				 version, file_hash);
...
static void do_post_notification(const char *notification)
{
	...
	if (!client) {
		if (lockdownd_client_new_with_handshake(
			    device, &client, TOOL_NAME) != LOCKDOWN_E_SUCCESS) {
			return;
		}
	}
...
int main(int argc, char *argv[])
{
	...
	char *info_path =
		mobilebackup_build_path(backup_directory, "Info", ".plist");
	if (cmd == CMD_RESTORE) {
	...
	ldret = lockdownd_start_service(client, MOBILEBACKUP_SERVICE_NAME,
					&service);
	if ((ldret == LOCKDOWN_E_SUCCESS) && service && service->port) {
		...
		if (stat(info_path, &st) == 0) {
			...
			if (info_plist && (cmd == CMD_BACKUP)) {
				if (mobilebackup_info_is_current_device(
					    info_plist)) {
		...
		char *manifest_path = mobilebackup_build_path(
			backup_directory, "Manifest", ".plist");
		...
		case CMD_BACKUP:
			...
			err = mobilebackup_request_backup(
				mobilebackup, manifest_plist, "/", "1.6");
			...
			do {
				...
				if ((hunk_index == 0) && (file_index == 0)) {
					node = plist_dict_get_item(
						node_tmp, "BackupTotalSizeKey");
					if (node) {
						plist_get_uint_val(
							node,
							&backup_total_size);
						format_size = string_format_size(
							backup_total_size);
				...
				node = plist_dict_get_item(node_tmp,
							   "DLFileStatusKey");
				...
				if ((hunk_index == 0) && (!is_manifest)) {
					/* get source filename */
					node = plist_dict_get_item(
						node_tmp, "DLFileSource");
					plist_get_string_val(node,
							     &filename_source);
			...
			files_out:
				if (quit_flag > 0) {
					...
					mobilebackup_send_error(
						mobilebackup,
						"Cancelling DLSendFile");

					/* remove any atomic Manifest.plist.tmp */
					if (manifest_path)
						free(manifest_path);

					manifest_path = mobilebackup_build_path(
			...
			if (!quit_flag &&
			    !plist_strcmp(node, "DLMessageProcessMessage")) {
				node_tmp = plist_array_get_item(message, 1);
				node = plist_dict_get_item(
					node_tmp, "BackupMessageTypeKey");
				...
				if (node &&
					...
					if (node) {
						...
						while ((node_tmp =
							...
							if (mobilebackup_delete_backup_file_by_hash(
								    backup_directory,
								    file_path)) {
								printf("DONE\n");
							} else
					...
					node_tmp = plist_array_get_item(message,
									1);
					manifest_plist = plist_dict_get_item(
						node_tmp, "BackupManifestKey");
					if (manifest_plist) {
						remove(manifest_path);
		...
		case CMD_RESTORE:
			...
			if (!node ||
			    (plist_get_node_type(node) != PLIST_DATA)) {
			...
			if (bin) {
				...
				node = plist_dict_get_item(manifest_plist,
							   "AuthVersion");
				plist_get_string_val(node, &auth_ver);
				if (auth_ver &&
				    (strcmp(auth_ver, "2.0") == 0)) {
					node = plist_dict_get_item(
						manifest_plist,
						"AuthSignature");
					if (node &&
					    (plist_get_node_type(node) ==
					     PLIST_DATA)) {
						plist_get_data_val(
							node,
							(char **)&auth_sig,
					...
					if (auth_sig && (auth_sig_len == 20)) {
						...
						sha1_of_data(bin, binsize,
							     data_sha1);
						if (compare_hash(auth_sig,
								 data_sha1,
								 20)) {
				...
				plist_from_bin(bin, (uint32_t)binsize,
					       &backup_data);
				free(bin);
			}
			if (!backup_data) {
			...
			plist_t files =
				plist_dict_get_item(backup_data, "Files");
			if (files &&
			    (plist_get_node_type(files) == PLIST_DICT)) {
				...
				if (iter) {
					...
					int file_ok = 0;
					int total_files =
						plist_dict_get_size(files);
					int cur_file = 1;
					node = NULL;
					plist_dict_next_item(files, iter, &hash,
							     &node);
					while (node) {
						printf("Verifying file %d/%d (%d%%) \r",
						       cur_file, total_files,
						       (cur_file * 100 /
							total_files));
						cur_file++;
			...
			int restore_flags = MB_RESTORE_NOTIFY_SPRINGBOARD |
					    MB_RESTORE_PRESERVE_SETTINGS |
					    MB_RESTORE_PRESERVE_CAMERA_ROLL;
			err = mobilebackup_request_restore(mobilebackup,
							   manifest_plist,
							   restore_flags,
							   "1.6");
			if (err != MOBILEBACKUP_E_SUCCESS) {
			...
			if (files &&
				...
				if (iter) {
					...
					while (node) {
						...
						tmp_node = plist_dict_get_item(
							file_info, "Metadata");
						plist_get_data_val(tmp_node,
								   &buffer,
								   &length);
						tmp_node = NULL;
						plist_from_bin(buffer, length,
							       &tmp_node);
						file_path_node =
							plist_dict_get_item(
								tmp_node,
						...
						do {
							...
							if (file_status ==
								...
								err = mobilebackup_receive_restore_file_received(
									mobilebackup,
									NULL);
								if (err !=
								    MOBILEBACKUP_E_SUCCESS) {
			...
			plist_t applications = plist_dict_get_item(
				backup_data, "Applications");
			if (applications &&
			    (plist_get_node_type(applications) == PLIST_DICT) &&
			    restore_ok) {
				plist_dict_iter iter = NULL;
				...
				if (iter) {
					...
					char *hash = NULL;
					int total_files = plist_dict_get_size(
						applications);
					int cur_file = 1;
					plist_t tmp_node = NULL;
					plist_t dict = NULL;
					plist_t array = NULL;
					node = NULL;
					plist_dict_next_item(applications, iter,
							     &hash, &node);
					while (node) {
						printf("Restoring Application %s %d/%d (%d%%)...",
						       hash, cur_file,
						       total_files,
						       (cur_file * 100 /
							total_files));
						...
						if (restore_ok) {
							err = mobilebackup_receive_restore_application_received(
								mobilebackup,
								NULL);
							if (err !=
							    MOBILEBACKUP_E_SUCCESS) {
			...
			if (restore_ok) {
				err = mobilebackup_send_restore_complete(
					mobilebackup);
				if (err != MOBILEBACKUP_E_SUCCESS) {
