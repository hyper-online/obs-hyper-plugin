...
enum { OP_INSTALL, OP_LIST, OP_COPY, OP_REMOVE, OP_DUMP, NUM_OPS };
...
static size_t asn1_item_get_size(unsigned char *p)
{
	...
	if (bsize & 0x80) {
		...
		default:
			fprintf(stderr,
				"ERROR: Invalid or unimplemented byte size %d\n",
				bsize & 0xF);
			break;
		}
	} else {
...
static int profile_read_from_file(const char *path,
...
int main(int argc, char *argv[])
{
	...
	if (op == OP_DUMP) {
		...
		if (profile_read_from_file(param, &profile_data,
					   &profile_size) != 0) {
			return -1;
		}
		plist_t pdata =
			plist_new_data((char *)profile_data, profile_size);
	...
	if (pver_s) {
		sscanf(pver_s, "%d.%d.%d", &product_version_major,
		       &product_version_minor, &product_version_patch);
		free(pver_s);
	}
	if (product_version_major == 0) {
		fprintf(stderr,
			"ERROR: Could not determine the device's ProductVersion\n");
		lockdownd_client_free(client);
		idevice_free(device);
		return -1;
	}
	int product_version = ((product_version_major & 0xFF) << 16) |
			      ((product_version_minor & 0xFF) << 8) |
			      (product_version_patch & 0xFF);

	if (LOCKDOWN_E_SUCCESS !=
	    lockdownd_start_service(client, "com.apple.misagent", &service)) {
		fprintf(stderr,
	...
	case OP_INSTALL: {
		...
		plist_t pdata =
			plist_new_data((const char *)profile_data, psize);
		free(profile_data);
	...
	case OP_COPY: {
		...
		if (merr == MISAGENT_E_SUCCESS) {
			...
			for (j = 0; !found_match && j < num_profiles; j++) {
				...
				char *p_uuid = NULL;
				plist_t profile =
					plist_array_get_item(profiles, j);
				plist_t pl =
					profile_get_embedded_plist(profile);
				if (pl &&
				    (plist_get_node_type(pl) == PLIST_DICT)) {
					plist_t node;
			...
			if (param2 && !found_match) {
				fprintf(stderr,
					"Profile '%s' was not found on the device.\n",
					param);
				res = -1;
			}
		} else {
	...
	case OP_REMOVE:
		if (param) {
			...
			if (merr == MISAGENT_E_SUCCESS) {
				...
				for (j = 0; j < plist_array_get_size(profiles);
				     j++) {
					char *p_name = NULL;
					char *p_uuid = NULL;
					plist_t profile = plist_array_get_item(
						profiles, j);
					plist_t pl = profile_get_embedded_plist(
						profile);
					if (pl && (plist_get_node_type(pl) ==
						   PLIST_DICT)) {
						plist_t node;
