...
static void *thread_device_to_client(void *data)
{
	...
	while (!quit_flag && !socket_info->stop_dtoc &&
	       socket_info->client_fd > 0) {
		debug("%s: receiving data from device...\n", __func__);

		res = debugserver_client_receive_with_timeout(
			socket_info->debugserver_client, buffer, sizeof(buffer),
			(uint32_t *)&recv_len, 5000);
		...
		if (recv_len <= 0) {
			...
			sent = socket_send(socket_info->client_fd, buffer,
					   recv_len);
			if (sent < recv_len) {
				if (sent <= 0) {
...
static void *thread_client_to_device(void *data)
{
	...
	while (!quit_flag && !socket_info->stop_ctod &&
	       socket_info->client_fd > 0) {
		debug("%s: receiving data from client...\n", __func__);

		/* attempt to read incoming data from client */
		recv_len = socket_receive_timeout(socket_info->client_fd,
		...
		if (recv_len <= 0) {
			...
			res = debugserver_client_send(
				socket_info->debugserver_client, buffer,
				recv_len, (uint32_t *)&sent);
...
static void *connection_handler(void *data)
{
	...
	derr = debugserver_client_start_service(
		socket_info->device, &socket_info->debugserver_client,
		TOOL_NAME);
...
int main(int argc, char *argv[])
{
	...
	while (!quit_flag) {
		...
		thread_info_t *el =
			(thread_info_t *)malloc(sizeof(thread_info_t));
		if (!el) {
		...
		socket_info_t *sinfo =
			(socket_info_t *)malloc(sizeof(socket_info_t));
		if (!sinfo) {
			fprintf(stderr, "Out of memory\n");
			exit(EXIT_FAILURE);
		}
		...
		if (thread_new(&(el->th), connection_handler, (void *)sinfo) !=
		    0) {
			fprintf(stderr,
