...
static int extract_raw_crash_report(const char *filename)
{
	...
	if (plist_read_from_filename(&report, filename)) {
		plist_t description_node =
			plist_dict_get_item(report, "description");
		if (description_node &&
		    plist_get_node_type(description_node) == PLIST_STRING) {
			plist_get_string_val(description_node, &raw);

			if (raw != NULL) {
				/* write file */
				buffer_write_to_filename(raw_filename, raw,
							 strlen(raw));
				free(raw);
				res = 1;
			}
...
static int
afc_client_copy_and_remove_crash_reports(afc_client_t afc,
					 ...
					 const char *host_directory)
{
	...
	for (k = 0; list[k]; k++) {
		...
		strcpy(((char *)source_filename) + device_directory_length,
		       list[k]);
		...
		if (p != NULL && !strncmp(p, ".synced", 7)) {
			...
			strncpy(((char *)target_filename) +
					host_directory_length,
				list[k], newlen);
			target_filename[host_directory_length + newlen] = '\0';
		} else {
			strcpy(((char *)target_filename) +
				       host_directory_length,
			       list[k]);
		}
		...
		if (S_ISDIR(stbuf.st_mode)) {
...
#endif
			res = afc_client_copy_and_remove_crash_reports(
				afc, source_filename, target_filename);
			...
			/* copy file to host */
			afc_error = afc_file_open(afc, source_filename,
			...
			afc_error = afc_file_read(afc, handle, (char *)data,
						  0x1000, &bytes_read);
			while (afc_error == AFC_E_SUCCESS && bytes_read > 0) {
				fwrite(data, 1, bytes_read, output);
				bytes_total += bytes_read;
				afc_error = afc_file_read(afc, handle,
							  (char *)data, 0x1000,
							  &bytes_read);
			}
			afc_file_close(afc, handle);
...
int main(int argc, char *argv[])
{
	...
	lockdownd_error = lockdownd_client_new_with_handshake(
		device, &lockdownd, TOOL_NAME);
	...
	lockdownd_error = lockdownd_start_service(
		lockdownd, "com.apple.crashreportmover", &service);
	if (lockdownd_error != LOCKDOWN_E_SUCCESS) {
	...
	service_error_t service_error =
		service_client_new(device, service, &svcmove);
	...
	while ((strncmp(ping, "ping", 4) != 0) && (attempts < 10)) {
		uint32_t bytes = 0;
		service_error = service_receive_with_timeout(svcmove, ping, 4,
							     &bytes, 2000);
		if (service_error == SERVICE_E_SUCCESS ||
		    service_error == SERVICE_E_TIMEOUT) {
			attempts++;
			continue;
		} else {
			fprintf(stderr,
				"ERROR: Crash logs could not be moved. Connection interrupted (%d).\n",
				service_error);
			break;
		}
	}
	...
	lockdownd_error = lockdownd_start_service(
		lockdownd, "com.apple.crashreportcopymobile", &service);
	if (lockdownd_error != LOCKDOWN_E_SUCCESS) {
		lockdownd_client_free(lockdownd);
		idevice_free(device);
		return -1;
	...
	if (afc_client_copy_and_remove_crash_reports(afc, ".",
						     target_directory) < 0) {
		fprintf(stderr,
