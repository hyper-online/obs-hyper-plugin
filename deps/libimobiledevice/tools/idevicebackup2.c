...
void usleep(DWORD waitTime)
{
	...
	} while ((now.QuadPart - start.QuadPart) / (float)(perfCnt.QuadPart) *
			 1000 * 1000 <
		 waitTime);
}
#endif
						   ...
						   afc_client_t afc)
{
	...
	/* get iTunes settings */
	lockdownd_get_value(lockdown, "com.apple.iTunes", NULL,
			    &itunes_settings);

	/* get minimum iTunes version */
	lockdownd_get_value(lockdown, "com.apple.mobile.iTunes",
			    "MinITunesVersion", &min_itunes_version);
	...
	if (instproxy_client_start_service(device, &ip, TOOL_NAME) ==
		...
		instproxy_client_options_add(client_opts, "ApplicationType",
					     "User", NULL);
		instproxy_client_options_set_return_attributes(
			client_opts, "CFBundleIdentifier", "ApplicationSINF",
			"iTunesMetadata", NULL);
		...
		if (apps && (plist_get_node_type(apps) == PLIST_ARRAY)) {
			...
			for (i = 0; i < app_count; i++) {
				plist_t app_entry =
					plist_array_get_item(apps, i);
				plist_t bundle_id = plist_dict_get_item(
					app_entry, "CFBundleIdentifier");
				if (bundle_id) {
					char *bundle_id_str = NULL;
					plist_array_append_item(
						installed_apps,
						plist_copy(bundle_id));

					plist_get_string_val(bundle_id,
							     &bundle_id_str);
					plist_t sinf = plist_dict_get_item(
						app_entry, "ApplicationSINF");
					plist_t meta = plist_dict_get_item(
						app_entry, "iTunesMetadata");
					if (sinf && meta) {
						plist_t adict =
							plist_new_dict();
						plist_dict_set_item(
						...
						plist_dict_set_item(
							adict, "iTunesMetadata",
							plist_copy(meta));
						plist_dict_set_item(
							app_dict, bundle_id_str,
							adict);
					}
					free(bundle_id_str);
				}
	...
	value_node = plist_dict_get_item(
		root_node, "InternationalMobileEquipmentIdentity");
	if (value_node)
	...
	value_node =
		plist_dict_get_item(root_node, "MobileEquipmentIdentifier");
	if (value_node)
	...
	if (value_node && (plist_get_node_type(value_node) == PLIST_STRING)) {
		plist_dict_set_item(ret, "Phone Number",
	...
	const char *itunesfiles[] = {
		"ApertureAlbumPrefs",   "IC-Info.sidb",
		"IC-Info.sidv",         "PhotosFolderAlbums",
		"PhotosFolderName",     "PhotosFolderPrefs",
		"VoiceMemos.plist",     "iPhotoAlbumPrefs",
		"iTunesApplicationIDs", "iTunesPrefs",
		"iTunesPrefs.plist",    NULL};
	int i = 0;
	for (i = 0; itunesfiles[i]; i++) {
		...
		mobilebackup_afc_get_file_contents(afc, fname, &data_buf,
						   &data_size);
		free(fname);
		if (data_buf) {
			plist_dict_set_item(files, itunesfiles[i],
					    plist_new_data(data_buf,
							   data_size));
			free(data_buf);
		}
	}
	...
	if (min_itunes_version) {
		plist_dict_set_item(ret, "iTunes Version",
				    plist_copy(min_itunes_version));
	} else {
		plist_dict_set_item(ret, "iTunes Version",
...
static int write_restore_applications(plist_t info_plist, afc_client_t afc)
{
	...
	plist_t applications_plist =
		plist_dict_get_item(info_plist, "Applications");
	...
	afc_err = afc_file_write(afc, restore_applications_file,
				 applications_plist_xml,
				...
				plist_t *errplist)
{
	...
	err = mobilebackup2_send_raw(mobilebackup2, (const char *)&nlen,
				     sizeof(nlen), &bytes);
	...
	do {
		...
		err = mobilebackup2_send_raw(mobilebackup2, (const char *)buf,
					     5, &bytes);
		if (err != MOBILEBACKUP2_E_SUCCESS) {
			goto leave_proto_err;
		}
				  ...
				  plist_t message, const char *backup_dir)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		if (mb2_handle_send_file(mobilebackup2, backup_dir, str,
					 &errplist) < 0) {
			free(str);
	...
	if (!errplist) {
		...
		mobilebackup2_send_status_response(mobilebackup2, 0, NULL,
						   emptydict);
		plist_free(emptydict);
	} else {
		mobilebackup2_send_status_response(mobilebackup2, -13,
						   "Multi status", errplist);
		plist_free(errplist);
	}
}
				...
				char **filename)
{
	...
	do {
		nlen = 0;
		rlen = 0;
		mobilebackup2_receive_raw(mobilebackup2, (char *)&nlen, 4,
					  &rlen);
		nlen = be32toh(nlen);
		...
		*filename = (char *)malloc(nlen + 1);
		...
		rlen = 0;
		mobilebackup2_receive_raw(mobilebackup2, *filename, nlen,
					  &rlen);
		if (rlen != nlen) {
	...
	do {
		...
		while (f && (code == CODE_FILE_DATA)) {
			...
			while (bdone < blocksize) {
				...
				mobilebackup2_receive_raw(mobilebackup2, buf,
							  rlen, &r);
				if ((int)r <= 0) {
					break;
				}
			...
			if (backup_total_size > 0) {
				print_progress(backup_real_size,
					       backup_total_size);
			}
			if (quit_flag)
				break;
			nlen = 0;
			mobilebackup2_receive_raw(mobilebackup2, (char *)&nlen,
						  4, &r);
			nlen = be32toh(nlen);
			if (nlen > 0) {
				last_code = code;
				mobilebackup2_receive_raw(mobilebackup2, &code,
							  1, &r);
			} else {
				break;
			}
		...
		if (code == CODE_ERROR_REMOTE) {
			...
			mobilebackup2_receive_raw(mobilebackup2, msg, nlen - 1,
						  &r);
			msg[r] = 0;
	...
	if (cur_dir) {
		...
		while ((ep = readdir(cur_dir))) {
			...
			if (fpath) {
				...
				plist_dict_set_item(
					fdict, "DLFileModificationDate",
					plist_new_date(st.st_mtime - MAC_EPOCH,
						       0));
				      ...
				      plist_t message, const char *backup_dir)
{
	...
	free(newpath);
	mobilebackup2_error_t err = mobilebackup2_send_status_response(
		mobilebackup2, errcode, errdesc, NULL);
	if (err != MOBILEBACKUP2_E_SUCCESS) {
		printf("Could not send status response, error %d\n", err);
	}
}
...
static void mb2_copy_directory_by_path(const char *src, const char *dst)
{
	...
	if (cur_dir) {
		...
		while ((ep = readdir(cur_dir))) {
			...
			char *srcpath =
				string_build_path(src, ep->d_name, NULL);
			char *dstpath =
				string_build_path(dst, ep->d_name, NULL);
			if (srcpath && dstpath) {
				/* copy file */
...
int main(int argc, char *argv[])
{
	...
	if (cmd == CMD_CHANGEPW || cmd == CMD_CLOUD) {
		backup_directory =
			(char *)".this_folder_is_not_present_on_purpose";
	} else {
	...
	if (cmd == CMD_CHANGEPW) {
		...
		info_path = string_build_path(backup_directory, source_udid,
					      "Info.plist", NULL);
		if (cmd == CMD_RESTORE || cmd == CMD_UNBACK) {
			...
			char *manifest_path =
				string_build_path(backup_directory, source_udid,
						  "Manifest.plist", NULL);
			if (stat(manifest_path, &st) != 0) {
				free(info_path);
			}
			plist_t manifest_plist = NULL;
			plist_read_from_filename(&manifest_plist,
						 manifest_path);
			if (!manifest_plist) {
				idevice_free(device);
				free(info_path);
			...
			node_tmp = plist_dict_get_item(manifest_plist,
						       "IsEncrypted");
			if (node_tmp &&
			    (plist_get_node_type(node_tmp) == PLIST_BOOLEAN)) {
	...
	if (cmd != CMD_CLOUD && is_encrypted) {
		...
		if (backup_password == NULL) {
			if (interactive_mode) {
				backup_password = ask_for_password(
					"Enter backup password", 0);
			}
			if (!backup_password ||
			    (strlen(backup_password) == 0)) {
				if (backup_password) {
					free(backup_password);
				}
				idevice_free(device);
	...
	if (product_version) {
		int vers[3] = {0, 0, 0};
		if (sscanf(product_version, "%d.%d.%d", &vers[0], &vers[1],
			   &vers[2]) >= 2) {
			device_version =
	...
	if (cmd == CMD_BACKUP || cmd == CMD_RESTORE) {
		...
		ldret = lockdownd_start_service(lockdown, AFC_SERVICE_NAME,
						&service);
	...
	if ((ldret == LOCKDOWN_E_SUCCESS) && service && service->port) {
		...
		err = mobilebackup2_version_exchange(
			mobilebackup2, local_versions, 2, &remote_version);
		if (err != MOBILEBACKUP2_E_SUCCESS) {
		...
		if (cmd == CMD_BACKUP || cmd == CMD_RESTORE) {
			...
			afc_file_open(afc, "/com.apple.itunes.lock_sync",
				      AFC_FOPEN_RW, &lockfile);
		}
		if (lockfile) {
			afc_error_t aerr;
			do_post_notification(device, NP_SYNC_LOCK_REQUEST);
			for (i = 0; i < LOCK_ATTEMPTS; i++) {
				aerr = afc_file_lock(afc, lockfile,
						     AFC_LOCK_EX);
				if (aerr == AFC_E_SUCCESS) {
					do_post_notification(device,
							     NP_SYNC_DID_START);
					break;
	...
	checkpoint:
		...
		case CMD_CLOUD:
			opts = plist_new_dict();
			plist_dict_set_item(
				opts, "CloudBackupState",
				plist_new_bool(cmd_flags & CMD_FLAG_CLOUD_ENABLE
						       ? 1
						       : 0));
			err = mobilebackup2_send_request(mobilebackup2,
							 "EnableCloudBackup",
							 udid, source_udid,
							 opts);
			plist_free(opts);
		...
		case CMD_BACKUP:
			...
			char *devbackupdir = string_build_path(
				backup_directory, source_udid, NULL);
			...
			if (strcmp(source_udid, udid) != 0) {
				...
				devbackupdir = string_build_path(
					backup_directory, udid, NULL);
				__mkdir(devbackupdir, 0755);
				free(devbackupdir);

				// use Info.plist path in target backup folder */
				free(info_path);
				info_path = string_build_path(backup_directory,
							      udid,
							      "Info.plist",
							      NULL);
			}
			...
			if (!info_plist) {
				fprintf(stderr,
					"Failed to generate Info.plist - aborting\n");
				cmd = CMD_LEAVE;
			}
			remove_file(info_path);
			plist_write_to_filename(info_plist, info_path,
						PLIST_FORMAT_XML);
			free(info_path);
			...
			if (cmd_flags & CMD_FLAG_FORCE_FULL_BACKUP) {
				...
				opts = plist_new_dict();
				plist_dict_set_item(opts, "ForceFullBackup",
						    plist_new_bool(1));
			}
		...
		case CMD_RESTORE:
			...
			if (!mb2_status_check_snapshot_state(backup_directory,
							     source_udid,
							     "finished")) {
			...
			if ((cmd_flags & CMD_FLAG_RESTORE_COPY_BACKUP) == 0)
				plist_dict_set_item(opts,
						    "RestoreDontCopyBackup",
						    plist_new_bool(1));
			PRINT_VERBOSE(
				1, "Don't copy backup: %s\n",
				((cmd_flags & CMD_FLAG_RESTORE_COPY_BACKUP) == 0
					 ? "Yes"
					 : "No"));
			plist_dict_set_item(
				...
				plist_new_bool((cmd_flags &
						CMD_FLAG_RESTORE_SETTINGS) ==
					       0));
			PRINT_VERBOSE(
				1, "Preserve settings of device: %s\n",
			...
			if (cmd_flags & CMD_FLAG_RESTORE_REMOVE_ITEMS)
				plist_dict_set_item(opts,
						    ...
						    plist_new_bool(1));
			PRINT_VERBOSE(
				1, "Remove items that are not restored: %s\n",
				((cmd_flags & CMD_FLAG_RESTORE_REMOVE_ITEMS)
					 ? "Yes"
					 : "No"));
			if (backup_password != NULL) {
			...
			if (cmd_flags & CMD_FLAG_RESTORE_SKIP_APPS) {
				...
				if (write_restore_applications(info_plist,
							       afc) < 0) {
					cmd = CMD_LEAVE;
					break;
				} else {
		...
		case CMD_INFO:
			PRINT_VERBOSE(
				1, "Requesting backup info from device...\n");
			err = mobilebackup2_send_request(
				mobilebackup2, "Info", udid, source_udid, NULL);
			if (err != MOBILEBACKUP2_E_SUCCESS) {
		...
		case CMD_LIST:
			PRINT_VERBOSE(
				1, "Requesting backup list from device...\n");
			err = mobilebackup2_send_request(
				mobilebackup2, "List", udid, source_udid, NULL);
			if (err != MOBILEBACKUP2_E_SUCCESS) {
				...
				if (device_version >=
					...
					if (diagnostics_relay_client_start_service(
						...
						plist_array_append_item(
							...
							plist_new_string(
								"PasswordConfigured"));
						if (diagnostics_relay_query_mobilegestalt(
							    diag, keys,
							    &dict) ==
						    DIAGNOSTICS_RELAY_E_SUCCESS) {
							plist_t node = plist_access_path(
								dict, 2,
								"MobileGestalt",
								"PasswordConfigured");
							plist_get_bool_val(
		...
		if (cmd != CMD_LEAVE) {
			...
			do {
				free(dlmsg);
				dlmsg = NULL;
				mberr = mobilebackup2_receive_message(
					mobilebackup2, &message, &dlmsg);
				if (mberr == MOBILEBACKUP2_E_RECEIVE_TIMEOUT) {
					PRINT_VERBOSE(
						2,
						"Device is not ready yet, retrying...\n");
					goto files_out;
				} else if (mberr != MOBILEBACKUP2_E_SUCCESS) {
					PRINT_VERBOSE(
						0,
						"ERROR: Could not receive from mobilebackup2 (%d)\n",
						mberr);
					quit_flag++;
					goto files_out;
				}
				...
				if (!strcmp(dlmsg, "DLMessageDownloadFiles")) {
					...
					mb2_set_overall_progress_from_message(
						message, dlmsg);
					mb2_handle_send_files(mobilebackup2,
							      message,
							      backup_directory);
				} else if (!strcmp(dlmsg,
						   "DLMessageUploadFiles")) {
					/* device wants to send files to the computer */
					mb2_set_overall_progress_from_message(
						message, dlmsg);
					file_count += mb2_handle_receive_files(
						mobilebackup2, message,
						backup_directory);
				} else if (!strcmp(dlmsg,
					...
					if (res == 0) {
						freespace =
							(uint64_t)fs.f_bavail *
							(uint64_t)fs.f_bsize;
					}
#endif
					plist_t freespace_item =
						plist_new_uint(freespace);
					mobilebackup2_send_status_response(
						mobilebackup2, res, NULL,
						freespace_item);
					...
					err = mobilebackup2_send_status_response(
						mobilebackup2, -1,
					...
					mb2_set_overall_progress_from_message(
						message, dlmsg);
					plist_t moves = plist_array_get_item(
						message, 1);
					uint32_t cnt =
						plist_dict_get_size(moves);
						...
						do {
							...
							if (key &&
								...
								plist_get_string_val(
									...
									&str);
								if (str) {
									char *newpath = string_build_path(
										backup_directory,
										str,
										NULL);
									free(str);
									char *oldpath = string_build_path(
										backup_directory,
										key,
										NULL);
					...
					mb2_set_overall_progress_from_message(
						message, dlmsg);
					plist_t removes = plist_array_get_item(
						message, 1);
					uint32_t cnt =
						plist_array_get_size(removes);
					...
					if (err != MOBILEBACKUP2_E_SUCCESS) {
						printf("Could not send status response, error %d\n",
						   ...
						   "DLMessageCopyItem")) {
					plist_t srcpath = plist_array_get_item(
						message, 1);
					plist_t dstpath = plist_array_get_item(
						message, 2);
					errcode = 0;
					errdesc = NULL;
					if ((plist_get_node_type(srcpath) ==
					     PLIST_STRING) &&
					    (plist_get_node_type(dstpath) ==
					     PLIST_STRING)) {
						char *src = NULL;
						char *dst = NULL;
						plist_get_string_val(srcpath,
								     &src);
						plist_get_string_val(dstpath,
								     &dst);
						if (src && dst) {
							char *oldpath = string_build_path(
								backup_directory,
					...
					node_tmp = plist_array_get_item(message,
									1);
					if (plist_get_node_type(node_tmp) !=
					    PLIST_DICT) {
						printf("Unknown message received!\n");
					}
					plist_t nn;
					int error_code = -1;
					nn = plist_dict_get_item(node_tmp,
								 "ErrorCode");
					if (nn && (plist_get_node_type(nn) ==
						   PLIST_UINT)) {
						uint64_t ec = 0;
					...
					nn = plist_dict_get_item(
						node_tmp, "ErrorDescription");
					char *str = NULL;
					if (nn && (plist_get_node_type(nn) ==
						   PLIST_STRING)) {
				...
				if ((overall_progress > 0) &&
					...
					print_progress_real(overall_progress,
							    0);
			...
			case CMD_BACKUP:
				PRINT_VERBOSE(
					1, "Received %d files from device.\n",
					file_count);
				if (operation_ok &&
