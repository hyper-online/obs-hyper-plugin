...
enum cmd_mode { CMD_NONE = 0, CMD_RUN };
...
instproxy_client_get_object_by_key_from_info_directionary_for_bundle_identifier(
	instproxy_client_t client, const char *appid, const char *key,
	plist_t *node)
{
	if (!client || !appid || !key)
...
debugserver_client_handle_response(debugserver_client_t client, char **response,
				   int send_reply)
{
	...
	if (r[0] == 'O') {
		...
		*response = NULL;
		...
		dres = debugserver_client_send_command(client, command,
						       response, NULL);
...
int main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "-d") || !strcmp(argv[i], "--debug")) {
			...
			if (!newlist)
				newlist = malloc((environment_count + 1) *
						 sizeof(char *));
			else
				newlist = realloc(environment,
						  (environment_count + 1) *
							  sizeof(char *));
	...
	default:
		...
		if (instproxy_client_start_service(device, &instproxy_client,
						   TOOL_NAME) !=
		    INSTPROXY_E_SUCCESS) {
			fprintf(stderr,
		...
		instproxy_client_get_object_by_key_from_info_directionary_for_bundle_identifier(
			instproxy_client, bundle_identifier, "Container",
			&container);
		instproxy_client_get_path_for_bundle_identifier(
			instproxy_client, bundle_identifier, &path);
		instproxy_client_free(instproxy_client);
		instproxy_client = NULL;

		if (container &&
		    (plist_get_node_type(container) == PLIST_STRING)) {
		...
		if (debugserver_client_start_service(
			    device, &debugserver_client, TOOL_NAME) !=
		...
		if (debug_level) {
			...
			debugserver_command_new(
				"QSetLogging:bitmask=LOG_ALL|LOG_RNB_REMOTE|LOG_RNB_PACKETS",
				0, NULL, &command);
			dres = debugserver_client_send_command(
				debugserver_client, command, &response, NULL);
			debugserver_command_free(command);
			command = NULL;
			if (response) {
				if (strncmp(response, "OK", 2)) {
					debugserver_client_handle_response(
						debugserver_client, &response,
						0);
					goto cleanup;
				}
				free(response);
		...
		debugserver_command_new("QSetMaxPacketSize:", 1, packet_size,
					&command);
		free(packet_size[0]);
		dres = debugserver_client_send_command(
			debugserver_client, command, &response, NULL);
		...
		debugserver_command_new("QSetWorkingDir:", 1, working_dir,
					&command);
		dres = debugserver_client_send_command(
			debugserver_client, command, &response, NULL);
		debugserver_command_free(command);
		...
		if (environment) {
			...
			for (environment_index = 0;
			     ...
			     environment_index++) {
				log_debug("setting environment variable: %s",
		...
		debugserver_client_set_argv(debugserver_client, app_argc,
					    app_argv, NULL);
		free(app_argv);
		...
		while (!quit_flag) {
			...
			if (response) {
				...
				dres = debugserver_client_handle_response(
					debugserver_client, &response, 1);
			}
