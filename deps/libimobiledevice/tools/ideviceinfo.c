...
static const char *domains[] = {
	...
	"com.apple.fairplay", "com.apple.iTunes",
	"com.apple.mobile.iTunes.store", "com.apple.mobile.iTunes", NULL};
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "dhu:nq:k:sxv", longopts, NULL)) !=
	       -1) {
		switch (c) {
		case 'd':
	...
	if (LOCKDOWN_E_SUCCESS !=
		...
		fprintf(stderr,
			"ERROR: Could not connect to lockdownd: %s (%d)\n",
	...
	if (lockdownd_get_value(client, domain, key, &node) ==
	    LOCKDOWN_E_SUCCESS) {
		if (node) {
			switch (format) {
			case FORMAT_XML:
