		...
		if ((*p == '|') || (*p == '\0')) {
			if (p - start > 0) {
				...
				if (!endp || *endp == 0) {
					...
					pid_filters[num_pid_filters] =
						pid_value;
					num_pid_filters++;
				} else {
					char **new_proc_filters = realloc(
						proc_filters,
						sizeof(char *) *
							(num_proc_filters + 1));
					if (!new_proc_filters) {
...
static void syslog_callback(char c, void *user_data)
{
	...
	if (c == '\0') {
		...
		do {
			if (lp < 16) {
				...
				if (num_untrigger_filters > 0 && triggered) {
					int found = 0;
					int i;
					for (i = 0; i < num_untrigger_filters;
					     i++) {
						if (strstr(device_name_end + 1,
							   untrigger_filters[i])) {
							found = 1;
							break;
						}
				...
				} else if (num_trigger_filters > 0 &&
					   !triggered) {
					int found = 0;
					int i;
					for (i = 0; i < num_trigger_filters;
					     i++) {
						if (strstr(device_name_end + 1,
							   trigger_filters[i])) {
							found = 1;
				...
				if (num_pid_filters > 0) {
					char *endp = NULL;
					int pid_value = (int)strtol(pid_start,
								    &endp, 10);
					if (endp && (*endp == ']')) {
						int found =
							proc_filter_excluding;
						int i = 0;
						for (i = 0; i < num_pid_filters;
						     i++) {
							if (pid_value ==
							    pid_filters[i]) {
				...
				fwrite(process_name_start, 1,
				       process_name_end - process_name_start,
				       stdout);
				TEXT_COLOR(COLOR_CYAN);
				fwrite(process_name_end, 1,
				       proc_name_end - process_name_end + 1,
				       stdout);

				/* write log level */
				TEXT_COLOR(level_color);
				if (level_end > level_start) {
					fwrite(level_start, 1,
					       level_end - level_start, stdout);
					p = level_end;
				}
		...
		if ((num_msg_filters == 0 && num_proc_filters == 0 &&
		     num_pid_filters == 0 && num_trigger_filters == 0 &&
		     num_untrigger_filters == 0) ||
		    shall_print) {
...
static int start_logging(void)
{
	idevice_error_t ret = idevice_new_with_options(
		&device, udid,
		(use_network) ? IDEVICE_LOOKUP_NETWORK : IDEVICE_LOOKUP_USBMUX);
	...
	if (lerr == LOCKDOWN_E_PASSWORD_PROTECTED) {
		...
		while (!quit_flag) {
			lerr = lockdownd_start_service(
				lockdown, SYSLOG_RELAY_SERVICE_NAME, &svc);
			if (lerr != LOCKDOWN_E_PASSWORD_PROTECTED) {
				break;
			}
			sleep(1);
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "dhu:nxt:T:m:e:p:qkKv", longopts,
				NULL)) != -1) {
		switch (c) {
		case 'd':
		...
		case 'm':
			if (!*optarg) {
				...
				char **new_msg_filters = realloc(
					msg_filters,
					sizeof(char *) * (num_msg_filters + 1));
				if (!new_msg_filters) {
					fprintf(stderr,
						"ERROR: realloc() failed\n");
					exit(EXIT_FAILURE);
		...
		case 't':
			if (!*optarg) {
				...
				char **new_trigger_filters = realloc(
					trigger_filters,
					sizeof(char *) *
						(num_trigger_filters + 1));
				...
				trigger_filters[num_trigger_filters] =
					strdup(optarg);
				num_trigger_filters++;
			}
			break;
		case 'T':
			if (!*optarg) {
				...
				char **new_untrigger_filters = realloc(
					untrigger_filters,
					sizeof(char *) *
						(num_untrigger_filters + 1));
				...
				untrigger_filters[num_untrigger_filters] =
					strdup(optarg);
