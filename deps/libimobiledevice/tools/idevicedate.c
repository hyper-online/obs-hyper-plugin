...
int main(int argc, char *argv[])
{
	...
	if (lockdownd_get_value(client, NULL, "TimeIntervalSince1970", &node) !=
	    LOCKDOWN_E_SUCCESS) {
		fprintf(stderr,
			"ERROR: Unable to retrieve 'TimeIntervalSince1970' node from device.\n");
		result = -1;
		goto cleanup;
	}
	...
	if (node == NULL) {
		fprintf(stderr,
			"ERROR: Empty node for 'TimeIntervalSince1970' received.\n");
		result = -1;
		goto cleanup;
	}
	...
	if (setdate == 0) {
		...
		default:
			fprintf(stderr,
				"ERROR: Unexpected node type for 'TimeIntervalSince1970'\n");
			break;
		}
		plist_free(node);
		node = NULL;
		...
		if (lockdownd_set_value(client, NULL, "TimeIntervalSince1970",
					node) == LOCKDOWN_E_SUCCESS) {
			tmp = localtime(&setdate);
			strftime(buffer, 80, DATE_FMT_LANGINFO, tmp);
			puts(buffer);
		} else {
