...
const ASN1_ARRAY_TYPE pkcs1_asn1_tab[] = {
	{"PKCS1", 536872976, 0},        {0, 1073741836, 0},
	{"RSAPublicKey", 536870917, 0}, {"modulus", 1073741827, 0},
	{"publicExponent", 3, 0},       {0, 0, 0}};
#endif
...
#ifdef WIN32
static char *userpref_utf16_to_utf8(wchar_t *unistr, long len, long *items_read,
				    long *items_written)
{
	if (!unistr || (len <= 0))
		return NULL;
...
const char *userpref_get_config_dir()
{
	...
	if (hr == S_OK) {
		...
		if (b) {
			base_config_dir = userpref_utf16_to_utf8(
				path, wcslen(path), NULL, NULL);
			CoTaskMemFree(pidl);
		}
	}
#else
...
#endif
#endif
	__config_dir = string_concat(base_config_dir, DIR_SEP_S,
				     USERPREF_CONFIG_DIR, NULL);
...
int userpref_read_system_buid(char **system_buid)
{
	...
	if (res == 0) {
		debug_info("using %s as %s", *system_buid,
	...
	*list = (char **)malloc(sizeof(char *));
	...
	if (config_dir) {
		...
		while ((entry = readdir(config_dir))) {
			...
			if (ext &&
			    (strcmp(ext, USERPREF_CONFIG_EXTENSION) == 0)) {
				size_t len = strlen(entry->d_name) -
					     strlen(USERPREF_CONFIG_EXTENSION);
				char **newlist = (char **)realloc(
					*list, sizeof(char *) * (found + 2));
				if (!newlist) {
					fprintf(stderr,
						"ERROR: Out of memory\n");
					break;
 ...
 */
userpref_error_t userpref_save_pair_record(const char *udid, uint32_t device_id,
					   plist_t pair_record)
{
	...
	int res = usbmuxd_save_pair_record_with_device_id(
		udid, device_id, record_data, record_size);
 ...
 *         been saved previously.
 */
userpref_error_t userpref_read_pair_record(const char *udid,
					   plist_t *pair_record)
{
	char *record_data = NULL;
	uint32_t record_size = 0;
...
static int X509_add_ext_helper(X509 *cert, int nid, char *value)
{
	...
	if (!ex) {
		debug_info("ERROR: X509V3_EXT_conf_nid(%d, %s) failed", nid,
			   value);
		return 0;
	}
						     ...
						     key_data_t public_key)
{
	...
	X509 *root_cert = X509_new();
	{
		...
		X509_add_ext_helper(root_cert, NID_basic_constraints,
	...
	X509 *host_cert = X509_new();
	{
		...
		/* set x509v3 basic constraints */
		X509_add_ext_helper(host_cert, NID_basic_constraints,
				    (char *)"critical,CA:FALSE");

		/* set x509v3 key usage */
		X509_add_ext_helper(
			host_cert, NID_key_usage,
	...
	if (root_cert && root_pkey && host_cert && host_pkey) {
		...
		if (PEM_write_bio_X509(membp, root_cert) > 0) {
			...
			root_cert_pem.data =
				(unsigned char *)malloc(root_cert_pem.size);
			if (root_cert_pem.data) {
				memcpy(root_cert_pem.data, bdata,
				       root_cert_pem.size);
			}
			BIO_free(membp);
			membp = NULL;
		...
		membp = BIO_new(BIO_s_mem());
		if (PEM_write_bio_PrivateKey(membp, root_pkey, NULL, NULL, 0, 0,
					     NULL) > 0) {
			root_key_pem.size = BIO_get_mem_data(membp, &bdata);
			root_key_pem.data =
				(unsigned char *)malloc(root_key_pem.size);
			if (root_key_pem.data) {
				memcpy(root_key_pem.data, bdata,
				       root_key_pem.size);
			}
			BIO_free(membp);
			membp = NULL;
		...
		if (PEM_write_bio_X509(membp, host_cert) > 0) {
			...
			host_cert_pem.data =
				(unsigned char *)malloc(host_cert_pem.size);
			if (host_cert_pem.data) {
				memcpy(host_cert_pem.data, bdata,
				       host_cert_pem.size);
			}
			BIO_free(membp);
			membp = NULL;
		...
		membp = BIO_new(BIO_s_mem());
		if (PEM_write_bio_PrivateKey(membp, host_pkey, NULL, NULL, 0, 0,
					     NULL) > 0) {
			host_key_pem.size = BIO_get_mem_data(membp, &bdata);
			host_key_pem.data =
				(unsigned char *)malloc(host_key_pem.size);
			if (host_key_pem.data) {
				memcpy(host_key_pem.data, bdata,
				       host_key_pem.size);
			}
			BIO_free(membp);
			membp = NULL;
	...
	if (pubkey && dev_cert) {
		...
		X509_add_ext_helper(dev_cert, NID_basic_constraints,
				    (char *)"critical,CA:FALSE");
		...
		if (X509_sign(dev_cert, root_pkey, EVP_sha1())) {
			...
			if (PEM_write_bio_X509(membp, dev_cert) > 0) {
				char *bdata = NULL;
				dev_cert_pem.size =
					BIO_get_mem_data(membp, &bdata);
				dev_cert_pem.data = (unsigned char *)malloc(
					dev_cert_pem.size);
				if (dev_cert_pem.data) {
					memcpy(dev_cert_pem.data, bdata,
					       dev_cert_pem.size);
				}
				BIO_free(membp);
				membp = NULL;
	...
	gnutls_x509_crt_set_expiration_time(
		root_cert, time(NULL) + (60 * 60 * 24 * 365 * 10));
	...
	gnutls_x509_crt_set_key_usage(host_cert,
				      GNUTLS_KEY_KEY_ENCIPHERMENT |
					      GNUTLS_KEY_DIGITAL_SIGNATURE);
	gnutls_x509_crt_set_activation_time(host_cert, time(NULL));
	gnutls_x509_crt_set_expiration_time(
		host_cert, time(NULL) + (60 * 60 * 24 * 365 * 10));
	...
	gnutls_x509_privkey_export(root_privkey, GNUTLS_X509_FMT_PEM, NULL,
				   &root_key_export_size);
	gnutls_x509_privkey_export(host_privkey, GNUTLS_X509_FMT_PEM, NULL,
	...
	gnutls_x509_privkey_export(root_privkey, GNUTLS_X509_FMT_PEM,
				   root_key_pem.data, &root_key_export_size);
	root_key_pem.size = root_key_export_size;
	gnutls_x509_privkey_export(host_privkey, GNUTLS_X509_FMT_PEM,
				   host_key_pem.data, &host_key_export_size);
	host_key_pem.size = host_key_export_size;

	size_t root_cert_export_size = 0;
	size_t host_cert_export_size = 0;

	gnutls_x509_crt_export(root_cert, GNUTLS_X509_FMT_PEM, NULL,
			       &root_cert_export_size);
	gnutls_x509_crt_export(host_cert, GNUTLS_X509_FMT_PEM, NULL,
	...
	gnutls_x509_crt_export(root_cert, GNUTLS_X509_FMT_PEM,
			       root_cert_pem.data, &root_cert_export_size);
	...
	int gnutls_error = gnutls_pem_base64_decode_alloc(
		"RSA PUBLIC KEY", &public_key, &der_pub_key);
	if (GNUTLS_E_SUCCESS == gnutls_error) {
		/* initalize asn.1 parser */
		ASN1_TYPE pkcs1 = ASN1_TYPE_EMPTY;
		if (ASN1_SUCCESS ==
		    asn1_array2tree(pkcs1_asn1_tab, &pkcs1, NULL)) {

			ASN1_TYPE asn1_pub_key = ASN1_TYPE_EMPTY;
			asn1_create_element(pkcs1, "PKCS1.RSAPublicKey",
					    &asn1_pub_key);

			if (ASN1_SUCCESS ==
			    asn1_der_decoding(&asn1_pub_key, der_pub_key.data,
					      der_pub_key.size, NULL)) {

				/* get size to read */
				int ret1 = asn1_read_value(
					asn1_pub_key, "modulus", NULL,
					(int *)&modulus.size);
				int ret2 = asn1_read_value(
					asn1_pub_key, "publicExponent", NULL,
					(int *)&exponent.size);
	...
	/* generate device certificate */
	if (modulus.data && 0 != modulus.size && exponent.data &&
	    0 != exponent.size) {

		gnutls_datum_t prime_p = {
			(unsigned char *)"\x00\xca\x4a\x03\x13\xdf\x9d\x7a\xfd",
			9};
		gnutls_datum_t prime_q = {
			(unsigned char *)"\x00\xf2\xff\xe0\x15\xd1\x60\x37\x63",
			9};
		gnutls_datum_t coeff = {
			(unsigned char *)"\x32\x07\xf1\x68\x57\xdf\x9a\xf4", 8};
		...
		gnutls_error = gnutls_x509_privkey_import_rsa_raw(
			fake_privkey, &modulus, &exponent, &exponent, &prime_p,
			&prime_q, &coeff);
		if (GNUTLS_E_SUCCESS == gnutls_error) {
			...
			const gnutls_digest_algorithm_t dig_sha1 =
				GNUTLS_DIG_SHA1;
			size_t hash_size = gnutls_hash_get_len(dig_sha1);
			unsigned char hash[hash_size];
			if (gnutls_hash_fast(dig_sha1, der_pub_key.data,
					     der_pub_key.size,
				...
				gnutls_x509_crt_set_subject_key_id(
					dev_cert, hash, hash_size);
			}

			gnutls_x509_crt_set_key_usage(
				dev_cert, GNUTLS_KEY_DIGITAL_SIGNATURE |
						  GNUTLS_KEY_KEY_ENCIPHERMENT);
			gnutls_error = gnutls_x509_crt_sign2(
				dev_cert, root_cert, root_privkey,
				GNUTLS_DIG_SHA1, 0);
			if (GNUTLS_E_SUCCESS == gnutls_error) {
				...
				debug_info(
					"ERROR: Signing device certificate with root private key failed: %s",
					gnutls_strerror(gnutls_error));
			}
		} else {
	...
	if (root_cert_pem.data && 0 != root_cert_pem.size &&
	    root_key_pem.data && 0 != root_key_pem.size && host_cert_pem.data &&
	    0 != host_cert_pem.size && host_key_pem.data &&
	    0 != host_key_pem.size && dev_cert_pem.data &&
	    0 != dev_cert_pem.size) {
		/* now set keys and certificates */
		pair_record_set_item_from_key_data(
			pair_record, USERPREF_DEVICE_CERTIFICATE_KEY,
			&dev_cert_pem);
		pair_record_set_item_from_key_data(
			pair_record, USERPREF_HOST_PRIVATE_KEY_KEY,
			&host_key_pem);
		pair_record_set_item_from_key_data(
			pair_record, USERPREF_HOST_CERTIFICATE_KEY,
			&host_cert_pem);
		pair_record_set_item_from_key_data(
			pair_record, USERPREF_ROOT_PRIVATE_KEY_KEY,
			&root_key_pem);
		pair_record_set_item_from_key_data(
 ...
 */
#ifdef HAVE_OPENSSL
userpref_error_t pair_record_import_key_with_name(plist_t pair_record,
						  const char *name,
						  key_data_t *key)
#else
userpref_error_t pair_record_import_key_with_name(plist_t pair_record,
...
#endif
{
	...
	if (ret == USERPREF_E_SUCCESS &&
	    GNUTLS_E_SUCCESS ==
		    gnutls_x509_privkey_import(key, &pem, GNUTLS_X509_FMT_PEM))
		ret = USERPREF_E_SUCCESS;
	else
 ...
 */
#ifdef HAVE_OPENSSL
userpref_error_t pair_record_import_crt_with_name(plist_t pair_record,
						  const char *name,
						  key_data_t *cert)
#else
userpref_error_t pair_record_import_crt_with_name(plist_t pair_record,
...
#endif
{
	...
	if (ret == USERPREF_E_SUCCESS &&
	    GNUTLS_E_SUCCESS ==
		    gnutls_x509_crt_import(cert, &pem, GNUTLS_X509_FMT_PEM))
		ret = USERPREF_E_SUCCESS;
	else
		ret = USERPREF_E_SSL_ERROR;
...
userpref_error_t pair_record_set_host_id(plist_t pair_record,
					 const char *host_id)
{
	plist_dict_set_item(pair_record, USERPREF_HOST_ID_KEY,
			    plist_new_string(host_id));

	return USERPREF_E_SUCCESS;
}

userpref_error_t pair_record_get_item_as_key_data(plist_t pair_record,
						  const char *name,
						  key_data_t *value)
{
...
userpref_error_t pair_record_set_item_from_key_data(plist_t pair_record,
						    const char *name,
						    key_data_t *value)
{
	userpref_error_t ret = USERPREF_E_SUCCESS;
	...
	/* set new item */
	plist_dict_set_item(pair_record, name,
