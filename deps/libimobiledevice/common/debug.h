...
void internal_set_debug_callback(idevice_debug_cb_t callback);
	...
	!defined(STRIP_DEBUG_CODE)
#define debug_info(...) \
	debug_info_real(__func__, __FILE__, __LINE__, __VA_ARGS__)
#define debug_plist(a) debug_plist_real(__func__, __FILE__, __LINE__, a)
#elif ((defined(__GNUC__) && __GNUC__ >= 3) || (defined(_MSC_VER))) && \
	!defined(STRIP_DEBUG_CODE)
#define debug_info(...) \
	debug_info_real(__FUNCTION__, __FILE__, __LINE__, __VA_ARGS__)
...
void debug_plist_real(const char *func, const char *file, int line,
		      plist_t plist);
