...
int socket_connect_unix(const char *filename)
{
	...
	do {
		if (connect(sfd, (struct sockaddr *)&name, sizeof(name)) !=
		    -1) {
			break;
		}
		if (errno == EINPROGRESS) {
			...
			timeout.tv_usec =
				(CONNECT_TIMEOUT - (timeout.tv_sec * 1000)) *
				1000;
			if (select(sfd + 1, NULL, &fds, NULL, &timeout) == 1) {
				int so_error;
				socklen_t len = sizeof(so_error);
				getsockopt(sfd, SOL_SOCKET, SO_ERROR,
					   (void *)&so_error, &len);
				if (so_error == 0) {
					break;
				}
...
static int32_t _sockaddr_in6_scope_id(struct sockaddr_in6 *addr)
{
	...
	for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
		...
		if ((addr_in->sin6_scope_id > addr->sin6_scope_id) &&
		    (res >= 0)) {
			// use last valid scope id as we're past the requested scope id
			break;
		}
...
int socket_connect_addr(struct sockaddr *addr, uint16_t port)
{
	...
	if (setsockopt(sfd, IPPROTO_TCP, TCP_NODELAY, (void *)&yes,
		       sizeof(int)) == -1) {
...
int socket_check_fd(int fd, fd_mode fdm, unsigned int timeout)
{
	...
	do {
		if (timeout > 0) {
			...
			to.tv_usec =
				(time_t)((timeout - (to.tv_sec * 1000)) * 1000);
			pto = &to;
		} else {
			pto = NULL;
		}
