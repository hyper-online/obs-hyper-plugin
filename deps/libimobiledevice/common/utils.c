...
static char *base64encode(const unsigned char *buf, size_t size)
{
	...
	while (n < size) {
		...
		outbuf[m++] = (n + 1 < size) ? base64_str[(int)output[2]]
					     : base64_pad;
		outbuf[m++] = (n + 2 < size) ? base64_str[(int)output[3]]
					     : base64_pad;
		n += 3;
	}
...
static void plist_node_print_to_stream(plist_t node, int *indent_level,
				       FILE *stream);

static void plist_array_print_to_stream(plist_t node, int *indent_level,
					FILE *stream)
{
	/* iterate over items */
	int i, count;
...
static void plist_dict_print_to_stream(plist_t node, int *indent_level,
				       FILE *stream)
{
	...
	while (subnode) {
		...
		if (plist_get_node_type(subnode) == PLIST_ARRAY)
			fprintf(stream,
				"[%d]: ", plist_array_get_size(subnode));
		else
			fprintf(stream, ": ");
		free(key);
		key = NULL;
				       ...
				       FILE *stream)
{
	...
	case PLIST_DATE:
		plist_get_date_val(node, (int32_t *)&tv.tv_sec,
				   (int32_t *)&tv.tv_usec);
		{
			...
			if (btime) {
				s = (char *)malloc(24);
				memset(s, 0, 24);
				if (strftime(s, 24, "%Y-%m-%dT%H:%M:%SZ",
					     btime) <= 0) {
					free(s);
					s = NULL;
				}
