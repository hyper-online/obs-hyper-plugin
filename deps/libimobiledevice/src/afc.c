...
LIBIMOBILEDEVICE_API afc_error_t afc_client_new_with_service_client(
	service_client_t service_client, afc_client_t *client)
{
	if (!service_client)
		return AFC_E_INVALID_ARG;

	afc_client_t client_loc =
		(afc_client_t)malloc(sizeof(struct afc_client_private));
...
afc_client_new(idevice_t device, lockdownd_service_descriptor_t service,
	       afc_client_t *client)
{
...
LIBIMOBILEDEVICE_API afc_error_t afc_client_start_service(idevice_t device,
							  afc_client_t *client,
							  const char *label)
{
	afc_error_t err = AFC_E_UNKNOWN_ERROR;
	service_client_factory_start_service(
		device, AFC_SERVICE_NAME, (void **)client, label,
		SERVICE_CONSTRUCTOR(afc_client_new), &err);
	return err;
}
 ...
 */
static afc_error_t afc_dispatch_packet(afc_client_t client, uint64_t operation,
				       ...
				       uint32_t *bytes_sent)
{
	...
	*bytes_sent = 0;
	...
	client->afc_packet->entire_length =
		sizeof(AFCPacket) + data_length + payload_length;
	...
	debug_buffer((char *)client->afc_packet,
		     sizeof(AFCPacket) + data_length);
	sent = 0;
	service_send(client->parent, (void *)client->afc_packet,
		     sizeof(AFCPacket) + data_length, &sent);
	...
	*bytes_sent += sent;
	...
	if (payload_length > 0) {
		if (payload_length > 256) {
			debug_info("packet payload follows (256/%u)",
				   payload_length);
 ...
 * @return AFC_E_SUCCESS on success or an AFC_E_* error value.
 */
static afc_error_t afc_receive_data(afc_client_t client, char **bytes,
				    uint32_t *bytes_recv)
{
	AFCPacket header;
	...
	service_receive(client->parent, (char *)&header, sizeof(AFCPacket),
			bytes_recv);
	...
	if (header.packet_num != client->afc_packet->packet_num) {
		...
		debug_info(
			...
			header.packet_num, client->afc_packet->packet_num);
	...
	debug_info(
		...
		header.entire_length, header.this_length, header.operation);
	...
	if (this_len > 0) {
		service_receive(client->parent, dump_here, this_len,
				bytes_recv);
		if (*bytes_recv <= 0) {
			free(dump_here);
			...
			free(dump_here);
			debug_info("Could not receive this_len=%d bytes",
				   this_len);
			return AFC_E_NOT_ENOUGH_DATA;
		}
	}
	...
	if (entire_len > this_len) {
		while (current_count < entire_len) {
			service_receive(client->parent,
					dump_here + current_count,
					entire_len - current_count, bytes_recv);
			if (*bytes_recv <= 0) {
				debug_info(
...
static int _afc_check_packet_buffer(afc_client_t client, uint32_t data_len)
{
	if (data_len > client->packet_extra) {
		...
		AFCPacket *newpkt = (AFCPacket *)realloc(
			client->afc_packet,
			sizeof(AFCPacket) + client->packet_extra);
		if (!newpkt) {
			return -1;
		}
...
LIBIMOBILEDEVICE_API afc_error_t afc_read_directory(
	afc_client_t client, const char *path, char ***directory_information)
{
	uint32_t bytes = 0;
	char *data = NULL, **list_loc = NULL;
	afc_error_t ret = AFC_E_UNKNOWN_ERROR;

	if (!client || !path || !directory_information ||
	...
	ret = afc_dispatch_packet(client, AFC_OP_READ_DIR, data_len, NULL, 0,
				  &bytes);
...
LIBIMOBILEDEVICE_API afc_error_t afc_get_device_info(afc_client_t client,
						     char ***device_information)
{
	...
	/* Send the command */
	ret = afc_dispatch_packet(client, AFC_OP_GET_DEVINFO, 0, NULL, 0,
				  &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
LIBIMOBILEDEVICE_API afc_error_t afc_get_device_info_key(afc_client_t client,
							 const char *key,
							 char **value)
{
...
LIBIMOBILEDEVICE_API afc_error_t afc_remove_path(afc_client_t client,
						 const char *path)
{
	uint32_t bytes = 0;
	afc_error_t ret = AFC_E_UNKNOWN_ERROR;
	...
	/* Send command */
	memcpy(AFC_PACKET_DATA_PTR, path, data_len);
	ret = afc_dispatch_packet(client, AFC_OP_REMOVE_PATH, data_len, NULL, 0,
				  &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
LIBIMOBILEDEVICE_API afc_error_t afc_rename_path(afc_client_t client,
						 const char *from,
						 const char *to)
{
	...
	ret = afc_dispatch_packet(client, AFC_OP_RENAME_PATH, data_len, NULL, 0,
				  &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
LIBIMOBILEDEVICE_API afc_error_t afc_make_directory(afc_client_t client,
						    const char *path)
{
	uint32_t bytes = 0;
	afc_error_t ret = AFC_E_UNKNOWN_ERROR;
	...
	/* Send command */
	memcpy(AFC_PACKET_DATA_PTR, path, data_len);
	ret = afc_dispatch_packet(client, AFC_OP_MAKE_DIR, data_len, NULL, 0,
				  &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
LIBIMOBILEDEVICE_API afc_error_t afc_get_file_info(afc_client_t client,
						   const char *path,
						   char ***file_information)
{
	...
	/* Send command */
	memcpy(AFC_PACKET_DATA_PTR, path, data_len);
	ret = afc_dispatch_packet(client, AFC_OP_GET_FILE_INFO, data_len, NULL,
				  0, &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
LIBIMOBILEDEVICE_API afc_error_t afc_file_open(afc_client_t client,
					       const char *filename,
					       ...
					       uint64_t *handle)
{
	...
	*(uint64_t *)(AFC_PACKET_DATA_PTR) = htole64(file_mode);
	...
	ret = afc_dispatch_packet(client, AFC_OP_FILE_OPEN, data_len, NULL, 0,
				  &bytes);
	if (ret != AFC_E_SUCCESS) {
...
LIBIMOBILEDEVICE_API afc_error_t afc_file_read(afc_client_t client,
					       ...
					       uint32_t *bytes_read)
{
	...
	ret = afc_dispatch_packet(client, AFC_OP_FILE_READ,
				  sizeof(struct readinfo), NULL, 0, &bytes_loc);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
	}
	...
	if (ret != AFC_E_SUCCESS) {
		...
		*bytes_read = current_count;
		...
		if (input) {
			...
			memcpy(data + current_count, input,
			       (bytes_loc > length) ? length : bytes_loc);
			free(input);
			input = NULL;
			current_count += (bytes_loc > length) ? length
							      : bytes_loc;
		}
	}
	afc_unlock(client);
...
LIBIMOBILEDEVICE_API afc_error_t afc_file_write(afc_client_t client,
						uint64_t handle,
						const char *data,
						uint32_t length,
						uint32_t *bytes_written)
{
	...
	if (!client || !client->afc_packet || !client->parent ||
	    !bytes_written || (handle == 0))
		return AFC_E_INVALID_ARG;
	...
	*(uint64_t *)(AFC_PACKET_DATA_PTR) = handle;
	ret = afc_dispatch_packet(client, AFC_OP_FILE_WRITE, data_len, data,
				  length, &bytes_loc);
...
LIBIMOBILEDEVICE_API afc_error_t afc_file_close(afc_client_t client,
						uint64_t handle)
{
	uint32_t bytes = 0;
	afc_error_t ret = AFC_E_UNKNOWN_ERROR;
	...
	/* Send command */
	*(uint64_t *)(AFC_PACKET_DATA_PTR) = handle;
	ret = afc_dispatch_packet(client, AFC_OP_FILE_CLOSE, data_len, NULL, 0,
				  &bytes);
...
LIBIMOBILEDEVICE_API afc_error_t afc_file_lock(afc_client_t client,
					       uint64_t handle,
					       afc_lock_op_t operation)
{
	...
	ret = afc_dispatch_packet(client, AFC_OP_FILE_LOCK,
				  sizeof(struct lockinfo), NULL, 0, &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
...
LIBIMOBILEDEVICE_API afc_error_t afc_file_seek(afc_client_t client,
					       ...
					       int whence)
{
	...
	ret = afc_dispatch_packet(client, AFC_OP_FILE_SEEK,
				  sizeof(struct seekinfo), NULL, 0, &bytes);
...
LIBIMOBILEDEVICE_API afc_error_t afc_file_tell(afc_client_t client,
					       uint64_t handle,
					       uint64_t *position)
{
	char *buffer = NULL;
	...
	/* Send the command */
	*(uint64_t *)(AFC_PACKET_DATA_PTR) = handle;
	ret = afc_dispatch_packet(client, AFC_OP_FILE_TELL, data_len, NULL, 0,
				  &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
LIBIMOBILEDEVICE_API afc_error_t afc_file_truncate(afc_client_t client,
						   uint64_t handle,
						   uint64_t newsize)
{
	uint32_t bytes = 0;
	...
	ret = afc_dispatch_packet(client, AFC_OP_FILE_SET_SIZE,
				  sizeof(struct truncinfo), NULL, 0, &bytes);
...
LIBIMOBILEDEVICE_API afc_error_t afc_truncate(afc_client_t client,
					      const char *path,
					      uint64_t newsize)
{
	if (!client || !path || !client->afc_packet || !client->parent)
	...
	*(uint64_t *)(AFC_PACKET_DATA_PTR) = htole64(newsize);
	...
	ret = afc_dispatch_packet(client, AFC_OP_TRUNCATE, data_len, NULL, 0,
				  &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
LIBIMOBILEDEVICE_API afc_error_t afc_make_link(afc_client_t client,
					       afc_link_type_t linktype,
					       const char *target,
					       const char *linkname)
{
	if (!client || !target || !linkname || !client->afc_packet ||
	    !client->parent)
		return AFC_E_INVALID_ARG;
	...
	*(uint64_t *)(AFC_PACKET_DATA_PTR) = htole64(linktype);
	...
	memcpy(AFC_PACKET_DATA_PTR + 8 + target_len + 1, linkname,
	       link_len + 1);
	ret = afc_dispatch_packet(client, AFC_OP_MAKE_LINK, data_len, NULL, 0,
				  &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
LIBIMOBILEDEVICE_API afc_error_t afc_set_file_time(afc_client_t client,
						   const char *path,
						   uint64_t mtime)
{
	if (!client || !path || !client->afc_packet || !client->parent)
	...
	*(uint64_t *)(AFC_PACKET_DATA_PTR) = htole64(mtime);
	memcpy(AFC_PACKET_DATA_PTR + 8, path, data_len - 8);
	ret = afc_dispatch_packet(client, AFC_OP_SET_FILE_MOD_TIME, data_len,
				  NULL, 0, &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
...
LIBIMOBILEDEVICE_API afc_error_t
afc_remove_path_and_contents(afc_client_t client, const char *path)
{
	uint32_t bytes = 0;
	afc_error_t ret = AFC_E_UNKNOWN_ERROR;
	...
	/* Send command */
	memcpy(AFC_PACKET_DATA_PTR, path, data_len);
	ret = afc_dispatch_packet(client, AFC_OP_REMOVE_PATH_AND_CONTENTS,
				  data_len, NULL, 0, &bytes);
	if (ret != AFC_E_SUCCESS) {
		afc_unlock(client);
		return AFC_E_NOT_ENOUGH_DATA;
	}
