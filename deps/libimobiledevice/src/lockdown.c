...
static struct st_lockdownd_error_str_map lockdownd_error_str_map[] = {
	...
	{"RemoveProhibited", "Remove value prohibited",
	...
	{"UserDeniedPairing", "User denied pairing",
	 LOCKDOWN_E_USER_DENIED_PAIRING},
	{"PairingDialogResponsePending", "Pairing dialog response pending",
...
static lockdownd_error_t lockdownd_strtoerr(const char *name)
{
	...
	while (lockdownd_error_str_map[i].lockdown_errstr) {
		if (strcmp(lockdownd_error_str_map[i].lockdown_errstr, name) ==
		    0) {
 ...
 */
static lockdownd_error_t lockdown_check_result(plist_t dict,
	...
	if (result_type == PLIST_STRING) {
		...
		if (result_value) {
			if (!strcmp(result_value, "Success")) {
				...
				debug_info("ERROR: unknown result value '%s'",
					   result_value);
			}
		}
...
static void plist_dict_add_label(plist_t plist, const char *label)
{
	if (plist && label) {
		if (plist_get_node_type(plist) == PLIST_DICT)
			plist_dict_set_item(plist, "Label",
					    plist_new_string(label));
	}
}

LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_stop_session(lockdownd_client_t client, const char *session_id)
{
	if (!client)
		return LOCKDOWN_E_INVALID_ARG;
...
LIBIMOBILEDEVICE_API void lockdownd_client_set_label(lockdownd_client_t client,
						     const char *label)
{
	if (client) {
		if (client->label)
...
LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_receive(lockdownd_client_t client, plist_t *plist)
{
	if (!client || !plist || (plist && *plist))
		return LOCKDOWN_E_INVALID_ARG;

	return lockdownd_error(
		property_list_service_receive_plist(client->parent, plist));
}

LIBIMOBILEDEVICE_API lockdownd_error_t lockdownd_send(lockdownd_client_t client,
						      plist_t plist)
{
	if (!client || !plist)
		return LOCKDOWN_E_INVALID_ARG;
	...
	return lockdownd_error(
		property_list_service_send_xml_plist(client->parent, plist));
}

LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_query_type(lockdownd_client_t client, char **type)
{
	if (!client)
		return LOCKDOWN_E_INVALID_ARG;
...
LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_get_value(lockdownd_client_t client, const char *domain,
...
LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_set_value(lockdownd_client_t client, const char *domain,
		    const char *key, plist_t value)
{
	if (!client || !value)
		return LOCKDOWN_E_INVALID_ARG;
...
LIBIMOBILEDEVICE_API lockdownd_error_t lockdownd_remove_value(
	lockdownd_client_t client, const char *domain, const char *key)
{
	if (!client)
		return LOCKDOWN_E_INVALID_ARG;
...
LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_get_device_udid(lockdownd_client_t client, char **udid)
{
	lockdownd_error_t ret = LOCKDOWN_E_UNKNOWN_ERROR;
	plist_t value = NULL;
 ...
 */
static lockdownd_error_t
lockdownd_get_device_public_key_as_key_data(lockdownd_client_t client,
					    key_data_t *public_key)
{
	lockdownd_error_t ret = LOCKDOWN_E_UNKNOWN_ERROR;
	plist_t value = NULL;
...
LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_get_device_name(lockdownd_client_t client, char **device_name)
{
	lockdownd_error_t ret = LOCKDOWN_E_UNKNOWN_ERROR;
	plist_t value = NULL;
...
LIBIMOBILEDEVICE_API lockdownd_error_t lockdownd_client_new(
	idevice_t device, lockdownd_client_t *client, const char *label)
{
	if (!device || !client)
		return LOCKDOWN_E_INVALID_ARG;

	static struct lockdownd_service_descriptor service = {.port = 0xf27e,
							      .ssl_enabled = 0};

	property_list_service_client_t plistclient = NULL;
	if (property_list_service_client_new(
		    device, (lockdownd_service_descriptor_t)&service,
		    &plistclient) != PROPERTY_LIST_SERVICE_E_SUCCESS) {
		debug_info("could not connect to lockdownd (device %s)",
			   device->udid);
		return LOCKDOWN_E_MUX_ERROR;
	}

	lockdownd_client_t client_loc = (lockdownd_client_t)malloc(
...
LIBIMOBILEDEVICE_API lockdownd_error_t lockdownd_client_new_with_handshake(
	idevice_t device, lockdownd_client_t *client, const char *label)
{
	if (!client)
		return LOCKDOWN_E_INVALID_ARG;
	...
	if (device->version == 0) {
		...
		if (lockdownd_get_value(client_loc, NULL, "ProductVersion",
			...
			if (s_version && sscanf(s_version, "%d.%d.%d", &vers[0],
						&vers[1], &vers[2]) >= 2) {
	...
	if (LOCKDOWN_E_SUCCESS == ret) {
		if (!host_id) {
			userpref_read_pair_record(client_loc->udid,
						  &pair_record);
			if (pair_record) {
				pair_record_get_host_id(pair_record, &host_id);
				plist_free(pair_record);
 ...
 */
static plist_t
lockdownd_pair_record_to_plist(lockdownd_pair_record_t pair_record)
{
	if (!pair_record)
		return NULL;
	...
	plist_dict_set_item(
		dict, "HostCertificate",
		plist_new_data(pair_record->host_certificate,
			       strlen(pair_record->host_certificate)));
	plist_dict_set_item(dict, "HostID",
			    plist_new_string(pair_record->host_id));
	plist_dict_set_item(
		dict, "RootCertificate",
		plist_new_data(pair_record->root_certificate,
			       strlen(pair_record->root_certificate)));
	plist_dict_set_item(dict, "SystemBUID",
 ...
 * @return LOCKDOWN_E_SUCCESS on success
 */
static lockdownd_error_t pair_record_generate(lockdownd_client_t client,
					      plist_t *pair_record)
{
	lockdownd_error_t ret = LOCKDOWN_E_UNKNOWN_ERROR;
 ...
 */
static lockdownd_error_t lockdownd_do_pair(lockdownd_client_t client,
					   lockdownd_pair_record_t pair_record,
					   const char *verb, plist_t options,
					   plist_t *result)
{
	if (!client)
		return LOCKDOWN_E_INVALID_ARG;
	...
	if (pair_record && pair_record->system_buid && pair_record->host_id) {
		...
		if (!pair_record->device_certificate ||
		...
		if (!strcmp("Pair", verb)) {
			...
			lockdownd_get_value(client, NULL, "WiFiAddress",
					    &wifi_node);
		} else {
			/* use existing pair record */
			userpref_read_pair_record(client->udid,
						  &pair_record_plist);
			if (!pair_record_plist) {
	...
	plist_dict_remove_item(request_pair_record,
	...
	plist_dict_set_item(dict, "ProtocolVersion",
			    plist_new_string(LOCKDOWN_PROTOCOL_VERSION));

	if (options) {
		plist_dict_set_item(dict, "PairingOptions",
				    plist_copy(options));
	}
		...
		if (!pairing_mode) {
			...
			if (!strcmp("Unpair", verb)) {
				...
				if (!strcmp("Pair", verb)) {
					...
					plist_t extra_node = plist_dict_get_item(
						dict, USERPREF_ESCROW_BAG_KEY);
					if (extra_node &&
					    plist_get_node_type(extra_node) ==
						    PLIST_DATA) {
						debug_info(
...
LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_pair(lockdownd_client_t client, lockdownd_pair_record_t pair_record)
{

	plist_t options = plist_new_dict();
	plist_dict_set_item(options, "ExtendedPairingErrors",
			    plist_new_bool(1));

	lockdownd_error_t ret =
		lockdownd_do_pair(client, pair_record, "Pair", options, NULL);
...
LIBIMOBILEDEVICE_API lockdownd_error_t lockdownd_pair_with_options(
	lockdownd_client_t client, lockdownd_pair_record_t pair_record,
	plist_t options, plist_t *response)
{
	return lockdownd_do_pair(client, pair_record, "Pair", options,
				 response);
}

LIBIMOBILEDEVICE_API lockdownd_error_t lockdownd_validate_pair(
	lockdownd_client_t client, lockdownd_pair_record_t pair_record)
{
	return lockdownd_do_pair(client, pair_record, "ValidatePair", NULL,
				 NULL);
}

LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_unpair(lockdownd_client_t client, lockdownd_pair_record_t pair_record)
{
...
LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_goodbye(lockdownd_client_t client)
{
	if (!client)
		return LOCKDOWN_E_INVALID_ARG;
...
LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_start_session(lockdownd_client_t client, const char *host_id,
			char **session_id, int *ssl_enabled)
{
	lockdownd_error_t ret = LOCKDOWN_E_SUCCESS;
	plist_t dict = NULL;
	...
	if (ret == LOCKDOWN_E_SUCCESS) {
		...
		plist_t enable_ssl =
			plist_dict_get_item(dict, "EnableSessionSSL");
		if (enable_ssl &&
		    (plist_get_node_type(enable_ssl) == PLIST_BOOLEAN)) {
		...
		if (session_node &&
		    (plist_get_node_type(session_node) == PLIST_STRING)) {
		...
		debug_info("Enable SSL Session: %s",
			   (use_ssl ? "true" : "false"));

		if (use_ssl) {
			ret = lockdownd_error(property_list_service_enable_ssl(
				client->parent));
			client->ssl_enabled =
				(ret == LOCKDOWN_E_SUCCESS ? 1 : 0);
		} else {
			ret = LOCKDOWN_E_SUCCESS;
			client->ssl_enabled = 0;
		}
 ...
 */
static lockdownd_error_t
lockdownd_build_start_service_request(lockdownd_client_t client,
				      const char *identifier,
				      int send_escrow_bag, plist_t *request)
{
	plist_t dict = plist_new_dict();
	...
	if (send_escrow_bag) {
		...
		plist_t escrow_bag = plist_dict_get_item(
			pair_record, USERPREF_ESCROW_BAG_KEY);
		if (!escrow_bag ||
		    (PLIST_DATA != plist_get_node_type(escrow_bag))) {
			debug_info(
		...
		debug_info("Adding escrow bag to StartService for %s",
			   identifier);
		plist_dict_set_item(dict, USERPREF_ESCROW_BAG_KEY,
 ...
 */
static lockdownd_error_t
lockdownd_do_start_service(lockdownd_client_t client, const char *identifier,
			   int send_escrow_bag,
			   lockdownd_service_descriptor_t *service)
{
	...
	if (ret == LOCKDOWN_E_SUCCESS) {
		if (*service == NULL)
			*service = (lockdownd_service_descriptor_t)malloc(
		...
		if (error_node &&
		    PLIST_STRING == plist_get_node_type(error_node)) {
			char *error = NULL;
...
LIBIMOBILEDEVICE_API lockdownd_error_t lockdownd_start_service_with_escrow_bag(
...
LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_activate(lockdownd_client_t client, plist_t activation_record)
{
	if (!client)
		return LOCKDOWN_E_INVALID_ARG;
	...
	plist_dict_set_item(dict, "ActivationRecord",
...
LIBIMOBILEDEVICE_API lockdownd_error_t
lockdownd_deactivate(lockdownd_client_t client)
{
	if (!client)
		return LOCKDOWN_E_INVALID_ARG;
...
LIBIMOBILEDEVICE_API lockdownd_error_t lockdownd_get_sync_data_classes(
	lockdownd_client_t client, char ***classes, int *count)
{
	if (!client)
		return LOCKDOWN_E_INVALID_ARG;
	...
	*count = 0;
	...
	err = lockdownd_get_value(client, "com.apple.mobile.iTunes",
				  "SyncDataClasses", &dict);
