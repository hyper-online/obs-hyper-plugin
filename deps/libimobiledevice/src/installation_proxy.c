...
instproxy_client_new(idevice_t device, lockdownd_service_descriptor_t service,
		     instproxy_client_t *client)
{
	property_list_service_client_t plistclient = NULL;
	instproxy_error_t err = instproxy_error(
		property_list_service_client_new(device, service,
						 &plistclient));
	if (err != INSTPROXY_E_SUCCESS) {
		return err;
	}

	instproxy_client_t client_loc = (instproxy_client_t)malloc(
...
LIBIMOBILEDEVICE_API instproxy_error_t instproxy_client_start_service(
	idevice_t device, instproxy_client_t *client, const char *label)
{
	instproxy_error_t err = INSTPROXY_E_UNKNOWN_ERROR;
	service_client_factory_start_service(
		device, INSTPROXY_SERVICE_NAME, (void **)client, label,
		SERVICE_CONSTRUCTOR(instproxy_client_new), &err);
	return err;
}

LIBIMOBILEDEVICE_API instproxy_error_t
instproxy_client_free(instproxy_client_t client)
{
	if (!client)
		return INSTPROXY_E_INVALID_ARG;
 ...
 *     an error occurred.
 */
static instproxy_error_t instproxy_send_command(instproxy_client_t client,
						plist_t command)
{
	if (!client || !command)
		return INSTPROXY_E_INVALID_ARG;

	instproxy_error_t res = instproxy_error(
 ...
 */
static instproxy_error_t
instproxy_receive_status_loop(instproxy_client_t client, plist_t command,
			      instproxy_status_cb_t status_cb, void *user_data)
{
	...
	do {
		...
		if (node) {
			...
			res = instproxy_status_get_error(node, &error_name,
							 &error_description,
							 &error_code);
			if (res != INSTPROXY_E_SUCCESS) {
				debug_info(
					"command: %s, error %d, code 0x%08" PRIx64
					", name: %s, description: \"%s\"",
					command_name, res, error_code,
					error_name,
...
static void *instproxy_receive_status_loop_thread(void *arg)
{
	struct instproxy_status_data *data =
		(struct instproxy_status_data *)arg;

	/* run until the command is complete or an error occurs */
	(void)instproxy_receive_status_loop(data->client, data->command,
					    data->cbfunc, data->user_data);
 ...
 */
static instproxy_error_t instproxy_receive_status_loop_with_callback(
	instproxy_client_t client, plist_t command,
	instproxy_command_type_t async, instproxy_status_cb_t status_cb,
	void *user_data)
{
	...
	if (async == INSTPROXY_COMMAND_TYPE_ASYNC) {
		/* async mode */
		struct instproxy_status_data *data =
			(struct instproxy_status_data *)malloc(
				sizeof(struct instproxy_status_data));
		if (data) {
			data->client = client;
			...
			if (thread_new(&client->receive_status_thread,
		...
		res = instproxy_receive_status_loop(client, command, status_cb,
						    user_data);
	}
 ...
 */
static instproxy_error_t
instproxy_perform_command(instproxy_client_t client, plist_t command,
			  instproxy_command_type_t async,
			  instproxy_status_cb_t status_cb, void *user_data)
{
	if (!client || !client->parent || !command) {
		return INSTPROXY_E_INVALID_ARG;
	...
	res = instproxy_receive_status_loop_with_callback(
		client, command, async, status_cb, user_data);

	return res;
}

LIBIMOBILEDEVICE_API instproxy_error_t instproxy_browse_with_callback(
	instproxy_client_t client, plist_t client_options,
	instproxy_status_cb_t status_cb, void *user_data)
{
	...
	if (client_options)
		plist_dict_set_item(command, "ClientOptions",
				    plist_copy(client_options));

	res = instproxy_perform_command(client, command,
					INSTPROXY_COMMAND_TYPE_ASYNC, status_cb,
					(void *)user_data);
						       ...
						       void *user_data)
{
	...
	instproxy_status_get_current_list(status, NULL, NULL, &current_amount,
					  &current_list);
	...
	if (current_amount > 0) {
		for (i = 0; current_list && (i < current_amount); i++) {
			...
			plist_array_append_item(*result_array,
						plist_copy(item));
		}
	}
...
LIBIMOBILEDEVICE_API instproxy_error_t instproxy_browse(
	instproxy_client_t client, plist_t client_options, plist_t *result)
{
	...
	res = instproxy_perform_command(
		client, command, INSTPROXY_COMMAND_TYPE_SYNC,
...
static void instproxy_copy_lookup_result_cb(plist_t command, plist_t status,
					    void *user_data)
{
...
LIBIMOBILEDEVICE_API instproxy_error_t
instproxy_lookup(instproxy_client_t client, const char **appids,
		 plist_t client_options, plist_t *result)
{
	instproxy_error_t res = INSTPROXY_E_UNKNOWN_ERROR;
	int i = 0;
	...
	if (appids) {
		...
		while (appids[i]) {
			plist_array_append_item(appid_array,
						plist_new_string(appids[i]));
			i++;
		}
...
LIBIMOBILEDEVICE_API instproxy_error_t instproxy_install(
	instproxy_client_t client, const char *pkg_path, plist_t client_options,
	instproxy_status_cb_t status_cb, void *user_data)
{
	instproxy_error_t res = INSTPROXY_E_UNKNOWN_ERROR;
...
LIBIMOBILEDEVICE_API instproxy_error_t instproxy_uninstall(
	instproxy_client_t client, const char *appid, plist_t client_options,
	instproxy_status_cb_t status_cb, void *user_data)
{
	instproxy_error_t res = INSTPROXY_E_UNKNOWN_ERROR;
	...
	if (client_options)
		plist_dict_set_item(command, "ClientOptions",
				    plist_copy(client_options));
	plist_dict_set_item(command, "ApplicationIdentifier",
...
LIBIMOBILEDEVICE_API instproxy_error_t instproxy_lookup_archives(
	instproxy_client_t client, plist_t client_options, plist_t *result)
{
	instproxy_error_t res = INSTPROXY_E_UNKNOWN_ERROR;

	plist_t command = plist_new_dict();
	plist_dict_set_item(command, "Command",
			    plist_new_string("LookupArchives"));
	if (client_options)
		plist_dict_set_item(command, "ClientOptions",
				    plist_copy(client_options));
...
LIBIMOBILEDEVICE_API instproxy_error_t instproxy_archive(
	instproxy_client_t client, const char *appid, plist_t client_options,
	instproxy_status_cb_t status_cb, void *user_data)
{
	instproxy_error_t res = INSTPROXY_E_UNKNOWN_ERROR;
...
LIBIMOBILEDEVICE_API instproxy_error_t instproxy_restore(
	instproxy_client_t client, const char *appid, plist_t client_options,
	instproxy_status_cb_t status_cb, void *user_data)
{
	instproxy_error_t res = INSTPROXY_E_UNKNOWN_ERROR;
...
LIBIMOBILEDEVICE_API instproxy_error_t instproxy_remove_archive(
	instproxy_client_t client, const char *appid, plist_t client_options,
	instproxy_status_cb_t status_cb, void *user_data)
{
	instproxy_error_t res = INSTPROXY_E_UNKNOWN_ERROR;
	...
	plist_t command = plist_new_dict();
	plist_dict_set_item(command, "Command",
			    plist_new_string("RemoveArchive"));
	if (client_options)
		plist_dict_set_item(command, "ClientOptions",
				    plist_copy(client_options));
	plist_dict_set_item(command, "ApplicationIdentifier",
...
LIBIMOBILEDEVICE_API instproxy_error_t instproxy_check_capabilities_match(
	instproxy_client_t client, const char **capabilities,
	plist_t client_options, plist_t *result)
{
	...
	plist_t command = plist_new_dict();
	plist_dict_set_item(command, "Command",
			    plist_new_string("CheckCapabilitiesMatch"));
	if (client_options)
		plist_dict_set_item(command, "ClientOptions",
				    plist_copy(client_options));
	...
	if (capabilities) {
		...
		while (capabilities[i]) {
			plist_array_append_item(
				capabilities_array,
				plist_new_string(capabilities[i]));
			i++;
		}
		plist_dict_set_item(command, "Capabilities",
				    capabilities_array);
	}
...
LIBIMOBILEDEVICE_API instproxy_error_t instproxy_status_get_error(
	plist_t status, char **name, char **description, uint64_t *code)
{
	instproxy_error_t res = INSTPROXY_E_UNKNOWN_ERROR;
...
LIBIMOBILEDEVICE_API void instproxy_status_get_percent_complete(plist_t status,
								int *percent)
{
	uint64_t val = 0;
	if (percent) {
...
instproxy_status_get_current_list(plist_t status, uint64_t *total,
				  uint64_t *current_index,
				  uint64_t *current_amount, plist_t *list)
{
	plist_t node = NULL;
...
LIBIMOBILEDEVICE_API void instproxy_command_get_name(plist_t command,
						     char **name)
{
	if (name) {
						       ...
						       ...)
{
	...
	while (arg) {
		...
		if (!strcmp(key, "SkipUninstall")) {
			...
			plist_dict_set_item(client_options, key,
					    plist_new_bool(intval));
		} else if (!strcmp(key, "ApplicationSINF") ||
			   !strcmp(key, "iTunesMetadata") ||
			   !strcmp(key, "ReturnAttributes")) {
...
LIBIMOBILEDEVICE_API void
instproxy_client_options_set_return_attributes(plist_t client_options, ...)
{
	if (!client_options)
		return;
	...
	while (arg) {
		...
		plist_array_append_item(return_attributes,
					plist_new_string(attribute));
		free(attribute);
		arg = va_arg(args, char *);
	}
	va_end(args);

	plist_dict_set_item(client_options, "ReturnAttributes",
			    return_attributes);
}
...
LIBIMOBILEDEVICE_API instproxy_error_t
instproxy_client_get_path_for_bundle_identifier(instproxy_client_t client,
						const char *appid, char **path)
{
	...
	instproxy_error_t ierr =
		instproxy_lookup(client, appids, client_opts, &apps);
