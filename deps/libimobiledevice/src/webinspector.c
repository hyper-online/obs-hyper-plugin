 ...
 */
static webinspector_error_t
webinspector_error(property_list_service_error_t err)
{
	switch (err) {
...
LIBIMOBILEDEVICE_API webinspector_error_t webinspector_client_new(
	...
	webinspector_client_t *client)
{
	*client = NULL;
	...
	webinspector_error_t ret = webinspector_error(
	...
	webinspector_client_t client_loc = (webinspector_client_t)malloc(
...
LIBIMOBILEDEVICE_API webinspector_error_t webinspector_client_start_service(
	idevice_t device, webinspector_client_t *client, const char *label)
{
	webinspector_error_t err = WEBINSPECTOR_E_UNKNOWN_ERROR;
	service_client_factory_start_service(
		device, WEBINSPECTOR_SERVICE_NAME, (void **)client, label,
		SERVICE_CONSTRUCTOR(webinspector_client_new), &err);
	return err;
}

LIBIMOBILEDEVICE_API webinspector_error_t
webinspector_client_free(webinspector_client_t client)
{
	if (!client)
		return WEBINSPECTOR_E_INVALID_ARG;

	webinspector_error_t err = webinspector_error(
...
webinspector_send(webinspector_client_t client, plist_t plist)
{
	...
	do {
		...
		if (!is_final_message) {
			...
			/* send final chunk */
			plist_dict_set_item(outplist, "WIRFinalMessageKey",
					    plist_new_data(packet + offset,
							   packet_length));
			offset += packet_length;
			packet_length -= packet_length;
		}

		res = webinspector_error(
			property_list_service_send_binary_plist(client->parent,
								outplist));
		plist_free(outplist);
		outplist = NULL;
...
LIBIMOBILEDEVICE_API webinspector_error_t webinspector_receive_with_timeout(
	webinspector_client_t client, plist_t *plist, uint32_t timeout_ms)
{
	webinspector_error_t res = WEBINSPECTOR_E_UNKNOWN_ERROR;
	...
	do {
		...
		res = webinspector_error(
			property_list_service_receive_plist_with_timeout(
				client->parent, &message, timeout_ms));
		...
		if (!key) {
			key = plist_dict_get_item(message,
