...
LIBIMOBILEDEVICE_API sbservices_error_t
sbservices_client_new(idevice_t device, lockdownd_service_descriptor_t service,
		      sbservices_client_t *client)
{
	property_list_service_client_t plistclient = NULL;
	sbservices_error_t err = sbservices_error(
	...
	sbservices_client_t client_loc = (sbservices_client_t)malloc(
...
LIBIMOBILEDEVICE_API sbservices_error_t sbservices_client_start_service(
	idevice_t device, sbservices_client_t *client, const char *label)
{
	sbservices_error_t err = SBSERVICES_E_UNKNOWN_ERROR;
	service_client_factory_start_service(
		device, SBSERVICES_SERVICE_NAME, (void **)client, label,
		SERVICE_CONSTRUCTOR(sbservices_client_new), &err);
	return err;
}

LIBIMOBILEDEVICE_API sbservices_error_t
sbservices_client_free(sbservices_client_t client)
{
	if (!client)
		return SBSERVICES_E_INVALID_ARG;
...
LIBIMOBILEDEVICE_API sbservices_error_t sbservices_get_icon_state(
	sbservices_client_t client, plist_t *state, const char *format_version)
{
	...
	if (format_version) {
		plist_dict_set_item(dict, "formatVersion",
				    plist_new_string(format_version));
	}

	sbservices_lock(client);

	res = sbservices_error(
...
LIBIMOBILEDEVICE_API sbservices_error_t
sbservices_set_icon_state(sbservices_client_t client, plist_t newstate)
{
...
LIBIMOBILEDEVICE_API sbservices_error_t
sbservices_get_icon_pngdata(sbservices_client_t client, const char *bundleId,
			    char **pngdata, uint64_t *pngsize)
{
	...
	sbservices_interface_orientation_t *interface_orientation)
{
	...
	if (res == SBSERVICES_E_SUCCESS) {
		plist_t node =
			plist_dict_get_item(dict, "interfaceOrientation");
		if (node) {
			uint64_t value =
				SBSERVICES_INTERFACE_ORIENTATION_UNKNOWN;
			plist_get_uint_val(node, &value);
			*interface_orientation =
				(sbservices_interface_orientation_t)value;
		}
	}
...
LIBIMOBILEDEVICE_API sbservices_error_t
sbservices_get_home_screen_wallpaper_pngdata(sbservices_client_t client,
					     char **pngdata, uint64_t *pngsize)
{
