...
static device_link_service_error_t
device_link_error(property_list_service_error_t err)
{
	switch (err) {
...
static int device_link_service_get_message(plist_t dl_msg, char **message)
{
	...
	/* sanity check */
	if ((plist_get_node_type(dl_msg) != PLIST_ARRAY) ||
	    (plist_array_get_size(dl_msg) < 1)) {
		return 0;
	}
...
device_link_service_client_new(idevice_t device,
			       lockdownd_service_descriptor_t service,
			       device_link_service_client_t *client)
{
	...
	device_link_service_error_t err = device_link_error(
	...
	device_link_service_client_t client_loc =
		(device_link_service_client_t)malloc(
 ...
 */
device_link_service_error_t
device_link_service_client_free(device_link_service_client_t client)
{
	if (!client)
		return DEVICE_LINK_SERVICE_E_INVALID_ARG;
 ...
 */
device_link_service_error_t
device_link_service_version_exchange(device_link_service_client_t client,
				     ...
				     uint64_t version_minor)
{
	...
	err = device_link_error(
		property_list_service_receive_plist(client->parent, &array));
	if (err != DEVICE_LINK_SERVICE_E_SUCCESS) {
	...
	if (vmajor > version_major) {
		debug_info(
			...
			vmajor, vminor, version_major, version_minor);
	...
	err = device_link_error(
		property_list_service_send_binary_plist(client->parent, array));
	if (err != DEVICE_LINK_SERVICE_E_SUCCESS) {
 ...
 */
device_link_service_error_t
device_link_service_send_ping(device_link_service_client_t client,
			      const char *message)
{
 ...
 */
device_link_service_error_t
device_link_service_send_process_message(device_link_service_client_t client,
					 plist_t message)
{
	if (!client || !client->parent || !message)
		return DEVICE_LINK_SERVICE_E_INVALID_ARG;
 ...
 */
device_link_service_error_t
device_link_service_receive_message(device_link_service_client_t client,
				    plist_t *msg_plist, char **dlmessage)
{
	...
	*msg_plist = NULL;
	device_link_service_error_t err = device_link_error(
		property_list_service_receive_plist(client->parent, msg_plist));
	if (err != DEVICE_LINK_SERVICE_E_SUCCESS) {
		return err;
	}
 ...
 */
device_link_service_error_t
device_link_service_receive_process_message(device_link_service_client_t client,
					    plist_t *message)
{
	if (!client || !client->parent || !message)
		return DEVICE_LINK_SERVICE_E_INVALID_ARG;
	...
	plist_t pmsg = NULL;
	device_link_service_error_t err = device_link_error(
		property_list_service_receive_plist(client->parent, &pmsg));
	if (err != DEVICE_LINK_SERVICE_E_SUCCESS) {
		return err;
	}
 ...
 */
device_link_service_error_t
device_link_service_send(device_link_service_client_t client, plist_t plist)
{
	if (!client || !plist) {
		return DEVICE_LINK_SERVICE_E_INVALID_ARG;
	}
	return device_link_error(
 ...
 */
device_link_service_error_t
device_link_service_receive(device_link_service_client_t client, plist_t *plist)
{
