...
LIBIMOBILEDEVICE_API debugserver_error_t
debugserver_client_new(idevice_t device, lockdownd_service_descriptor_t service,
		       debugserver_client_t *client)
{
	*client = NULL;
	...
	debugserver_error_t ret =
		debugserver_error(service_client_new(device, service, &parent));
	if (ret != DEBUGSERVER_E_SUCCESS) {
		debug_info("Creating base service client failed. Error: %i",
			   ret);
		return ret;
	}

	if (service->identifier &&
	    (strcmp(service->identifier, DEBUGSERVER_SECURE_SERVICE_NAME) !=
	     0)) {
		service_disable_bypass_ssl(parent, 1);
	}

	debugserver_client_t client_loc = (debugserver_client_t)malloc(
...
LIBIMOBILEDEVICE_API debugserver_error_t debugserver_client_start_service(
	idevice_t device, debugserver_client_t *client, const char *label)
{
	debugserver_error_t err = DEBUGSERVER_E_UNKNOWN_ERROR;
	service_client_factory_start_service(
		device, DEBUGSERVER_SECURE_SERVICE_NAME, (void **)client, label,
		SERVICE_CONSTRUCTOR(debugserver_client_new), &err);
	if (err != DEBUGSERVER_E_SUCCESS) {
		err = DEBUGSERVER_E_UNKNOWN_ERROR;
		service_client_factory_start_service(
			device, DEBUGSERVER_SERVICE_NAME, (void **)client,
			label, SERVICE_CONSTRUCTOR(debugserver_client_new),
			&err);
	}
	return err;
}
...
LIBIMOBILEDEVICE_API debugserver_error_t
debugserver_client_free(debugserver_client_t client)
{
	if (!client)
		return DEBUGSERVER_E_INVALID_ARG;

	debugserver_error_t err =
		debugserver_error(service_client_free(client->parent));
	client->parent = NULL;
	free(client);
...
LIBIMOBILEDEVICE_API debugserver_error_t
debugserver_client_send(debugserver_client_t client, const char *data,
			uint32_t size, uint32_t *sent)
{
...
LIBIMOBILEDEVICE_API debugserver_error_t
debugserver_client_receive_with_timeout(debugserver_client_t client, char *data,
					...
					unsigned int timeout)
{
	...
	res = debugserver_error(service_receive_with_timeout(
		client->parent, data, size, (uint32_t *)&bytes, timeout));
	if (bytes <= 0) {
...
LIBIMOBILEDEVICE_API debugserver_error_t
debugserver_client_receive(debugserver_client_t client, char *data,
			   uint32_t size, uint32_t *received)
{
	return debugserver_client_receive_with_timeout(client, data, size,
						       received, 1000);
}

LIBIMOBILEDEVICE_API debugserver_error_t
debugserver_command_new(const char *name, int argc, char *argv[],
			debugserver_command_t *command)
{
	int i;
	debugserver_command_t tmp = (debugserver_command_t)malloc(
...
LIBIMOBILEDEVICE_API debugserver_error_t
debugserver_command_free(debugserver_command_t command)
{
	int i;
	debugserver_error_t res = DEBUGSERVER_E_UNKNOWN_ERROR;
...
#define DEBUGSERVER_HEX_ENCODE_FIRST_BYTE(byte) \
	debugserver_int2hex((byte >> 0x4) & 0xf)
...
static uint32_t debugserver_get_checksum_for_buffer(const char *buffer,
						    uint32_t size)
{
	uint32_t checksum = 0;
	uint32_t i;
...
static int debugserver_response_is_checksum_valid(const char *response,
						  uint32_t size)
{
	uint32_t checksum = 0;
	if ((size - DEBUGSERVER_CHECKSUM_HASH_LENGTH - 1) > 0)
		checksum = debugserver_get_checksum_for_buffer(
			&response[1],
			size - DEBUGSERVER_CHECKSUM_HASH_LENGTH - 1);

	debug_info("checksum: 0x%x", checksum);

	if ((unsigned)debugserver_hex2int(response[size - 2]) !=
	    DEBUGSERVER_HEX_DECODE_FIRST_BYTE(checksum))
		return 0;

	if ((unsigned)debugserver_hex2int(response[size - 1]) !=
	    DEBUGSERVER_HEX_DECODE_SECOND_BYTE(checksum))
		return 0;
...
LIBIMOBILEDEVICE_API void debugserver_encode_string(const char *buffer,
						    char **encoded_buffer,
						    uint32_t *encoded_length)
{
	...
	*encoded_buffer = malloc(sizeof(char) * (*encoded_length));
	...
	for (position = 0, index = 0; index < length; index++) {
		...
		(*encoded_buffer)[position] =
			DEBUGSERVER_HEX_ENCODE_FIRST_BYTE(buffer[index]);
		(*encoded_buffer)[position + 1] =
			DEBUGSERVER_HEX_ENCODE_SECOND_BYTE(buffer[index]);
	}
}

LIBIMOBILEDEVICE_API void debugserver_decode_string(const char *encoded_buffer,
...
static void debugserver_format_command(const char *prefix, const char *command,
				       const char *arguments,
				       int calculate_checksum, char **buffer,
				       uint32_t *size)
{
	char checksum_hash[DEBUGSERVER_CHECKSUM_HASH_LENGTH + 1] = {'#', '0',
								    '0', '\0'};
	char *encoded = NULL;
	...
	if (calculate_checksum) {
		uint32_t checksum = debugserver_get_checksum_for_buffer(
			encoded_command, encoded_length);
	...
	*size = strlen(prefix) + strlen(encoded_command) +
		DEBUGSERVER_CHECKSUM_HASH_LENGTH;

	debug_info("formatted command: %s size: %d checksum: 0x%s", *buffer,
		   *size, checksum_hash);
...
static debugserver_error_t
debugserver_client_send_ack(debugserver_client_t client)
{
...
static debugserver_error_t
debugserver_client_send_noack(debugserver_client_t client)
{
	debug_info("sending !ACK");
	return debugserver_client_send(client, "-", sizeof(char), NULL);
}
...
LIBIMOBILEDEVICE_API debugserver_error_t
debugserver_client_set_ack_mode(debugserver_client_t client, int enabled)
{
	if (!client)
		return DEBUGSERVER_E_INVALID_ARG;
...
static int
debugserver_client_receive_internal_check(debugserver_client_t client,
					  char *received_char)
{
	...
	res = debugserver_client_receive_with_timeout(
		client, &buffer, sizeof(char), &bytes, 1000);
...
LIBIMOBILEDEVICE_API debugserver_error_t debugserver_client_receive_response(
	debugserver_client_t client, char **response, size_t *response_size)
{
	debugserver_error_t res = DEBUGSERVER_E_SUCCESS;
	...
	if (!client->noack_mode) {
		...
		should_receive =
			debugserver_client_receive_internal_check(client, ack);
	...
	debug_info("should_receive: %d, skip_prefix: %d", should_receive,
		   skip_prefix);

	if (should_receive && !skip_prefix) {
		debug_info("attempting to receive prefix");
		should_receive = debugserver_client_receive_internal_check(
	...
	debug_info("buffer: %*s, should_receive: %d, skip_prefix: %d",
		   buffer_size, buffer, should_receive, skip_prefix);
	...
	if (should_receive) {
		...
		while ((checksum_length > 0)) {
			char data[2] = {'#', '\0'};
			if (debugserver_client_receive_internal_check(client,
								      data)) {
			...
			if (buffer_size + 1 >= buffer_capacity) {
				char *newbuffer =
					realloc(buffer, buffer_capacity + 1024);
				if (!newbuffer) {
					return DEBUGSERVER_E_UNKNOWN_ERROR;
				}
				buffer = newbuffer;
		...
		if (client->noack_mode ||
		    debugserver_response_is_checksum_valid(buffer,
							   buffer_size)) {
			if (response) {
				...
				uint32_t resp_size =
					sizeof(char) *
					(buffer_size -
					 DEBUGSERVER_CHECKSUM_HASH_LENGTH - 1);
...
LIBIMOBILEDEVICE_API debugserver_error_t debugserver_client_send_command(
	debugserver_client_t client, debugserver_command_t command,
	char **response, size_t *response_size)
{
	...
	for (i = 0; i < command->argc; i++) {
		...
		command_arguments = string_append(command_arguments,
						  command->argv[i], NULL);
	}

	debug_info("command_arguments(%d): %s", command->argc,
		   command_arguments);

	/* encode command arguments, add checksum if required and assemble entire command */
	debugserver_format_command("$", command->name, command_arguments, 1,
				   &send_buffer, &send_buffer_size);

	debug_info("sending encoded command: %s", send_buffer);

	res = debugserver_client_send(client, send_buffer, send_buffer_size,
				      &bytes);
...
LIBIMOBILEDEVICE_API debugserver_error_t
debugserver_client_set_environment_hex_encoded(debugserver_client_t client,
					       const char *env, char **response)
{
	if (!client || !env)
		return DEBUGSERVER_E_INVALID_ARG;
	...
	debugserver_command_new("QEnvironmentHexEncoded:", 1, env_arg,
				&command);
...
LIBIMOBILEDEVICE_API debugserver_error_t
debugserver_client_set_argv(debugserver_client_t client, int argc,
			    const char *argv[], char **response)
{
	if (!client || !argc)
		return DEBUGSERVER_E_INVALID_ARG;
