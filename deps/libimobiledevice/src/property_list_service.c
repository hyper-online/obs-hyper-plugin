...
service_to_property_list_service_error(service_error_t err)
{
	switch (err) {
...
LIBIMOBILEDEVICE_API property_list_service_error_t
property_list_service_client_new(idevice_t device,
				 ...
				 property_list_service_client_t *client)
{
	...
	property_list_service_client_t client_loc =
		(property_list_service_client_t)malloc(
...
LIBIMOBILEDEVICE_API property_list_service_error_t
property_list_service_client_free(property_list_service_client_t client)
{
	if (!client)
		return PROPERTY_LIST_SERVICE_E_INVALID_ARG;
 ...
 */
static property_list_service_error_t
internal_plist_send(property_list_service_client_t client, plist_t plist,
		    int binary)
{
	property_list_service_error_t res =
...
LIBIMOBILEDEVICE_API property_list_service_error_t
property_list_service_send_xml_plist(property_list_service_client_t client,
				     plist_t plist)
{
...
LIBIMOBILEDEVICE_API property_list_service_error_t
property_list_service_send_binary_plist(property_list_service_client_t client,
					plist_t plist)
{
 ...
 */
static property_list_service_error_t
internal_plist_receive_timeout(property_list_service_client_t client,
			       plist_t *plist, unsigned int timeout)
{
	property_list_service_error_t res =
		PROPERTY_LIST_SERVICE_E_UNKNOWN_ERROR;
	uint32_t pktlen = 0;
	...
	*plist = NULL;
	service_error_t serr =
		service_receive_with_timeout(client->parent, (char *)&pktlen,
					     sizeof(pktlen), &bytes, timeout);
	...
	while (curlen < pktlen) {
		serr = service_receive(client->parent, content + curlen,
				       pktlen - curlen, &bytes);
		if (serr != SERVICE_E_SUCCESS) {
	...
	if ((pktlen > 8) && !memcmp(content, "bplist00", 8)) {
		...
		for (bytes = 0; bytes < pktlen - 1; bytes++) {
			if ((content[bytes] >= 0) && (content[bytes] < 0x20) &&
			    (content[bytes] != 0x09) &&
			    (content[bytes] != 0x0a) &&
			    (content[bytes] != 0x0d))
				content[bytes] = 0x20;
...
property_list_service_receive_plist_with_timeout(
	property_list_service_client_t client, plist_t *plist,
...
LIBIMOBILEDEVICE_API property_list_service_error_t
property_list_service_receive_plist(property_list_service_client_t client,
				    plist_t *plist)
{
...
LIBIMOBILEDEVICE_API property_list_service_error_t
property_list_service_enable_ssl(property_list_service_client_t client)
{
	if (!client || !client->parent)
		return PROPERTY_LIST_SERVICE_E_INVALID_ARG;
	return service_to_property_list_service_error(
		service_enable_ssl(client->parent));
}

LIBIMOBILEDEVICE_API property_list_service_error_t
property_list_service_disable_ssl(property_list_service_client_t client)
{
	if (!client || !client->parent)
		return PROPERTY_LIST_SERVICE_E_INVALID_ARG;
	return service_to_property_list_service_error(
