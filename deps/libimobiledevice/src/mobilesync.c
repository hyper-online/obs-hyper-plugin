...
LIBIMOBILEDEVICE_API mobilesync_error_t
mobilesync_client_new(idevice_t device, lockdownd_service_descriptor_t service,
		      mobilesync_client_t *client)
{
	...
	mobilesync_error_t ret = mobilesync_error(
		device_link_service_client_new(device, service, &dlclient));
	if (ret != MOBILESYNC_E_SUCCESS) {
		return ret;
	}

	mobilesync_client_t client_loc = (mobilesync_client_t)malloc(
	...
	/* perform handshake */
	ret = mobilesync_error(device_link_service_version_exchange(
		dlclient, MSYNC_VERSION_INT1, MSYNC_VERSION_INT2));
	if (ret != MOBILESYNC_E_SUCCESS) {
...
LIBIMOBILEDEVICE_API mobilesync_error_t mobilesync_client_start_service(
	idevice_t device, mobilesync_client_t *client, const char *label)
{
	mobilesync_error_t err = MOBILESYNC_E_UNKNOWN_ERROR;
	service_client_factory_start_service(
		device, MOBILESYNC_SERVICE_NAME, (void **)client, label,
		SERVICE_CONSTRUCTOR(mobilesync_client_new), &err);
	return err;
}

LIBIMOBILEDEVICE_API mobilesync_error_t
mobilesync_client_free(mobilesync_client_t client)
{
	if (!client)
		return MOBILESYNC_E_INVALID_ARG;
	device_link_service_disconnect(client->parent,
				       "All done, thanks for the memories");
	mobilesync_error_t err = mobilesync_error(
...
LIBIMOBILEDEVICE_API mobilesync_error_t
mobilesync_receive(mobilesync_client_t client, plist_t *plist)
{
	if (!client)
		return MOBILESYNC_E_INVALID_ARG;
	mobilesync_error_t ret = mobilesync_error(
...
LIBIMOBILEDEVICE_API mobilesync_error_t
mobilesync_send(mobilesync_client_t client, plist_t plist)
{
	if (!client || !plist)
		return MOBILESYNC_E_INVALID_ARG;
	return mobilesync_error(
		device_link_service_send(client->parent, plist));
}

LIBIMOBILEDEVICE_API mobilesync_error_t mobilesync_start(
	mobilesync_client_t client, const char *data_class,
	mobilesync_anchors_t anchors, uint64_t computer_data_class_version,
	mobilesync_sync_type_t *sync_type, uint64_t *device_data_class_version,
	char **error_description)
{
	if (!client || client->data_class || !data_class || !anchors ||
	...
	*error_description = NULL;
	...
	plist_array_append_item(
		msg, plist_new_string("SDMessageSyncDataClassWithDevice"));
	plist_array_append_item(msg, plist_new_string(data_class));
	if (anchors->device_anchor) {
		plist_array_append_item(
			msg, plist_new_string(anchors->device_anchor));
	} else {
	...
	if (!strcmp(response_type,
		...
		plist_get_string_val(plist_array_get_item(msg, 2),
				     error_description);
	...
	if (device_data_class_version != NULL) {
		plist_t device_data_class_version_node =
			plist_array_get_item(msg, 5);
		if (!device_data_class_version_node) {
			err = MOBILESYNC_E_PLIST_ERROR;
			goto out;
		}

		plist_get_uint_val(device_data_class_version_node,
...
mobilesync_finish(mobilesync_client_t client)
{
	...
	msg = plist_new_array();
	plist_array_append_item(
		msg, plist_new_string("SDMessageFinishSessionOnDevice"));
...
static mobilesync_error_t mobilesync_get_records(mobilesync_client_t client,
...
{
	return mobilesync_get_records(client,
...
LIBIMOBILEDEVICE_API mobilesync_error_t
mobilesync_receive_changes(mobilesync_client_t client, plist_t *entities,
			   uint8_t *is_last_record, plist_t *actions)
{
	if (!client || !client->data_class) {
		return MOBILESYNC_E_INVALID_ARG;
	}
	...
	if (is_last_record != NULL) {
		plist_get_bool_val(plist_array_get_item(msg, 3),
				   &has_more_changes);
...
mobilesync_clear_all_records_on_device(mobilesync_client_t client)
{
	...
	msg = plist_new_array();
	plist_array_append_item(
		msg, plist_new_string("SDMessageClearAllRecordsOnDevice"));
	plist_array_append_item(msg, plist_new_string(client->data_class));
	plist_array_append_item(msg, plist_new_string(EMPTY_PARAMETER_STRING));
...
mobilesync_acknowledge_changes_from_device(mobilesync_client_t client)
{
	...
	msg = plist_new_array();
	plist_array_append_item(
		msg, plist_new_string("SDMessageAcknowledgeChangesFromDevice"));
	plist_array_append_item(msg, plist_new_string(client->data_class));
...
static plist_t create_process_changes_message(const char *data_class,
					      plist_t entities,
					      uint8_t more_changes,
					      plist_t actions)
{
	...
	if (actions)
		...
		plist_array_append_item(
			msg, plist_new_string(EMPTY_PARAMETER_STRING));
...
mobilesync_ready_to_send_changes_from_computer(mobilesync_client_t client)
{
	...
	if (strcmp(response_type, "SDMessageDeviceReadyToReceiveChanges") !=
	    0) {
		err = MOBILESYNC_E_NOT_READY;
		goto out;
	}

	err = mobilesync_error(device_link_service_send_ping(
		client->parent, "Preparing to get changes for device"));
	if (err != MOBILESYNC_E_SUCCESS) {
		goto out;
	}
...
LIBIMOBILEDEVICE_API mobilesync_error_t
mobilesync_send_changes(mobilesync_client_t client, plist_t entities,
			uint8_t is_last_record, plist_t actions)
{
	...
	msg = create_process_changes_message(client->data_class, entities,
...
LIBIMOBILEDEVICE_API mobilesync_error_t
mobilesync_cancel(mobilesync_client_t client, const char *reason)
{
...
LIBIMOBILEDEVICE_API mobilesync_error_t
mobilesync_anchors_new(const char *device_anchor, const char *computer_anchor,
		       mobilesync_anchors_t *anchor)
{
	mobilesync_anchors_t anchors =
		(mobilesync_anchors_t)malloc(sizeof(mobilesync_anchors));
...
LIBIMOBILEDEVICE_API void mobilesync_actions_add(plist_t actions, ...)
{
	...
	while (arg) {
		...
		if (!strcmp(key, "SyncDeviceLinkEntityNamesKey")) {
			...
			plist_dict_set_item(actions, key,
