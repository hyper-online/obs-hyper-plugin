...
LIBIMOBILEDEVICE_API restored_error_t
...
LIBIMOBILEDEVICE_API void restored_client_set_label(restored_client_t client,
						    const char *label)
{
	if (client) {
		if (client->label)
...
LIBIMOBILEDEVICE_API restored_error_t restored_receive(restored_client_t client,
						       plist_t *plist)
{
...
LIBIMOBILEDEVICE_API restored_error_t restored_send(restored_client_t client,
						    plist_t plist)
{
	if (!client || !plist)
		return RESTORE_E_INVALID_ARG;
...
restored_query_type(restored_client_t client, char **type, uint64_t *version)
{
	...
	if (type_node && (plist_get_node_type(type_node) == PLIST_STRING)) {
		...
		if (version) {
			plist_t version_node = plist_dict_get_item(
				dict, "RestoreProtocolVersion");
			if (version_node &&
			    PLIST_UINT == plist_get_node_type(version_node)) {
				plist_get_uint_val(version_node, version);
				debug_info("restored protocol version %llu",
					   *version);
			} else {
				return RESTORE_E_UNKNOWN_ERROR;
			}
...
LIBIMOBILEDEVICE_API restored_error_t
restored_query_value(restored_client_t client, const char *key, plist_t *value)
{
	if (!client || !key)
		return RESTORE_E_INVALID_ARG;
...
LIBIMOBILEDEVICE_API restored_error_t
restored_get_value(restored_client_t client, const char *key, plist_t *value)
{
...
LIBIMOBILEDEVICE_API restored_error_t restored_client_new(
	idevice_t device, restored_client_t *client, const char *label)
{
	if (!client)
		return RESTORE_E_INVALID_ARG;
	...
	ret = restored_error(property_list_service_client_new(
		device, (lockdownd_service_descriptor_t)&service,
		&plistclient));
	if (ret != RESTORE_E_SUCCESS) {
		debug_info("could not connect to restored (device %s)",
			   device->udid);
		return ret;
	}

	restored_client_t client_loc = (restored_client_t)malloc(
...
LIBIMOBILEDEVICE_API restored_error_t restored_start_restore(
	restored_client_t client, plist_t options, uint64_t version)
{
	if (!client)
		return RESTORE_E_INVALID_ARG;
	...
	if (options) {
		plist_dict_set_item(dict, "RestoreOptions",
				    plist_copy(options));
	}
	plist_dict_set_item(dict, "RestoreProtocolVersion",
