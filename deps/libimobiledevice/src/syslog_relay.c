...
LIBIMOBILEDEVICE_API syslog_relay_error_t syslog_relay_client_new(
	...
	syslog_relay_client_t *client)
{
	*client = NULL;
	...
	syslog_relay_error_t ret = syslog_relay_error(
	...
	syslog_relay_client_t client_loc = (syslog_relay_client_t)malloc(
...
LIBIMOBILEDEVICE_API syslog_relay_error_t syslog_relay_client_start_service(
	idevice_t device, syslog_relay_client_t *client, const char *label)
{
	syslog_relay_error_t err = SYSLOG_RELAY_E_UNKNOWN_ERROR;
	service_client_factory_start_service(
		device, SYSLOG_RELAY_SERVICE_NAME, (void **)client, label,
		SERVICE_CONSTRUCTOR(syslog_relay_client_new), &err);
	return err;
}

LIBIMOBILEDEVICE_API syslog_relay_error_t
syslog_relay_client_free(syslog_relay_client_t client)
{
	if (!client)
		return SYSLOG_RELAY_E_INVALID_ARG;
	syslog_relay_stop_capture(client);
	syslog_relay_error_t err =
		syslog_relay_error(service_client_free(client->parent));
	free(client);
...
LIBIMOBILEDEVICE_API syslog_relay_error_t
syslog_relay_receive(syslog_relay_client_t client, char *data, uint32_t size,
		     uint32_t *received)
{
	return syslog_relay_receive_with_timeout(client, data, size, received,
						 1000);
}

LIBIMOBILEDEVICE_API syslog_relay_error_t syslog_relay_receive_with_timeout(
	syslog_relay_client_t client, char *data, uint32_t size,
	uint32_t *received, unsigned int timeout)
{
	...
	res = syslog_relay_error(service_receive_with_timeout(
		client->parent, data, size, (uint32_t *)&bytes, timeout));
	if (res != SYSLOG_RELAY_E_SUCCESS && res != SYSLOG_RELAY_E_TIMEOUT &&
	    res != SYSLOG_RELAY_E_NOT_ENOUGH_DATA) {
...
void *syslog_relay_worker(void *arg)
{
	...
	struct syslog_relay_worker_thread *srwt =
		(struct syslog_relay_worker_thread *)arg;
	...
	while (srwt->client->parent) {
		char c;
		uint32_t bytes = 0;
		ret = syslog_relay_receive_with_timeout(srwt->client, &c, 1,
							&bytes, 100);
		if (ret == SYSLOG_RELAY_E_TIMEOUT ||
		    ret == SYSLOG_RELAY_E_NOT_ENOUGH_DATA ||
		    ((bytes == 0) && (ret == SYSLOG_RELAY_E_SUCCESS))) {
			continue;
		} else if (ret < 0) {
...
LIBIMOBILEDEVICE_API syslog_relay_error_t
syslog_relay_start_capture(syslog_relay_client_t client,
			   syslog_relay_receive_cb_t callback, void *user_data)
{
	if (!client || !callback)
		return SYSLOG_RELAY_E_INVALID_ARG;
	...
	/* start worker thread */
	struct syslog_relay_worker_thread *srwt =
		(struct syslog_relay_worker_thread *)malloc(
	...
	if (srwt) {
		...
		if (thread_new(&client->worker, syslog_relay_worker, srwt) ==
		    0) {
...
LIBIMOBILEDEVICE_API syslog_relay_error_t syslog_relay_start_capture_raw(
	syslog_relay_client_t client, syslog_relay_receive_cb_t callback,
	void *user_data)
{
	if (!client || !callback)
		return SYSLOG_RELAY_E_INVALID_ARG;
...
LIBIMOBILEDEVICE_API syslog_relay_error_t
syslog_relay_stop_capture(syslog_relay_client_t client)
{
	if (client->worker) {
