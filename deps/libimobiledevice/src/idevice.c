...
#define ETIMEDOUT 138
	...
	(defined(LIBRESSL_VERSION_NUMBER) && \
	 (LIBRESSL_VERSION_NUMBER < 0x20020000L))
...
static void usbmux_event_cb(const usbmuxd_event_t *event, void *user_data)
{
	...
	if (event->device.conn_type == CONNECTION_TYPE_USB) {
		...
		debug_info("Unknown connection type %d",
			   event->device.conn_type);
	}
...
idevice_event_subscribe(idevice_event_cb_t callback, void *user_data)
{
	event_cb = callback;
...
LIBIMOBILEDEVICE_API idevice_error_t
idevice_get_device_list_extended(idevice_info_t **devices, int *count)
{
	...
	*count = 0;
	...
	for (i = 0; dev_list[i].handle > 0; i++) {
		newlist = realloc(*devices,
				  sizeof(idevice_info_t) * (newcount + 1));
...
LIBIMOBILEDEVICE_API idevice_error_t
idevice_device_list_extended_free(idevice_info_t *devices)
{
	if (devices) {
		int i = 0;
		while (devices[i]) {
...
LIBIMOBILEDEVICE_API idevice_error_t idevice_get_device_list(char ***devices,
							     int *count)
{
	usbmuxd_device_info_t *dev_list;
...
LIBIMOBILEDEVICE_API void
idevice_set_debug_callback(idevice_debug_cb_t callback)
{
...
LIBIMOBILEDEVICE_API idevice_error_t idevice_new_with_options(
	idevice_t *device, const char *udid, enum idevice_options options)
{
...
LIBIMOBILEDEVICE_API idevice_error_t idevice_new(idevice_t *device,
						 const char *udid)
{
...
LIBIMOBILEDEVICE_API idevice_error_t idevice_connect(
	idevice_t device, uint16_t port, idevice_connection_t *connection)
{
	if (!device) {
		return IDEVICE_E_INVALID_ARG;
	}
...
LIBIMOBILEDEVICE_API idevice_error_t
idevice_disconnect(idevice_connection_t connection)
{
	if (!connection) {
		return IDEVICE_E_INVALID_ARG;
	}
	...
	if (connection->type == CONNECTION_USBMUXD ||
	    connection->type == CONNECTION_NETWORK) {
 ...
 */
static idevice_error_t internal_connection_send(idevice_connection_t connection,
						...
						uint32_t *sent_bytes)
{
	...
	if (connection->type == CONNECTION_USBMUXD ||
		...
		int res;
		do {
			res = usbmuxd_send((int)(long)connection->data, data,
					   len, sent_bytes);
		} while (res == -EAGAIN);
		if (res < 0) {
			debug_info("ERROR: usbmuxd_send returned %d (%s)", res,
				   strerror(-res));
			return IDEVICE_E_UNKNOWN_ERROR;
		}
		return IDEVICE_E_SUCCESS;
...
LIBIMOBILEDEVICE_API idevice_error_t
idevice_connection_send(idevice_connection_t connection, const char *data,
			uint32_t len, uint32_t *sent_bytes)
{
	...
	if (connection->ssl_data) {
		...
		while (sent < len) {
#ifdef HAVE_OPENSSL
			int c = socket_check_fd((int)(long)connection->data,
						FDM_WRITE, 100);
			...
			int s = SSL_write(connection->ssl_data->session,
					  (const void *)(data + sent),
					  (int)(len - sent));
			if (s <= 0) {
				int sslerr = SSL_get_error(
					connection->ssl_data->session, s);
...
#else
			ssize_t s = gnutls_record_send(
				connection->ssl_data->session,
				(void *)(data + sent), (size_t)(len - sent));
#endif
			if (s < 0) {
				break;
			}
		...
		*sent_bytes = sent;
		...
		while (sent < len) {
			uint32_t bytes = 0;
			int s = internal_connection_send(
				connection, data + sent, len - sent, &bytes);
			if (s < 0) {
				break;
			}
			sent += bytes;
...
static idevice_error_t
socket_recv_to_idevice_error(int conn_error, uint32_t len, uint32_t received)
{
	if (conn_error < 0) {
		switch (conn_error) {
		case -EAGAIN:
			debug_info("ERROR: received partial data %d/%d (%s)",
 ...
 */
static idevice_error_t
internal_connection_receive_timeout(idevice_connection_t connection, char *data,
				    ...
				    unsigned int timeout)
{
	...
	if (connection->type == CONNECTION_USBMUXD ||
	    connection->type == CONNECTION_NETWORK) {
		int conn_error =
			usbmuxd_recv_timeout((int)(long)connection->data, data,
					     len, recv_bytes, timeout);
		idevice_error_t error = socket_recv_to_idevice_error(
			conn_error, len, *recv_bytes);

		if (error == IDEVICE_E_UNKNOWN_ERROR) {
			debug_info(
				"ERROR: usbmuxd_recv_timeout returned %d (%s)",
				conn_error, strerror(-conn_error));
		}
...
LIBIMOBILEDEVICE_API idevice_error_t idevice_connection_receive_timeout(
	idevice_connection_t connection, char *data, uint32_t len,
	uint32_t *recv_bytes, unsigned int timeout)
{
...
#ifdef HAVE_OPENSSL
			do_select =
				(SSL_pending(connection->ssl_data->session) ==
				 0);
#endif
			if (do_select) {
				int conn_error = socket_check_fd(
					(int)(long)connection->data, FDM_READ,
					timeout);
				idevice_error_t error =
					socket_recv_to_idevice_error(
						conn_error, len, received);
				...
				default:
					debug_info(
						"ERROR: socket_check_fd returned %d (%s)",
						conn_error,
						strerror(-conn_error));
					return error;
				}
	...
	return internal_connection_receive_timeout(connection, data, len,
						   recv_bytes, timeout);
}
 ...
 */
static idevice_error_t
internal_connection_receive(idevice_connection_t connection, char *data,
			    uint32_t len, uint32_t *recv_bytes)
{
	if (!connection) {
		return IDEVICE_E_INVALID_ARG;
	...
	if (connection->type == CONNECTION_USBMUXD ||
	    connection->type == CONNECTION_NETWORK) {
		int res = usbmuxd_recv((int)(long)connection->data, data, len,
				       recv_bytes);
		if (res < 0) {
			debug_info("ERROR: usbmuxd_recv returned %d (%s)", res,
				   strerror(-res));
			return IDEVICE_E_UNKNOWN_ERROR;
		}
...
LIBIMOBILEDEVICE_API idevice_error_t
idevice_connection_receive(idevice_connection_t connection, char *data,
			   uint32_t len, uint32_t *recv_bytes)
{
	if (!connection ||
	    (connection->ssl_data && !connection->ssl_data->session)) {
	...
	if (connection->ssl_data) {
#ifdef HAVE_OPENSSL
		int received = SSL_read(connection->ssl_data->session,
					(void *)data, (int)len);
		debug_info("SSL_read %d, received %d", len, received);
#else
		ssize_t received =
			gnutls_record_recv(connection->ssl_data->session,
...
LIBIMOBILEDEVICE_API idevice_error_t idevice_get_udid(idevice_t device,
						      char **udid)
{
	if (!device || !udid)
		return IDEVICE_E_INVALID_ARG;
 ...
 */
static ssize_t internal_ssl_read(gnutls_transport_ptr_t transport, char *buffer,
				 size_t length)
{
	...
	do {
		if ((res = internal_connection_receive(
			     connection, recv_buffer, this_len,
			     (uint32_t *)&bytes)) != IDEVICE_E_SUCCESS) {
			debug_info(
 ...
 */
static ssize_t internal_ssl_write(gnutls_transport_ptr_t transport,
				  char *buffer, size_t length)
{
	...
	if ((res = internal_connection_send(connection, buffer, length,
					    &bytes)) != IDEVICE_E_SUCCESS) {
...
static int internal_cert_callback(gnutls_session_t session,
...
idevice_connection_enable_ssl(idevice_connection_t connection)
{
	...
	pair_record_import_crt_with_name(
		pair_record, USERPREF_ROOT_CERTIFICATE_KEY, &root_cert);
	pair_record_import_key_with_name(
		pair_record, USERPREF_ROOT_PRIVATE_KEY_KEY, &root_privkey);
	...
	if (ssl_error != 0) {
		debug_info("ERROR during SSL handshake: %s",
			   ssl_error_to_string(ssl_error));
		SSL_free(ssl);
		SSL_CTX_free(ssl_ctx);
	} else {
		ssl_data_t ssl_data_loc =
			(ssl_data_t)malloc(sizeof(struct ssl_data_private));
		ssl_data_loc->session = ssl;
		ssl_data_loc->ctx = ssl_ctx;
		connection->ssl_data = ssl_data_loc;
		ret = IDEVICE_E_SUCCESS;
		debug_info("SSL mode enabled, %s, cipher: %s",
	...
	gnutls_certificate_client_set_retrieve_function(
		ssl_data_loc->certificate, internal_cert_callback);
#endif
	...
	gnutls_transport_set_ptr(ssl_data_loc->session,
				 (gnutls_transport_ptr_t)connection);
	debug_info("GnuTLS step 2...");
	gnutls_transport_set_push_function(
		ssl_data_loc->session, (gnutls_push_func)&internal_ssl_write);
	debug_info("GnuTLS step 3...");
	gnutls_transport_set_pull_function(
		ssl_data_loc->session, (gnutls_pull_func)&internal_ssl_read);
	...
	} while (return_me == GNUTLS_E_AGAIN ||
		 return_me == GNUTLS_E_INTERRUPTED);
...
LIBIMOBILEDEVICE_API idevice_error_t
idevice_connection_disable_ssl(idevice_connection_t connection)
{
	return idevice_connection_disable_bypass_ssl(connection, 0);
}

LIBIMOBILEDEVICE_API idevice_error_t idevice_connection_disable_bypass_ssl(
	idevice_connection_t connection, uint8_t sslBypass)
{
	if (!connection)
		return IDEVICE_E_INVALID_ARG;
	if (!connection->ssl_data) {
		...
		if (connection->ssl_data->session) {
			...
			if (SSL_shutdown(connection->ssl_data->session) == 0) {
				...
				if ((ssl_error = SSL_get_error(
					...
					debug_info(
						"Skipping bidirectional SSL shutdown. SSL error code: %i\n",
						ssl_error);
				}
			}
		}
#else
		if (connection->ssl_data->session) {
			gnutls_bye(connection->ssl_data->session,
				   GNUTLS_SHUT_RDWR);
		}
#endif
	}
...
LIBIMOBILEDEVICE_API idevice_error_t
idevice_set_socket_type(enum idevice_socket_type value)
{
...
LIBIMOBILEDEVICE_API idevice_error_t
idevice_get_socket_type(enum idevice_socket_type *value)
{
...
LIBIMOBILEDEVICE_API idevice_error_t idevice_set_tcp_endpoint(const char *host,
							      uint16_t port)
{
...
LIBIMOBILEDEVICE_API idevice_error_t idevice_get_tcp_endpoint(char **host,
							      uint16_t *port)
{
