...
LIBIMOBILEDEVICE_API mobileactivation_error_t mobileactivation_client_new(
	...
	mobileactivation_client_t *client)
{
	...
	mobileactivation_client_t client_loc =
		(mobileactivation_client_t)malloc(
			sizeof(struct mobileactivation_client_private));
				      ...
				      const char *label)
{
	...
	service_client_factory_start_service(
		device, MOBILEACTIVATION_SERVICE_NAME, (void **)client, label,
...
static mobileactivation_error_t
mobileactivation_check_result(plist_t dict, const char *command)
{
...
static mobileactivation_error_t
mobileactivation_send_command_plist(mobileactivation_client_t client,
				    plist_t command, plist_t *result)
{
	if (!client || !command)
		return MOBILEACTIVATION_E_INVALID_ARG;
	...
	*result = NULL;
	...
	ret = mobileactivation_error(property_list_service_send_binary_plist(
		client->parent, command));

	plist_t dict = NULL;
	ret = mobileactivation_error(
...
static mobileactivation_error_t
mobileactivation_send_command(mobileactivation_client_t client,
			      const char *command, plist_t value,
			      plist_t *result)
{
				      ...
				      plist_t *state)
{
	...
	mobileactivation_error_t ret = mobileactivation_send_command(
		client, "GetActivationStateRequest", NULL, &result);
	if (ret == MOBILEACTIVATION_E_SUCCESS) {
		plist_t node = plist_dict_get_item(result, "Value");
		if (!node) {
			debug_info(
				"ERROR: GetActivationStateRequest command returned success but has no value in reply");
...
mobileactivation_create_activation_session_info(
	mobileactivation_client_t client, plist_t *blob)
{
	if (!client || !blob)
		return MOBILEACTIVATION_E_INVALID_ARG;
	...
	plist_t result = NULL;
	mobileactivation_error_t ret = mobileactivation_send_command(
		client, "CreateTunnel1SessionInfoRequest", NULL, &result);
	if (ret == MOBILEACTIVATION_E_SUCCESS) {
		plist_t node = plist_dict_get_item(result, "Value");
		if (!node) {
			debug_info(
				"ERROR: CreateTunnel1SessionInfoRequest command returned success but has no value in reply");
			ret = MOBILEACTIVATION_E_UNKNOWN_ERROR;
		} else {
					...
					plist_t *info)
{
	...
	plist_t result = NULL;
	mobileactivation_error_t ret = mobileactivation_send_command(
		client, "CreateActivationInfoRequest", NULL, &result);
	if (ret == MOBILEACTIVATION_E_SUCCESS) {
		plist_t node = plist_dict_get_item(result, "Value");
		if (!node) {
			debug_info(
				"ERROR: CreateActivationInfoRequest command returned success but has no value in reply");
			ret = MOBILEACTIVATION_E_UNKNOWN_ERROR;
		} else {
...
mobileactivation_create_activation_info_with_session(
	mobileactivation_client_t client, plist_t handshake_response,
	plist_t *info)
{
	if (!client || !info)
		return MOBILEACTIVATION_E_INVALID_ARG;
	...
	mobileactivation_error_t ret = mobileactivation_send_command(
		client, "CreateTunnel1ActivationInfoRequest", data, &result);
	plist_free(data);
	if (ret == MOBILEACTIVATION_E_SUCCESS) {
		plist_t node = plist_dict_get_item(result, "Value");
		if (!node) {
			debug_info(
				"ERROR: CreateTunnel1ActivationInfoRequest command returned success but has no value in reply");
			ret = MOBILEACTIVATION_E_UNKNOWN_ERROR;
		} else {
			*info = plist_copy(node);
		}
...
LIBIMOBILEDEVICE_API mobileactivation_error_t mobileactivation_activate(
	mobileactivation_client_t client, plist_t activation_record)
{
	...
	plist_t result = NULL;
	mobileactivation_error_t ret = mobileactivation_send_command(
		client, "HandleActivationInfoRequest", activation_record,
		&result);
	plist_free(result);
	result = NULL;
...
mobileactivation_deactivate(mobileactivation_client_t client)
{
	...
	plist_t result = NULL;
	mobileactivation_error_t ret = mobileactivation_send_command(
		client, "DeactivateRequest", NULL, &result);
	plist_free(result);
	result = NULL;
