...
LIBIMOBILEDEVICE_API np_error_t
np_client_new(idevice_t device, lockdownd_service_descriptor_t service,
	      np_client_t *client)
{
	property_list_service_client_t plistclient = NULL;
	np_error_t err = np_error(property_list_service_client_new(
		device, service, &plistclient));
	if (err != NP_E_SUCCESS) {
		return err;
	}

	np_client_t client_loc =
		(np_client_t)malloc(sizeof(struct np_client_private));
...
LIBIMOBILEDEVICE_API np_error_t np_client_start_service(idevice_t device,
							np_client_t *client,
							const char *label)
{
	np_error_t err = NP_E_UNKNOWN_ERROR;
	service_client_factory_start_service(device, NP_SERVICE_NAME,
					     (void **)client, label,
...
LIBIMOBILEDEVICE_API np_error_t np_client_free(np_client_t client)
{
	...
	if (client->notifier) {
		...
		if (dict) {
			...
			char *cmd_value = NULL;
			plist_t cmd_value_node =
				plist_dict_get_item(dict, "Command");
			if (plist_get_node_type(cmd_value_node) ==
			    PLIST_STRING) {
				plist_get_string_val(cmd_value_node,
						     &cmd_value);
			}
...
LIBIMOBILEDEVICE_API np_error_t np_post_notification(np_client_t client,
...
static np_error_t internal_np_observe_notification(np_client_t client,
						   const char *notification)
{
	plist_t dict = plist_new_dict();
	plist_dict_set_item(dict, "Command",
...
LIBIMOBILEDEVICE_API np_error_t
np_observe_notification(np_client_t client, const char *notification)
{
	if (!client || !notification) {
		return NP_E_INVALID_ARG;
	}
...
LIBIMOBILEDEVICE_API np_error_t
np_observe_notifications(np_client_t client, const char **notification_spec)
{
	int i = 0;
	...
	while (notifications[i]) {
		res = internal_np_observe_notification(client,
						       notifications[i]);
		if (res != NP_E_SUCCESS) {
			break;
		}
...
static int np_get_notification(np_client_t client, char **notification)
{
	...
	property_list_service_error_t perr =
		property_list_service_receive_plist_with_timeout(client->parent,
								 &dict, 500);
	...
	if (dict) {
		...
		if (cmd_value && !strcmp(cmd_value, "RelayNotification")) {
			...
			plist_t name_value_node =
				plist_dict_get_item(dict, "Name");

			if (plist_get_node_type(name_value_node) ==
			    PLIST_STRING) {
				plist_get_string_val(name_value_node,
						     &name_value);
			}
			...
			if (name_value_node && name_value) {
				*notification = name_value;
				debug_info("got notification %s", __func__,
					   name_value);
				res = 0;
			}
...
LIBIMOBILEDEVICE_API np_error_t np_set_notify_callback(np_client_t client,
						       ...
						       void *user_data)
{
	...
	if (notify_cb) {
		struct np_thread *npt =
			(struct np_thread *)malloc(sizeof(struct np_thread));
		if (npt) {
			npt->client = client;
			npt->cbfunc = notify_cb;
			npt->user_data = user_data;

			if (thread_new(&client->notifier, np_notifier, npt) ==
			    0) {
				res = NP_E_SUCCESS;
			}
		}
