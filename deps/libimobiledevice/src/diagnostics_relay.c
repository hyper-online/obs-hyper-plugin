...
LIBIMOBILEDEVICE_API diagnostics_relay_error_t diagnostics_relay_client_new(
	...
	diagnostics_relay_client_t *client)
{
	...
	diagnostics_relay_client_t client_loc =
		(diagnostics_relay_client_t)malloc(
...
LIBIMOBILEDEVICE_API diagnostics_relay_error_t
diagnostics_relay_client_start_service(idevice_t device,
				       diagnostics_relay_client_t *client,
				       const char *label)
{
	diagnostics_relay_error_t err = DIAGNOSTICS_RELAY_E_UNKNOWN_ERROR;
	service_client_factory_start_service(
		device, DIAGNOSTICS_RELAY_SERVICE_NAME, (void **)client, label,
		SERVICE_CONSTRUCTOR(diagnostics_relay_client_new), &err);
	return err;
}
 ...
 */
static diagnostics_relay_error_t
diagnostics_relay_receive(diagnostics_relay_client_t client, plist_t *plist)
{
 ...
 */
static diagnostics_relay_error_t
diagnostics_relay_send(diagnostics_relay_client_t client, plist_t plist)
{
	if (!client || !plist)
		return DIAGNOSTICS_RELAY_E_INVALID_ARG;
...
static diagnostics_relay_error_t
internal_diagnostics_relay_action(diagnostics_relay_client_t client,
				  const char *name,
				  diagnostics_relay_action_t flags)
{
	...
	if (flags & DIAGNOSTICS_RELAY_ACTION_FLAG_WAIT_FOR_DISCONNECT) {
		plist_dict_set_item(dict, "WaitForDisconnect",
				    plist_new_bool(1));
	}
...
LIBIMOBILEDEVICE_API diagnostics_relay_error_t diagnostics_relay_restart(
	diagnostics_relay_client_t client, diagnostics_relay_action_t flags)
{
...
LIBIMOBILEDEVICE_API diagnostics_relay_error_t
diagnostics_relay_request_diagnostics(diagnostics_relay_client_t client,
				      const char *type, plist_t *diagnostics)
{
...
LIBIMOBILEDEVICE_API diagnostics_relay_error_t
diagnostics_relay_query_mobilegestalt(diagnostics_relay_client_t client,
				      plist_t keys, plist_t *result)
{
	if (!client || plist_get_node_type(keys) != PLIST_ARRAY ||
	    result == NULL)
		return DIAGNOSTICS_RELAY_E_INVALID_ARG;
...
LIBIMOBILEDEVICE_API diagnostics_relay_error_t
diagnostics_relay_query_ioregistry_entry(diagnostics_relay_client_t client,
					 const char *entry_name,
					 const char *entry_class,
					 plist_t *result)
{
	if (!client || (entry_name == NULL && entry_class == NULL) ||
	    result == NULL)
		return DIAGNOSTICS_RELAY_E_INVALID_ARG;
	...
	plist_t dict = plist_new_dict();
	if (entry_name)
		plist_dict_set_item(dict, "EntryName",
				    plist_new_string(entry_name));
	if (entry_class)
		plist_dict_set_item(dict, "EntryClass",
				    plist_new_string(entry_class));
...
LIBIMOBILEDEVICE_API diagnostics_relay_error_t
diagnostics_relay_query_ioregistry_plane(diagnostics_relay_client_t client,
					 const char *plane, plist_t *result)
{
