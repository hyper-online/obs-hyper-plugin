...
LIBIMOBILEDEVICE_API preboard_error_t
preboard_client_new(idevice_t device, lockdownd_service_descriptor_t service,
		    preboard_client_t *client)
{
	*client = NULL;
	...
	if (!device || !service || service->port == 0 || !client || *client) {
		debug_info(
			"Incorrect parameter passed to preboard_client_new.");
		return PREBOARD_E_INVALID_ARG;
	}
	...
	preboard_error_t ret = preboard_error(
	...
	preboard_client_t client_loc = (preboard_client_t)malloc(
		sizeof(struct preboard_client_private));
...
LIBIMOBILEDEVICE_API preboard_error_t preboard_client_start_service(
	idevice_t device, preboard_client_t *client, const char *label)
{
	preboard_error_t err = PREBOARD_E_UNKNOWN_ERROR;
	service_client_factory_start_service(
		device, PREBOARD_SERVICE_NAME, (void **)client, label,
		SERVICE_CONSTRUCTOR(preboard_client_new), &err);
	return err;
}

LIBIMOBILEDEVICE_API preboard_error_t
preboard_client_free(preboard_client_t client)
{
	if (!client)
		return PREBOARD_E_INVALID_ARG;
	...
	preboard_error_t err =
		preboard_error(property_list_service_client_free(parent));
	free(client);

	return err;
}

LIBIMOBILEDEVICE_API preboard_error_t preboard_send(preboard_client_t client,
						    plist_t plist)
{
...
LIBIMOBILEDEVICE_API preboard_error_t preboard_receive_with_timeout(
	preboard_client_t client, plist_t *plist, uint32_t timeout_ms)
{
	preboard_error_t res = PREBOARD_E_UNKNOWN_ERROR;
	plist_t outplist = NULL;
	res = preboard_error(property_list_service_receive_plist_with_timeout(
		client->parent, &outplist, timeout_ms));
...
LIBIMOBILEDEVICE_API preboard_error_t preboard_receive(preboard_client_t client,
						       plist_t *plist)
{
...
static void *preboard_receive_status_loop_thread(void *arg)
{
	...
	while (data->client && data->client->parent) {
		...
		preboard_error_t perr =
			preboard_receive_with_timeout(data->client, &pl, 1000);
...
static preboard_error_t
preboard_receive_status_loop_with_callback(preboard_client_t client,
					   ...
					   void *user_data)
{
	...
	preboard_error_t res = PREBOARD_E_UNKNOWN_ERROR;
	struct preboard_status_data *data =
		(struct preboard_status_data *)malloc(
			sizeof(struct preboard_status_data));
	if (data) {
		data->client = client;
		data->cbfunc = status_cb;
		data->user_data = user_data;
		if (thread_new(&client->receive_status_thread,
			       preboard_receive_status_loop_thread,
			       data) == 0) {
...
LIBIMOBILEDEVICE_API preboard_error_t
preboard_create_stashbag(preboard_client_t client, plist_t manifest,
			 preboard_status_cb_t status_cb, void *user_data)
{
	if (!client) {
		return PREBOARD_E_INVALID_ARG;
	}
	...
	plist_dict_set_item(dict, "Command",
			    plist_new_string("CreateStashbag"));
	if (manifest) {
	...
	return preboard_receive_status_loop_with_callback(client, status_cb,
							  user_data);
}

LIBIMOBILEDEVICE_API preboard_error_t
preboard_commit_stashbag(preboard_client_t client, plist_t manifest,
			 preboard_status_cb_t status_cb, void *user_data)
{
	if (!client) {
		return PREBOARD_E_INVALID_ARG;
	...
	plist_t dict = plist_new_dict();
	plist_dict_set_item(dict, "Command",
			    plist_new_string("CommitStashbag"));
	if (manifest) {
		plist_dict_set_item(dict, "Manifest", plist_copy(manifest));
	}
	preboard_error_t perr = preboard_send(client, dict);
