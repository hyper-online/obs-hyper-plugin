...
service_client_new(idevice_t device, lockdownd_service_descriptor_t service,
		   service_client_t *client)
{
	...
	if (idevice_connect(device, service->port, &connection) !=
	    IDEVICE_E_SUCCESS) {
		return SERVICE_E_MUX_ERROR;
	}

	/* create client object */
	service_client_t client_loc =
		(service_client_t)malloc(sizeof(struct service_client_private));
...
LIBIMOBILEDEVICE_API service_error_t service_client_factory_start_service(
	idevice_t device, const char *service_name, void **client,
	const char *label,
	int32_t (*constructor_func)(idevice_t, lockdownd_service_descriptor_t,
				    void **),
	int32_t *error_code)
{
	*client = NULL;
	...
	if (LOCKDOWN_E_SUCCESS !=
	    lockdownd_client_new_with_handshake(device, &lckd, label)) {
	...
	if (constructor_func) {
		...
		ec = service_client_new(device, service,
					(service_client_t *)client);
	}
	if (error_code) {
		*error_code = ec;
	...
	if (ec != SERVICE_E_SUCCESS) {
		debug_info(
			...
			service_name, service->port, ec);
	}

	lockdownd_service_descriptor_free(service);
	service = NULL;

	return (ec == SERVICE_E_SUCCESS) ? SERVICE_E_SUCCESS
					 : SERVICE_E_START_SERVICE_ERROR;
}
...
LIBIMOBILEDEVICE_API service_error_t service_client_free(service_client_t client)
{
	...
	service_error_t err = idevice_to_service_error(
...
LIBIMOBILEDEVICE_API service_error_t service_send(service_client_t client,
						  const char *data,
						  uint32_t size, uint32_t *sent)
{
	...
	if (!client || (client && !client->connection) || !data ||
	    (size == 0)) {
		return SERVICE_E_INVALID_ARG;
	}

	debug_info("sending %d bytes", size);
	res = idevice_to_service_error(idevice_connection_send(
		client->connection, data, size, &bytes));
...
LIBIMOBILEDEVICE_API service_error_t
service_receive_with_timeout(service_client_t client, char *data, uint32_t size,
			     uint32_t *received, unsigned int timeout)
{
	...
	res = idevice_to_service_error(idevice_connection_receive_timeout(
		client->connection, data, size, &bytes, timeout));
...
LIBIMOBILEDEVICE_API service_error_t service_receive(service_client_t client,
						     char *data, uint32_t size,
						     uint32_t *received)
{
	return service_receive_with_timeout(client, data, size, received,
					    30000);
}
...
LIBIMOBILEDEVICE_API service_error_t service_enable_ssl(service_client_t client)
{
	if (!client || !client->connection)
		return SERVICE_E_INVALID_ARG;
	return idevice_to_service_error(
...
LIBIMOBILEDEVICE_API service_error_t
service_disable_bypass_ssl(service_client_t client, uint8_t sslBypass)
{
	if (!client || !client->connection)
		return SERVICE_E_INVALID_ARG;
	return idevice_to_service_error(idevice_connection_disable_bypass_ssl(
		client->connection, sslBypass));
}
