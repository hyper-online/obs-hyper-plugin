 ...
 */
static companion_proxy_error_t
companion_proxy_error(property_list_service_error_t err)
{
	switch (err) {
...
LIBIMOBILEDEVICE_API companion_proxy_error_t companion_proxy_client_new(
	...
	companion_proxy_client_t *client)
{
	*client = NULL;
	...
	if (!device || !service || service->port == 0 || !client || *client) {
		debug_info(
			"Incorrect parameter passed to companion_proxy_client_new.");
		return COMPANION_PROXY_E_INVALID_ARG;
	}

	debug_info("Creating companion_proxy_client, port = %d.",
		   service->port);

	property_list_service_client_t plclient = NULL;
	companion_proxy_error_t ret = companion_proxy_error(
		property_list_service_client_new(device, service, &plclient));
	if (ret != COMPANION_PROXY_E_SUCCESS) {
		debug_info("Creating a property list client failed. Error: %i",
			   ret);
		return ret;
	}

	companion_proxy_client_t client_loc = (companion_proxy_client_t)malloc(
		sizeof(struct companion_proxy_client_private));
...
LIBIMOBILEDEVICE_API companion_proxy_error_t
companion_proxy_client_start_service(idevice_t device,
				     companion_proxy_client_t *client,
				     const char *label)
{
	companion_proxy_error_t err = COMPANION_PROXY_E_UNKNOWN_ERROR;
	service_client_factory_start_service(
		device, COMPANION_PROXY_SERVICE_NAME, (void **)client, label,
		SERVICE_CONSTRUCTOR(companion_proxy_client_new), &err);
	return err;
}

LIBIMOBILEDEVICE_API companion_proxy_error_t
companion_proxy_client_free(companion_proxy_client_t client)
{
	if (!client)
		return COMPANION_PROXY_E_INVALID_ARG;
	...
	companion_proxy_error_t err = companion_proxy_error(
...
LIBIMOBILEDEVICE_API companion_proxy_error_t
companion_proxy_send(companion_proxy_client_t client, plist_t plist)
{
	companion_proxy_error_t res = COMPANION_PROXY_E_UNKNOWN_ERROR;

	res = companion_proxy_error(
		property_list_service_send_binary_plist(client->parent, plist));
...
LIBIMOBILEDEVICE_API companion_proxy_error_t
companion_proxy_receive(companion_proxy_client_t client, plist_t *plist)
{
	companion_proxy_error_t res = COMPANION_PROXY_E_UNKNOWN_ERROR;
	plist_t outplist = NULL;
	res = companion_proxy_error(
		property_list_service_receive_plist_with_timeout(
			client->parent, &outplist, 10000));
	if (res != COMPANION_PROXY_E_SUCCESS &&
	    res != COMPANION_PROXY_E_TIMEOUT) {
...
LIBIMOBILEDEVICE_API companion_proxy_error_t
companion_proxy_get_device_registry(companion_proxy_client_t client,
				    plist_t *paired_devices)
{
	...
	plist_dict_set_item(dict, "Command",
			    plist_new_string("GetDeviceRegistry"));
...
{
	struct companion_proxy_cb_data *data =
		(struct companion_proxy_cb_data *)arg;
	...
	plist_dict_set_item(command, "Command",
			    plist_new_string("StartListeningForDevices"));
...
companion_proxy_start_listening_for_devices(
	companion_proxy_client_t client,
	companion_proxy_device_event_cb_t callback, void *userdata)
{
	...
	companion_proxy_error_t res = COMPANION_PROXY_E_UNKNOWN_ERROR;
	struct companion_proxy_cb_data *data =
		(struct companion_proxy_cb_data *)malloc(
			sizeof(struct companion_proxy_cb_data));
	if (data) {
		data->client = client;
		data->cbfunc = callback;
		data->user_data = userdata;

		if (thread_new(&client->event_thread,
			       companion_proxy_event_thread, data) == 0) {
			res = COMPANION_PROXY_E_SUCCESS;
		} else {
			free(data);
		}
...
LIBIMOBILEDEVICE_API companion_proxy_error_t
companion_proxy_stop_listening_for_devices(companion_proxy_client_t client)
{
	property_list_service_client_t parent = client->parent;
	client->parent = NULL;
	if (client->event_thread) {
...
LIBIMOBILEDEVICE_API companion_proxy_error_t
companion_proxy_get_value_from_registry(companion_proxy_client_t client,
					...
					const char *key, plist_t *value)
{
	...
	plist_t dict = plist_new_dict();
	plist_dict_set_item(dict, "Command",
			    plist_new_string("GetValueFromRegistry"));
	plist_dict_set_item(dict, "GetValueGizmoUDIDKey",
			    plist_new_string(companion_udid));
	...
	if (val) {
		*value = plist_copy(val);
		...
		val = plist_dict_get_item(dict, "Error");
		if (val) {
			if (!plist_string_val_compare(val,
						      "UnsupportedWatchKey")) {
				res = COMPANION_PROXY_E_UNSUPPORTED_KEY;
			} else if (plist_string_val_compare(val,
							    "TimeoutReply")) {
...
LIBIMOBILEDEVICE_API companion_proxy_error_t
companion_proxy_start_forwarding_service_port(companion_proxy_client_t client,
					      ...
					      plist_t options)
{
	...
	plist_t dict = plist_new_dict();
	plist_dict_set_item(dict, "Command",
			    plist_new_string("StartForwardingServicePort"));
	plist_dict_set_item(dict, "GizmoRemotePortNumber",
			    plist_new_uint(remote_port));
	if (service_name) {
		plist_dict_set_item(dict, "ForwardedServiceName",
				    plist_new_string(service_name));
	}
...
LIBIMOBILEDEVICE_API companion_proxy_error_t
companion_proxy_stop_forwarding_service_port(companion_proxy_client_t client,
					     uint16_t remote_port)
{
	if (!client) {
		return COMPANION_PROXY_E_INVALID_ARG;
	...
	plist_t dict = plist_new_dict();
	plist_dict_set_item(dict, "Command",
			    plist_new_string("StopForwardingServicePort"));
	plist_dict_set_item(dict, "GizmoRemotePortNumber",
			    plist_new_uint(remote_port));
