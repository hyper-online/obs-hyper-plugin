...
static void *run_stoc_loop(void *arg)
{
	...
	while (!cdata->stop_stoc && cdata->fd > 0 && cdata->sfd > 0) {
		recv_len = socket_receive_timeout(cdata->sfd, buffer,
						  sizeof(buffer), 0, 5000);
		if (recv_len <= 0) {
...
static void *run_ctos_loop(void *arg)
{
	...
	cdata->stop_stoc = 0;
#ifdef WIN32
	stoc = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)run_stoc_loop,
			    cdata, 0, NULL);
#else
	...
	while (!cdata->stop_ctos && cdata->fd > 0 && cdata->sfd > 0) {
		recv_len = socket_receive_timeout(cdata->fd, buffer,
						  sizeof(buffer), 0, 5000);
		if (recv_len <= 0) {
			if (recv_len == 0 || recv_len == -ETIMEDOUT) {
				// try again
...
static void *acceptor_thread(void *arg)
{
	...
	if (dev->conn_type == CONNECTION_TYPE_NETWORK) {
		...
		if (((char *)dev->conn_data)[1] == 0x02) { // AF_INET
			...
			memcpy(&saddr->sa_data[0], (char *)dev->conn_data + 2,
			       14);
		} else if (((char *)dev->conn_data)[1] ==
			   0x1E) { //AF_INET6 (bsd)
#ifdef AF_INET6
...
#else
			fprintf(stderr,
				"ERROR: Got an IPv6 address but this system doesn't support IPv6\n");
			free(cdata);
			return NULL;
#endif
		} else {
		...
		if (!socket_addr_to_string(saddr, addrtxt, sizeof(addrtxt))) {
			fprintf(stderr,
				"Failed to convert network address: %d (%s)\n",
				errno, strerror(errno));
		}
		fprintf(stdout,
			"Requesting connecion to NETWORK device %s (serial: %s), port %d\n",
			addrtxt, dev->udid, device_port);
		cdata->sfd = socket_connect_addr(saddr, device_port);
	} else if (dev->conn_type == CONNECTION_TYPE_USB) {
		fprintf(stdout,
			"Requesting connecion to USB device handle %d (serial: %s), port %d\n",
...
#ifdef WIN32
		ctos = CreateThread(NULL, 0,
				    (LPTHREAD_START_ROUTINE)run_ctos_loop,
				    cdata, 0, NULL);
...
static void print_usage(int argc, char **argv, int is_error)
{
	...
	fprintf(is_error ? stderr : stdout,
		"Usage: %s [OPTIONS] LOCAL_PORT DEVICE_PORT\n",
		(name ? name + 1 : argv[0]));
	fprintf(is_error ? stderr : stdout,
		...
		while (1) {
			...
			if (c_sock) {
				printf("accepted connection, fd = %d\n",
				       c_sock);
				cdata = (struct client_data *)malloc(
				...
				acceptor = CreateThread(
					NULL, 0,
					(LPTHREAD_START_ROUTINE)acceptor_thread,
					cdata, 0, NULL);
				CloseHandle(acceptor);
#else
				pthread_create(&acceptor, NULL, acceptor_thread,
					       cdata);
