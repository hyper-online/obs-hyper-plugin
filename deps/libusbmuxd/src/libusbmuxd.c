...
#endif
#define LIBUSBMUXD_DEBUG(level, format, ...)                             \
	if (level <= libusbmuxd_debug)                                   \
		fprintf(stderr, ("[" PACKAGE "] " format), __VA_ARGS__); \
	fflush(stderr);
	...
	if (usbmuxd_socket_addr) {
		if (strncmp(usbmuxd_socket_addr, "UNIX:", 5) == 0) {
			...
			if (usbmuxd_socket_addr[5] != '\0') {
				return socket_connect_unix(usbmuxd_socket_addr +
							   5);
			}
#endif
		} else {
			...
			if (p && port > 0) {
				...
				if (usbmuxd_socket_addr[0] == '[') {
					connect_addr =
						strdup(usbmuxd_socket_addr + 1);
					connect_addr[p - usbmuxd_socket_addr -
						     1] = '\0';
					...
					connect_addr =
						strdup(usbmuxd_socket_addr);
					connect_addr[p - usbmuxd_socket_addr] =
						'\0';
				}
				if (connect_addr && *connect_addr != '\0') {
					int res = socket_connect(connect_addr,
								 port);
#ifdef HAVE_INOTIFY
					use_inotify = 0;
#endif
...
static usbmuxd_device_info_t *device_info_from_plist(plist_t props)
{
	...
	devinfo =
		(usbmuxd_device_info_t *)malloc(sizeof(usbmuxd_device_info_t));
	if (!devinfo)
		return NULL;
	...
	if (n && plist_get_node_type(n) == PLIST_STRING) {
		...
		if (strval) {
			char *t = stpncpy(devinfo->udid, strval,
					  sizeof(devinfo->udid) - 1);
			*t = '\0';
	...
	if (n && plist_get_node_type(n) == PLIST_STRING) {
		...
		if (strval) {
			if (strcmp(strval, "USB") == 0) {
				...
				n = plist_dict_get_item(props,
							"NetworkAddress");
				if (n && plist_get_node_type(n) == PLIST_DATA) {
					char *netaddr = NULL;
					uint64_t addr_len = 0;
					plist_get_data_val(n, &netaddr,
							   &addr_len);
					if (netaddr && addr_len > 0 &&
					    addr_len <
						    sizeof(devinfo->conn_data)) {
						memcpy(devinfo->conn_data,
						       netaddr, addr_len);
					}
					free(netaddr);
				}
	...
	if (!devinfo->conn_type) {
		...
		free(devinfo);
		devinfo = NULL;
	} else if (devinfo->conn_type == CONNECTION_TYPE_NETWORK &&
		   !devinfo->conn_data[0]) {
...
static usbmuxd_device_info_t *
device_info_from_device_record(struct usbmuxd_device_record *dev)
{
	if (!dev) {
		return NULL;
	}
	...
	if (!devinfo) {
		LIBUSBMUXD_ERROR(
			"%s: Out of memory while allocating device info object\n",
			__func__);
		return NULL;
	}
	...
	char *t = stpncpy(devinfo->udid, dev->serial_number,
			  sizeof(devinfo->udid) - 2);
	*t = '\0';
	sanitize_udid(devinfo);
...
static int receive_packet(int sfd, struct usbmuxd_header *header,
			  void **payload, int timeout)
{
	int recv_len;
	...
	if (recv_len < 0) {
		...
		LIBUSBMUXD_DEBUG(
			1, "%s: Received packet is too small, got %d bytes!\n",
			__func__, recv_len);
		return recv_len;
	}
	...
	if (payload_size > 0) {
		...
		uint32_t rsize = 0;
		do {
			int res = socket_receive_timeout(sfd,
							 payload_loc + rsize,
							 payload_size - rsize,
							 0, 5000);
			if (res < 0) {
	...
	if (hdr.message == MESSAGE_PLIST) {
		...
		if (!plist) {
			LIBUSBMUXD_DEBUG(
				1, "%s: Error getting plist from payload!\n",
				__func__);
			return -EBADMSG;
		}
		...
		if (message) {
			...
			if (strcmp(message, "Result") == 0) {
				...
				uint32_t dwval = 0;
				plist_t n =
					plist_dict_get_item(plist, "Number");
				plist_get_uint_val(n, &val);
				*payload = malloc(sizeof(uint32_t));
				...
				/* device add message */
				usbmuxd_device_info_t *devinfo = NULL;
				plist_t props = plist_dict_get_item(
					plist, "Properties");
				if (!props) {
					LIBUSBMUXD_DEBUG(
						1,
				...
				if (!devinfo) {
					LIBUSBMUXD_DEBUG(
						1,
						"%s: Could not create device info object from properties!\n",
						__func__);
					free(message);
					plist_free(plist);
					return -EBADMSG;
				...
				*payload = (void *)devinfo;
				...
				LIBUSBMUXD_DEBUG(
					1,
					"%s: Unexpected message '%s' in plist:\n%s\n",
		...
		usbmuxd_device_info_t *devinfo = device_info_from_device_record(
			(struct usbmuxd_device_record *)payload_loc);
		free(payload_loc);
		*payload = devinfo;
	} else {
		*payload = payload_loc;
	...
	*result = -1;
	...
	if (hdr.message == MESSAGE_RESULT) {
		...
		if (!result_plist) {
			LIBUSBMUXD_DEBUG(
				1,
				"%s: MESSAGE_PLIST result but result_plist pointer is NULL!\n",
				__func__);
			return -1;
		}
...
static int send_packet(int sfd, uint32_t message, uint32_t tag, void *payload,
		       uint32_t payload_size)
{
	...
	if (payload && (payload_size > 0)) {
		uint32_t ssize = 0;
		do {
			int res = socket_send(sfd, (char *)payload + ssize,
					      payload_size - ssize);
			if (res < 0) {
				break;
			}
	...
	if (sent != (int)header.length) {
		LIBUSBMUXD_DEBUG(
			1,
			"%s: ERROR: could not send whole packet (sent %d of %d)\n",
...
static void get_bundle_id()
{
	...
	if (fread(buf, 1, fsize, f) == fsize) {
		...
		if (pl) {
			plist_t bid =
				plist_dict_get_item(pl, "CFBundleIdentifier");
...
static void get_prog_name()
{
	...
	if (GetModuleFileName(NULL, _pname, MAX_PATH + 1) > 0) {
		...
		char *__pname = NULL;
		int l = WideCharToMultiByte(CP_UTF8, 0, _pname, -1, NULL, 0,
					    NULL, NULL);
		if (l > 0) {
			__pname = malloc(l);
			if (WideCharToMultiByte(CP_UTF8, 0, _pname, -1, __pname,
						l, NULL, NULL) > 0) {
				pname = __pname;
			}
		}
...
static plist_t create_plist_message(const char *message_type)
{
	...
	if (bundle_id) {
		plist_dict_set_item(plist, "BundleID",
				    plist_new_string(bundle_id));
	}
	plist_dict_set_item(plist, "ClientVersionString",
			    plist_new_string(PLIST_CLIENT_VERSION_STRING));
	plist_dict_set_item(plist, "MessageType",
			    plist_new_string(message_type));
	if (prog_name) {
		plist_dict_set_item(plist, "ProgName",
				    plist_new_string(prog_name));
	}
	plist_dict_set_item(plist, "kLibUSBMuxVersion",
			    plist_new_uint(PLIST_LIBUSBMUX_VERSION));
	return plist;
}
...
static int send_connect_packet(int sfd, uint32_t tag, uint32_t device_id,
			       uint16_t port)
{
	int res = 0;
	if (proto_version == 1) {
		...
		plist_dict_set_item(plist, "DeviceID",
				    plist_new_uint(device_id));
		plist_dict_set_item(plist, "PortNumber",
				    plist_new_uint(htons(port)));
		...
		res = send_packet(sfd, MESSAGE_CONNECT, tag, &conninfo,
				  sizeof(conninfo));
	}
	return res;
}
...
static int send_pair_record_packet(int sfd, uint32_t tag, const char *msgtype,
				   const char *pair_record_id,
				   uint32_t device_id, plist_t data)
{
	int res = -1;

	/* construct message plist */
	plist_t plist = create_plist_message(msgtype);
	plist_dict_set_item(plist, "PairRecordID",
			    plist_new_string(pair_record_id));
	if (data) {
 ...
 */
static void generate_event(const usbmuxd_device_info_t *dev,
			   enum usbmuxd_event_type event)
{
	usbmuxd_event_t ev;
...
#ifdef HAVE_INOTIFY
#ifndef HAVE_PSELECT
static int pselect(int nfds, fd_set *readfds, fd_set *writefds,
...
static int usbmuxd_listen_inotify()
{
	...
	if (watch_d < 0) {
		LIBUSBMUXD_DEBUG(
			1,
			"%s: Failed to setup watch descriptor for socket dir\n",
			__func__);
		close(inot_fd);
		return -2;
	}
...
retry:
	...
	if (sfd < 0) {
		if (!cancelling) {
			LIBUSBMUXD_DEBUG(
				1,
				"%s: ERROR: usbmuxd was supposed to be running here...\n",
				__func__);
		}
		return sfd;
	}
...
static int get_next_event(int sfd)
{
	...
	if ((hdr.length > sizeof(hdr)) && !payload) {
		LIBUSBMUXD_DEBUG(
			1, "%s: Invalid packet received, payload is missing!\n",
			__func__);
		return -EBADMSG;
	}
	...
	if (hdr.message == MESSAGE_DEVICE_ADD) {
		...
		LIBUSBMUXD_DEBUG(
			1,
			"%s: Unexpected message type %d length %d received!\n",
...
USBMUXD_API int usbmuxd_events_subscribe(usbmuxd_subscription_context_t *ctx,
					 usbmuxd_event_cb_t callback,
					 void *user_data)
{
	if (!ctx || !callback) {
...
retry:
	...
	if ((proto_version == 1) && (try_list_devices)) {
		if (send_list_devices_packet(sfd, tag) > 0) {
			plist_t list = NULL;
			if ((usbmuxd_get_result(sfd, tag, &res, &list) == 1) &&
			    (res == 0)) {
				plist_t devlist =
					plist_dict_get_item(list, "DeviceList");
				if (devlist && plist_get_node_type(devlist) ==
						       PLIST_ARRAY) {
					collection_init(&tmpdevs);
					uint32_t numdevs =
						plist_array_get_size(devlist);
					uint32_t i;
					for (i = 0; i < numdevs; i++) {
						plist_t pdev =
...
got_device_list:
	...
	newlist = (usbmuxd_device_info_t *)malloc(
		sizeof(usbmuxd_device_info_t) *
		(collection_count(&tmpdevs) + 1));
	dev_cnt = 0;
	FOREACH(usbmuxd_device_info_t * di, &tmpdevs)
	{
		if (di) {
			memcpy(&newlist[dev_cnt], di,
			       sizeof(usbmuxd_device_info_t));
			free(di);
			dev_cnt++;
		}
...
USBMUXD_API int usbmuxd_get_device_by_udid(const char *udid,
					   usbmuxd_device_info_t *device)
{
	...
	if (dev) {
		...
		char *t = stpncpy(device->udid, dev->udid,
				  sizeof(device->udid) - 1);
		*t = '\0';
		device->conn_type = dev->conn_type;
		memcpy(device->conn_data, dev->conn_data,
		       sizeof(device->conn_data));
		result = 1;
	}
...
USBMUXD_API int usbmuxd_get_device(const char *udid,
				   usbmuxd_device_info_t *device,
				   enum usbmux_lookup_options options)
{
	...
	for (i = 0; dev_list[i].handle > 0; i++) {
		if (!udid) {
			if ((options & DEVICE_LOOKUP_USBMUX) &&
			    (dev_list[i].conn_type == CONNECTION_TYPE_USB)) {
				dev_usbmuxd = &dev_list[i];
				break;
			} else if ((options & DEVICE_LOOKUP_NETWORK) &&
				   (dev_list[i].conn_type ==
	...
	if (dev_network && dev_usbmuxd) {
		dev = (options & DEVICE_LOOKUP_PREFER_NETWORK) ? dev_network
							       : dev_usbmuxd;
	} else if (dev_network) {
		dev = dev_network;
	} else if (dev_usbmuxd) {
...
USBMUXD_API int usbmuxd_connect(const uint32_t handle,
...
retry:
	...
	if (sfd < 0) {
		result = errno;
		LIBUSBMUXD_DEBUG(
			1, "%s: Error: Connection to usbmuxd failed: %s\n",
			__func__, strerror(result));
		return -result;
	}

	tag = ++use_tag;
	if (send_connect_packet(sfd, tag, (uint32_t)handle, (uint16_t)port) <=
	    0) {
...
USBMUXD_API int usbmuxd_send(int sfd, const char *data, uint32_t len,
			     uint32_t *sent_bytes)
{
	int num_sent;
	...
	if (num_sent < 0) {
		*sent_bytes = 0;
		...
		LIBUSBMUXD_DEBUG(
			1, "%s: Warning: Did not send enough (only %d of %d)\n",
...
USBMUXD_API int usbmuxd_recv_timeout(int sfd, char *data, uint32_t len,
				     uint32_t *recv_bytes, unsigned int timeout)
{
	int num_recv =
		socket_receive_timeout(sfd, (void *)data, len, 0, timeout);
	if (num_recv < 0) {
		*recv_bytes = 0;
		return num_recv;
	}
...
USBMUXD_API int usbmuxd_recv(int sfd, char *data, uint32_t len,
			     uint32_t *recv_bytes)
{
...
USBMUXD_API int usbmuxd_read_pair_record(const char *record_id,
					 char **record_data,
					 uint32_t *record_size)
{
	...
	*record_size = 0;
	...
	if (send_pair_record_packet(sfd, tag, "ReadPairRecord", record_id, 0,
				    NULL) <= 0) {
		LIBUSBMUXD_DEBUG(1,
		...
		if ((ret == 1) && (rc == 0)) {
			plist_t node =
				plist_dict_get_item(pl, "PairRecordData");
			if (node && plist_get_node_type(node) == PLIST_DATA) {
				uint64_t int64val = 0;
				plist_get_data_val(node, record_data,
						   &int64val);
...
USBMUXD_API int usbmuxd_save_pair_record_with_device_id(const char *record_id,
							uint32_t device_id,
							...
							uint32_t record_size)
{
	...
	if (send_pair_record_packet(sfd, tag, "SavePairRecord", record_id,
				    device_id, data) <= 0) {
		LIBUSBMUXD_DEBUG(1,
		...
		if ((ret == 1) && (rc == 0)) {
			...
			ret = -(int)rc;
			LIBUSBMUXD_DEBUG(
				1, "%s: Error: saving pair record failed: %d\n",
				__func__, ret);
		}
	}
	plist_free(data);
...
USBMUXD_API int usbmuxd_save_pair_record(const char *record_id,
					 const char *record_data,
					 uint32_t record_size)
{
	return usbmuxd_save_pair_record_with_device_id(
		record_id, 0, record_data, record_size);
}
...
USBMUXD_API int usbmuxd_delete_pair_record(const char *record_id)
{
	...
	if (send_pair_record_packet(sfd, tag, "DeletePairRecord", record_id, 0,
				    NULL) <= 0) {
		LIBUSBMUXD_DEBUG(
			1, "%s: Error sending DeletePairRecord message!\n",
			__func__);
	} else {
		uint32_t rc = 0;
		ret = usbmuxd_get_result(sfd, tag, &rc, NULL);
		if ((ret == 1) && (rc == 0)) {
			...
			LIBUSBMUXD_DEBUG(
				1,
				"%s: Error: deleting pair record failed: %d\n",
				__func__, ret);
		}
	}
	socket_close(sfd);
